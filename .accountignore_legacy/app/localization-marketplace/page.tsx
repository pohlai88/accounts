...
export default function LocalizationMarketplacePage() {
    ...
    const handleActivatePack = async (packId: string, licenseType: string = 'Standard') => {
        const result = await LocalizationService.activateLocalizationPack(companyId, packId, licenseType as any)
        if (result.success) {
            await loadCompanySettings()
        }
    }
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig = {
            'Active': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' },
            'Inactive': { variant: 'secondary' as const, className: 'bg-muted text-muted-foreground' },
            'Suspended': { variant: 'destructive' as const, className: 'bg-destructive/10 text-destructive border-destructive/20' },
            'Expired': { variant: 'destructive' as const, className: 'bg-destructive/10 text-destructive border-destructive/20' },
            'Trial': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' },
            'Standard': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' },
            'Premium': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' },
            'Enterprise': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' },
            'Published': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' },
            'Certified': { variant: 'default' as const, className: 'bg-primary/10 text-primary border-primary/20' }
        }
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {packPricing.slice(0, 9).map((pack) => (
                                <Card key={pack.pack_id} className="relative overflow-hidden hover:shadow-lg transition-shadow">
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        <div className="text-center p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
                                            <div className="flex items-center justify-center space-x-2">
                                                <span className="text-2xl font-bold">{formatCurrency(pack.monthly_price)}</span>
                        ...
                        <Card key={country.id} className="hover:shadow-md transition-shadow">
                            ...
                            <CardContent className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span>Currency:</span>
                                    <span className="font-medium">{country.currency_symbol} {country.currency_code}</span>
                                </div>
                                <div className="flex justify-between">
        ...
        <div className="space-y-4">
            ...
            {loading ? (
                ...
                <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground mb-4">Browse the marketplace to find packs for your region</p>
                    <Button onClick={() => setActiveTab('marketplace')}>
                            ...
                            <CardContent className="space-y-2 text-sm">
                                ...
                                {setting.max_transactions && (
                                    <div className="flex justify-between">
                                        ...
                                        <span>{setting.transactions_used} / {setting.max_transactions}</span>
                                    </div>
                                )}
                                ...
                                <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-muted-foreground">
                                            {((pack.activation_count / (localizationStats?.active_companies || 1)) * 100).toFixed(1)}% of companies
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-blue-100 mb-4">
                                Expand your business globally with country-specific accounting rules, tax systems, and compliance frameworks.
                                Our localization packs ensure you're compliant and competitive in any market.
                            </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        <div className="flex justify-between">
                                            <span>Monthly:</span>
                                            <span className="font-medium">{formatCurrency(selectedPack.monthly_price)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Annual:</span>
                                            <span className="font-medium">{formatCurrency(selectedPack.annual_price)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Setup Fee:</span>
