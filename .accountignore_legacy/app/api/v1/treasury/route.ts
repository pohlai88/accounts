...
export async function GET(request: NextRequest) {
    try {
        ...
        if (params.search) {
            query = query.or(`forecast_name.ilike.%${params.search}%,scenario_name.ilike.%${params.search}%`)
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (fetchError) {
            return NextResponse.json(
                { error: 'Treasury forecast created but failed to fetch details', details: fetchError.message },
                { status: 201 }
            )
        }
