    ...
    try {
        ...
        if (!transactionType) {
            return NextResponse.json(
                { error: 'Transaction type is required. Use ?type=sales_invoice|payment_entry|journal_entry' },
                { status: 400 }
            )
        }
...
async function handleJournalEntry(data: any) {
    ...
    const voucherNo = validatedData.voucher_no || ERPNextAccountingEngine.generateVoucherNo('Journal Entry')
...
export async function GET(request: NextRequest) {
    try {
        ...
        switch (action) {
            ...
            default:
                return NextResponse.json(
                    { error: 'Invalid action. Use ?action=balance' },
                    { status: 400 }
                )
        }
