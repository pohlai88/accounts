...
const fixedAssetSchema = z.object({
    ...
    depreciation_method: z.enum(['Straight Line', 'Double Declining Balance', 'Manual']).default('Straight Line'),
    ...
    status: z.enum(['Draft', 'Submitted', 'Partially Depreciated', 'Fully Depreciated', 'Sold', 'Scrapped', 'Capitalized', 'Decapitalized']).default('Draft'),
})
...
const querySchema = z.object({
    ...
    maintenance_required: z.string().optional().transform(val => val === 'true'),
    insurance_required: z.string().optional().transform(val => val === 'true'),
    sort_by: z.enum(['asset_name', 'purchase_date', 'purchase_amount', 'current_value']).default('asset_name'),
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Apply filters
        if (params.search) {
            query = query.or(`asset_name.ilike.%${params.search}%,item_code.ilike.%${params.search}%,location.ilike.%${params.search}%`)
        }
