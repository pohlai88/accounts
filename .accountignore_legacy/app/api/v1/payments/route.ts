...
export async function GET(request: NextRequest) {
  try {
    ...
    if (params.search) {
      query = query.or(
        `payment_no.ilike.%${params.search}%,party_name.ilike.%${params.search}%,reference_no.ilike.%${params.search}%`,
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const receivedAmount =
      validatedData.received_amount ||
      (validatedData.paid_amount * validatedData.source_exchange_rate) /
        validatedData.target_exchange_rate;
