...
export async function GET(request: NextRequest) {
    try {
        ...
        if (params.search) {
            query = query.or(`invoice_no.ilike.%${params.search}%,supplier_name.ilike.%${params.search}%,supplier_invoice.ilike.%${params.search}%`)
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (fetchError) {
            return NextResponse.json(
                { error: 'Purchase invoice created but failed to fetch details', details: fetchError.message },
                { status: 201 }
            )
        }
