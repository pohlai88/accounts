...
const querySchema = z.object({
    ...
    department: z.string().optional(),
    project: z.string().optional(),
    status: z.enum(['Draft', 'Submitted', 'Approved by Manager', 'Approved by Finance', 'Rejected', 'Paid', 'Cancelled']).optional(),
    from_date: z.string().optional(),
    to_date: z.string().optional(),
    search: z.string().optional(),
    sort_by: z.enum(['posting_date', 'total_claimed_amount', 'employee_name']).default('posting_date'),
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (params.search) {
            query = query.or(`name.ilike.%${params.search}%,employee_name.ilike.%${params.search}%,remarks.ilike.%${params.search}%`)
        }
...
function calculateExpenseTotals(expenses: any[]) {
    ...
    const totalSanctionedAmount = expenses.reduce((sum, expense) => sum + (expense.sanctioned_amount || expense.amount), 0)
