...
export default function CreditNotesPage() {
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig = {
            ...
            Overdue: { variant: 'destructive' as const, label: 'Overdue', color: 'bg-red-100 text-red-800' },
            Cancelled: { variant: 'outline' as const, label: 'Cancelled', color: 'bg-gray-100 text-gray-800' }
        }
    ...
    const getTotalStats = () => {
        ...
        const outstandingAmount = creditNotes.reduce((sum, cn) => sum + Math.abs(cn.outstanding_amount), 0)
        ...
        <div className="max-w-7xl mx-auto p-6 space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        {filteredCreditNotes.length} credit notes found
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {loading ? (
                                ...
                                <TableBody>
                                    {filteredCreditNotes.map((creditNote) => (
                                        <TableRow key={creditNote.id}>
                                            ...
                                            <TableCell>
                                                <div>
                                                    <div className="font-medium">{creditNote.customer_name || creditNote.supplier_name}</div>
                                                    <div className="text-sm text-muted-foreground">
                                            ...
                                            <TableCell>
                                                <div className="text-sm">
                                                    <div>Credit Note: {format(new Date(creditNote.invoice_date), 'MMM dd, yyyy')}</div>
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    {creditNote.outstanding_amount !== 0 && (
                                                        <div className="text-xs text-muted-foreground">
                                                            Outstanding: {creditNote.currency} {creditNote.outstanding_amount.toFixed(2)}
                                                        </div>
                                                    )}
                                                </div>
                                            </TableCell>
            ...
            <Dialog open={showCreditNoteDetails} onOpenChange={setShowCreditNoteDetails}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>
                            Credit Note Details - {selectedCreditNote?.invoice_no}
                        </DialogTitle>
                        <DialogDescription>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        ...
                                        <div className="flex justify-between">
                                            <span>Date:</span>
                                            <span>{format(new Date(selectedCreditNote.invoice_date), 'MMM dd, yyyy')}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Due Date:</span>
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        <div className="flex justify-between">
                                            <span>Net Total:</span>
                                            <span className="font-medium text-red-600">{selectedCreditNote.currency} {selectedCreditNote.net_total.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Tax Total:</span>
                                            <span className="font-medium text-red-600">{selectedCreditNote.currency} {selectedCreditNote.tax_total.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Grand Total:</span>
                                            <span className="font-medium text-red-600">{selectedCreditNote.currency} {selectedCreditNote.grand_total.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Paid Amount:</span>
                                            <span className="font-medium text-red-600">{selectedCreditNote.currency} {selectedCreditNote.paid_amount.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Outstanding:</span>
                                            <span className="font-medium text-red-600">{selectedCreditNote.currency} {selectedCreditNote.outstanding_amount.toFixed(2)}</span>
                                        </div>
                                    </CardContent>
                                </Card>
                            </div>
