...
export default function BankReconciliationPage() {
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig = {
            ...
            Disputed: { variant: 'destructive' as const, label: 'Disputed', color: 'bg-red-100 text-red-800' },
            Imported: { variant: 'default' as const, label: 'Imported', color: 'bg-purple-100 text-purple-800' },
            Reconciled: { variant: 'default' as const, label: 'Reconciled', color: 'bg-green-100 text-green-800' },
    ...
    const getTotalStats = () => {
        ...
        const pendingReconciliations = reconciliations.filter(r => r.status === 'Draft' || r.status === 'In Progress').length
        const totalDifference = reconciliations.reduce((sum, r) => sum + Math.abs(r.difference), 0)

        return { totalReconciliations, completedReconciliations, pendingReconciliations, totalDifference }
    }
                                    ...
                                    <TableCell>
                                        <div>
                                            ...
                                            <div className="text-sm text-muted-foreground">
                                                {format(new Date(reconciliation.period_start), 'MMM dd')} - {format(new Date(reconciliation.period_end), 'MMM dd, yyyy')}
                                            </div>
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                        <div className="text-sm">
                                            {bankAccounts.find(ba => ba.id === reconciliation.bank_account_id)?.account_name || 'Unknown'}
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                        <div className="text-sm">
                                            {format(new Date(reconciliation.period_start), 'MMM dd')} - {format(new Date(reconciliation.period_end), 'MMM dd')}
                                        </div>
                                    </TableCell>
                                    <TableCell>
                        ...
                        <TableBody>
                            {bankStatements.map((statement) => (
                                <TableRow key={statement.id}>
                                    <TableCell>
                                        <div>
                                            <div className="font-medium">{statement.statement_reference || 'Statement'}</div>
                                            <div className="text-sm text-muted-foreground">
                                    ...
                                    <TableCell>
                                        <div className="text-sm">
                                            {bankAccounts.find(ba => ba.id === statement.bank_account_id)?.account_name || 'Unknown'}
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                        <div className="text-sm">
            ...
            <Dialog open={showStatementDetails} onOpenChange={setShowStatementDetails}>
                <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            View and manage bank statement transactions
                        </DialogDescription>
                    </DialogHeader>
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        <div className="flex justify-between">
                                            ...
                                            <span>{format(new Date(selectedStatement.statement_date), 'MMM dd, yyyy')}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Opening Balance:</span>
                                            <span className="font-medium">${selectedStatement.opening_balance.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Closing Balance:</span>
                                            <span className="font-medium">${selectedStatement.closing_balance.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-medium text-red-600">${selectedStatement.total_debits.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Total Credits:</span>
                                            <span className="font-medium text-green-600">${selectedStatement.total_credits.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Net Change:</span>
                                            <span className="font-medium">
                                                ${(selectedStatement.closing_balance - selectedStatement.opening_balance).toFixed(2)}
                                            </span>
                                        </div>
                                    </CardContent>
                                </Card>
                                            ...
                                            <TableBody>
                                                {statementTransactions.map((transaction) => (
                                                    <TableRow key={transaction.id}>
                                                        ...
                                                        <TableCell>
                                                            <div className="flex items-center space-x-2">
                                                                {getMatchingStatusIcon(transaction.is_matched, transaction.matching_confidence)}
