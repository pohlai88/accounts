...
export default function AdvancedReportsPage() {
  ...
  const loadFinancialRatios = async () => {
    const result = await AdvancedReportsService.getFinancialRatios(
      companyId,
      dateRange.start,
      dateRange.end,
    );
  ...
  const handleCalculateRatios = async () => {
    setLoading(true);
    try {
      await AdvancedReportsService.calculateFinancialRatios(
        companyId,
        dateRange.start,
        dateRange.end,
      );
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {formatPercentage(dashboardData?.gross_profit_margin || 0)}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {dashboardData?.current_ratio?.toFixed(2) || "0.00"}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Quick Ratio:</span>
              <span className="font-medium">
                {dashboardData?.quick_ratio?.toFixed(2) || "0.00"}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {dashboardData?.receivables_turnover?.toFixed(2) || "0.00"}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Payables Turnover:</span>
              <span className="font-medium">
                {dashboardData?.payables_turnover?.toFixed(2) || "0.00"}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {formatPercentage(dashboardData?.return_on_assets || 0)}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Return on Equity:</span>
              <span className="font-medium">
                {formatPercentage(dashboardData?.return_on_equity || 0)}
              </span>
            </div>
          </CardContent>
        </Card>
            ...
            <Card key={ratio.id}>
              <CardHeader>
                <CardTitle className="text-lg">
                  {format(new Date(ratio.period_start), "MMM yyyy")} -{" "}
                  {format(new Date(ratio.period_end), "MMM yyyy")}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {formatPercentage(ratio.gross_profit_margin || 0)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Net Margin:</span>
                  <span className="font-medium">
                    {formatPercentage(ratio.net_profit_margin || 0)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>ROA:</span>
                  <span className="font-medium">
                    {formatPercentage(ratio.return_on_assets || 0)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>ROE:</span>
                  <span className="font-medium">
                    {formatPercentage(ratio.return_on_equity || 0)}
                  </span>
                </div>
              </CardContent>
            </Card>
                ...
                <TableBody>
                  {agedReceivables.map(receivable => (
                    <TableRow key={receivable.id}>
                      ...
                      <TableCell>
                        {formatCurrency(receivable.current_amount, receivable.currency)}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(receivable.days_1_30, receivable.currency)}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(receivable.days_31_60, receivable.currency)}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(receivable.days_61_90, receivable.currency)}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(receivable.days_91_120, receivable.currency)}
                      </TableCell>
                      <TableCell>
                        {formatCurrency(receivable.days_over_120, receivable.currency)}
                      </TableCell>
                ...
                <TableBody>
                  {agedPayables.map(payable => (
                    <TableRow key={payable.id}>
                      ...
                      <TableCell>
                        {formatCurrency(payable.current_amount, payable.currency)}
                      </TableCell>
                      ...
                      <TableCell>
                        {formatCurrency(payable.days_over_120, payable.currency)}
                      </TableCell>
                      <TableCell className="font-medium">
