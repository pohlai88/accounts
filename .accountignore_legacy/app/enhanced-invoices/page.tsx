...
export default function EnhancedInvoicesPage() {
    ...
    const getTypeBadge = (type: string) => {
        const typeConfig = {
            ...
            Purchase: { variant: 'secondary' as const, label: 'Purchase', color: 'bg-green-100 text-green-800' },
            'Credit Note': { variant: 'outline' as const, label: 'Credit Note', color: 'bg-purple-100 text-purple-800' },
            'Debit Note': { variant: 'outline' as const, label: 'Debit Note', color: 'bg-orange-100 text-orange-800' }
        }
                        ...
                        <TableBody>
                            {invoices.map((invoice) => (
                                <TableRow key={invoice.id}>
                                    <TableCell>
                                        <div className="flex items-center space-x-3">
                                            <div className="flex-shrink-0">
                                                {getWorkflowStatusIcon(invoice.status)}
                                            </div>
                                            <div>
                                    ...
                                    <TableCell>
                                        <div>
                                            <div className="font-medium">{invoice.customer_name || invoice.supplier_name}</div>
                                            <div className="text-sm text-muted-foreground">
                        ...
                        <Card key={template.id}>
                            ...
                            <CardContent>
                                <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between">
                                        <span>Font:</span>
                                        <span>{template.font_family} {template.font_size}px</span>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        <div className="flex justify-between">
                                            <span>Net Total:</span>
                                            <span className="font-medium">{selectedInvoice.currency} {selectedInvoice.net_total.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Tax Total:</span>
                                            <span className="font-medium">{selectedInvoice.currency} {selectedInvoice.tax_total.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Grand Total:</span>
                                            <span className="font-medium text-primary">{selectedInvoice.currency} {selectedInvoice.grand_total.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Paid Amount:</span>
                                            <span className="font-medium">{selectedInvoice.currency} {selectedInvoice.paid_amount.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Outstanding:</span>
                                            <span className="font-medium">{selectedInvoice.currency} {selectedInvoice.outstanding_amount.toFixed(2)}</span>
                                        </div>
                                    </CardContent>
                                </Card>
                            </div>
