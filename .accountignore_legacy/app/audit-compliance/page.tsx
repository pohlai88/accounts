...
export default function AuditCompliancePage() {
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig = {
            ...
            'Partially Compliant': { variant: 'default' as const, color: 'bg-yellow-100 text-yellow-800' },
        ...
        const config = statusConfig[status as keyof typeof statusConfig] || statusConfig['Not Assessed']
                                    ...
                                    <CardContent>
                                        <div className="space-y-2 text-sm">
                                            <div className="flex justify-between">
                                                ...
                                                <span>{framework.last_assessment_date ?
                                                    format(new Date(framework.last_assessment_date), 'MMM dd, yyyy') : 'Never'}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Next Due:</span>
                                                <span>{framework.next_assessment_due ?
                                                    format(new Date(framework.next_assessment_due), 'MMM dd, yyyy') : 'Not Set'}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Violations:</span>
                                                <span className={framework.violations_count > 0 ? 'text-red-600 font-medium' : ''}>
                                                    {framework.violations_count || 0}
                                                </span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Critical:</span>
                                                <span className={framework.high_severity_violations > 0 ? 'text-red-600 font-medium' : ''}>
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center space-x-2">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold">{auditStats?.financial_data_access_events || 0}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                        ...
                        <TableBody>
                            {accessLogs.map((log) => (
                                <TableRow key={log.id}>
                                    ...
                                    <TableCell>
                                        {getStatusBadge(log.data_classification)}
                                    </TableCell>
                                    <TableCell>
                            ...
                            {complianceFrameworks.map((framework) => (
                                <Card key={framework.id}>
                                    <CardHeader>
                                        ...
                                        <CardDescription>{framework.framework_code} v{framework.version}</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                            ...
                            {complianceViolations.slice(0, 5).map((violation) => (
                                <div key={violation.id} className="border rounded-lg p-4">
                                    ...
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                        <span>Detected: {format(new Date(violation.detection_date), 'MMM dd, yyyy')}</span>
