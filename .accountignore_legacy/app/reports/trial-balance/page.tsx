...
export default function TrialBalancePage() {
    ...
    const [sortBy, setSortBy] = useState<'account_name' | 'account_code' | 'account_type' | 'debit_balance' | 'credit_balance'>('account_name')
    ...
    const getAccountTypeColor = (accountType: string) => {
        ...
        return typeConfig[accountType as keyof typeof typeConfig] || 'text-muted-foreground bg-muted border-border'
    }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <Select
                                ...
                                onValueChange={(value) => setFilters(prev => ({ ...prev, accountType: value as any }))}
                            >
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-6">
                                <div className="text-center">
                                    ...
                                    <p className="text-lg font-bold">{formatCurrency(trialBalanceData.total_debits, filters.currency)}</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-sm text-muted-foreground">Total Credits</p>
                                    <p className="text-lg font-bold">{formatCurrency(trialBalanceData.total_credits, filters.currency)}</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-sm text-muted-foreground">Difference</p>
                                    <p className={`text-lg font-bold ${Math.abs(trialBalanceData.total_debits - trialBalanceData.total_credits) < 0.01 ? 'text-primary' : 'text-destructive'}`}>
                                        {formatCurrency(trialBalanceData.total_debits - trialBalanceData.total_credits, filters.currency)}
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            {loading ? (
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            As of {format(new Date(trialBalanceData.as_of_date), 'MMMM dd, yyyy')} • {filteredAndSortedAccounts().length} accounts
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    {filteredAndSortedAccounts().map((account, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell className="text-right font-medium">
                                                {account.debit_balance > 0 ? formatCurrency(account.debit_balance, filters.currency) : '-'}
                                            </TableCell>
                                            <TableCell className="text-right font-medium">
                                                {account.credit_balance > 0 ? formatCurrency(account.credit_balance, filters.currency) : '-'}
                                            </TableCell>
                                        </TableRow>
                                    ))}
