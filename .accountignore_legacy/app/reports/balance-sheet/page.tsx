  ...
  const loadComparisonData = async () => {
    try {
      ...
      if (filters.comparisonDate === 'previous_month') {
        ...
        comparisonDate = format(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, currentDate.getDate()), 'yyyy-MM-dd')
      } else if (filters.comparisonDate === 'previous_quarter') {
        const currentDate = new Date(filters.asOfDate)
        comparisonDate = format(new Date(currentDate.getFullYear(), currentDate.getMonth() - 3, currentDate.getDate()), 'yyyy-MM-dd')
      } else if (filters.comparisonDate === 'previous_year') {
        const currentDate = new Date(filters.asOfDate)
        comparisonDate = format(new Date(currentDate.getFullYear() - 1, currentDate.getMonth(), currentDate.getDate()), 'yyyy-MM-dd')
      } else {
        return
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setFilters(prev => ({ ...prev, comparisonDate: value as any }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                ...
                <div>
                  ...
                  <div className="border rounded-lg">
                    <Table>
                      ...
                      <TableBody>
                        {groupAccountsByCategory(balanceSheetData.assets, 'Current Assets').map((item, index) => {
                          const comparisonItem = comparisonData?.assets.find(c => c.account_name === item.account_name)
                ...
                <div>
                  ...
                  <div className="border rounded-lg">
                    <Table>
                      ...
                      <TableBody>
                        {groupAccountsByCategory(balanceSheetData.assets, 'Fixed Assets').map((item, index) => {
                          const comparisonItem = comparisonData?.assets.find(c => c.account_name === item.account_name)
                          const change = comparisonItem ? calculateChange(item.amount, comparisonItem.amount) : 0
                    ...
                    <div className="text-right">
                      ...
                      {comparisonData && (
                        <div className="flex items-center justify-end space-x-1 mt-1 text-sm font-normal">
                          {getChangeIcon(calculateChange(balanceSheetData.totalAssets, comparisonData.totalAssets))}
                          <span className={getChangeColor(calculateChange(balanceSheetData.totalAssets, comparisonData.totalAssets))}>
                            {calculateChange(balanceSheetData.totalAssets, comparisonData.totalAssets) > 0 ? '+' : ''}{calculateChange(balanceSheetData.totalAssets, comparisonData.totalAssets).toFixed(1)}%
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="border rounded-lg">
                    <Table>
                      ...
                      <TableBody>
                        {groupAccountsByCategory(balanceSheetData.liabilities, 'Current Liabilities').map((item, index) => {
                          const comparisonItem = comparisonData?.liabilities.find(c => c.account_name === item.account_name)
                          const change = comparisonItem ? calculateChange(item.amount, comparisonItem.amount) : 0
                ...
                <div>
                  ...
                  <div className="border rounded-lg">
                    <Table>
                      ...
                      <TableBody>
                        {groupAccountsByCategory(balanceSheetData.liabilities, 'Long-term Liabilities').map((item, index) => {
                          const comparisonItem = comparisonData?.liabilities.find(c => c.account_name === item.account_name)
                          const change = comparisonItem ? calculateChange(item.amount, comparisonItem.amount) : 0
                ...
                <div className="border-t-2 pt-4">
                  <div className="flex justify-between items-center font-bold text-lg">
                    ...
                    <div className="text-right">
                      <span>{formatCurrency(balanceSheetData.totalLiabilities, filters.currency)}</span>
                      {comparisonData && (
                        <div className="flex items-center justify-end space-x-1 mt-1 text-sm font-normal">
                          {getChangeIcon(calculateChange(balanceSheetData.totalLiabilities, comparisonData.totalLiabilities))}
                          <span className={getChangeColor(calculateChange(balanceSheetData.totalLiabilities, comparisonData.totalLiabilities))}>
                            {calculateChange(balanceSheetData.totalLiabilities, comparisonData.totalLiabilities) > 0 ? '+' : ''}{calculateChange(balanceSheetData.totalLiabilities, comparisonData.totalLiabilities).toFixed(1)}%
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {balanceSheetData.equity.map((item, index) => {
                      const comparisonItem = comparisonData?.equity.find(c => c.account_name === item.account_name)
                      const change = comparisonItem ? calculateChange(item.amount, comparisonItem.amount) : 0
                    ...
                    <TableRow className="border-t-2 font-bold">
                      ...
                      {comparisonData && (
                        <>
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end space-x-1">
                              {getChangeIcon(calculateChange(balanceSheetData.totalEquity, comparisonData.totalEquity))}
                              <span className={getChangeColor(calculateChange(balanceSheetData.totalEquity, comparisonData.totalEquity))}>
                                {calculateChange(balanceSheetData.totalEquity, comparisonData.totalEquity) > 0 ? '+' : ''}{calculateChange(balanceSheetData.totalEquity, comparisonData.totalEquity).toFixed(1)}%
                              </span>
                            </div>
                          </TableCell>
                        </>
              ...
              <div className="text-center">
                ...
                <div className="flex items-center justify-center space-x-8">
                  <div className="text-center">
                    ...
                    <p className="text-xl font-bold">{formatCurrency(balanceSheetData.totalAssets, filters.currency)}</p>
                  </div>
                  <div className="text-2xl font-bold text-muted-foreground">=</div>
                  <div className="text-center">
                    <p className="text-sm text-muted-foreground">Total Liabilities + Equity</p>
                    <p className="text-xl font-bold">{formatCurrency(balanceSheetData.totalLiabilities + balanceSheetData.totalEquity, filters.currency)}</p>
                  </div>
                </div>
                <div className="mt-4">
                  <Badge variant={Math.abs(balanceSheetData.totalAssets - (balanceSheetData.totalLiabilities + balanceSheetData.totalEquity)) < 0.01 ? "default" : "destructive"}>
                    {Math.abs(balanceSheetData.totalAssets - (balanceSheetData.totalLiabilities + balanceSheetData.totalEquity)) < 0.01 ? "Balanced" : "Not Balanced"}
                  </Badge>
                </div>
              </div>
            </CardContent>
