...
export interface PaymentMethod {
    ...
    name: string
    description?: string
    payment_type: 'Cash' | 'Check' | 'Credit Card' | 'Bank Transfer' | 'Wire Transfer' | 'ACH' | 'PayPal' | 'Stripe' | 'Other'
    company_id: string
    is_active: boolean
...
export interface PaymentWorkflowState {
    ...
    workflow_id: string
    state_name: string
    state_type: 'Draft' | 'Submitted' | 'Approved' | 'Rejected' | 'Processing' | 'Completed' | 'Failed' | 'Cancelled'
...
export class PaymentWorkflowsService {
     ...
     */
    static async createPaymentMethod(input: CreatePaymentMethodInput): Promise<ApiResponse<PaymentMethod>> {
        try {
     ...
     */
    static async createBankStatement(input: CreateBankStatementInput): Promise<ApiResponse<BankStatement>> {
        try {
    ...
    static async getBankStatements(
        ...
        try {
            ...
            const { data: bankStatements, error } = await query.order('statement_date', { ascending: false })
     ...
     */
    static async createBankReconciliation(input: CreateBankReconciliationInput): Promise<ApiResponse<BankReconciliation>> {
        try {
            ...
            return { success: true, data: reconciliation, message: 'Bank reconciliation created successfully' }
        } catch (error) {
    ...
    static async getBankReconciliations(
        ...
        try {
            ...
            const { data: reconciliations, error } = await query.order('reconciliation_date', { ascending: false })
     ...
     */
    static async getReconciliationStats(companyId: string): Promise<ApiResponse<{
        ...
        try {
            ...
            const stats = {
                ...
                pending_reconciliations: reconciliations.filter(r => r.status === 'Draft' || r.status === 'In Progress').length,
                total_difference: reconciliations.reduce((sum, r) => sum + Math.abs(r.difference), 0),
                average_difference: reconciliations.length > 0 ?
                    reconciliations.reduce((sum, r) => sum + Math.abs(r.difference), 0) / reconciliations.length : 0
            }
