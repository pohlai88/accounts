...
export class MasterDataService {
    ...
    static async searchCustomers(
        ...
        // Apply filters
        if (filters.search) {
            query = query.or(`customer_name.ilike.%${filters.search}%,customer_code.ilike.%${filters.search}%,email.ilike.%${filters.search}%`)
        }
    ...
    static async getCustomerOutstandingSummary(
        ...
        const { data, error } = await supabase
            .rpc('get_customer_outstanding_summary', {
                p_customer_id: customerId,
                p_company_id: companyId
            })
    ...
    static async validateCustomerForTransaction(
        ...
        try {
            ...
            if (creditCheck.utilization_percentage > 80 && creditCheck.utilization_percentage <= 100) {
                warnings.push(
                    `High credit utilization: ${creditCheck.utilization_percentage.toFixed(1)}%`
                )
            }
            ...
            errors.push('Error validating customer: ' + (error instanceof Error ? error.message : 'Unknown error'))
            return { valid: false, errors, warnings }
        }
    }
    ...
    static async getCustomerPaymentBehavior(
        ...
        const { data, error } = await supabase
            .rpc('get_customer_payment_behavior', {
                p_customer_id: customerId,
                p_company_id: companyId
            })
