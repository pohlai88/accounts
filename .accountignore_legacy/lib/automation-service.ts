...
export type AnomalyType =
  ...
  | "Invoice Anomaly"
  | "Expense Pattern Deviation"
  | "Cash Flow Anomaly"
  | "Account Balance Deviation"
  | "Timing Anomaly"
  | "Fraud Indicator"
  | "Process Deviation"
  | "Performance Anomaly";
...
export class AutomationService {
   ...
   */
  static async getAutomationRuleTemplates(
    category?: AutomationCategory,
  ): Promise<ApiResponse<AutomationRuleTemplate[]>> {
    try {
      let query = supabase.from("automation_rule_templates").select("*").eq("is_active", true);

      if (category) {
        query = query.eq("template_category", category);
      }

      const { data: templates, error } = await query.order("popularity_score", {
        ascending: false,
      });
   ...
   */
  static async executeAutomationRule(
    ruleId: string,
    triggerData?: any,
  ...
  static async getAIModels(
    ...
    try {
      let query = supabase.from("ai_models").select("*").eq("company_id", companyId);
   ...
   */
  static async createSmartRecommendation(
    input: CreateSmartRecommendationInput,
    ...
    try {
      ...
      return {
        success: true,
        data: recommendation,
        message: "Smart recommendation created successfully",
      };
    } catch (error) {
  ...
  static async getSmartRecommendations(
    ...
    try {
      let query = supabase.from("smart_recommendations").select("*").eq("company_id", companyId);
  ...
  static async getAnomalyDetections(
    ...
    try {
      let query = supabase.from("anomaly_detections").select("*").eq("company_id", companyId);
      ...
      const { data: anomalies, error } = await query.order("detected_at", { ascending: false });
   ...
   */
  static async getWorkflowAutomations(
    companyId: string,
    filters?: {
      workflow_type?: WorkflowType;
      entity_type?: string;
    ...
    try {
      let query = supabase.from("workflow_automations").select("*").eq("company_id", companyId);
   ...
   */
  static async getDataQualityInsights(
    companyId: string,
    filters?: {
      entity_type?: string;
      check_category?: QualityCategory;
    ...
    try {
      let query = supabase.from("data_quality_insights").select("*").eq("company_id", companyId);
      ...
      const { data: insights, error } = await query.order("checked_at", { ascending: false });
  ...
  ): Promise<ApiResponse<IntelligentCategorizationRule>> {
    try {
      const { data: rule, error } = await supabase
  ...
  static async getAutomationAnalytics(
    companyId: string,
  ): Promise<ApiResponse<AutomationAnalytics>> {
    try {
      ...
      const analytics: AutomationAnalytics = {
        ...
        average_execution_time:
          executions?.reduce((acc, e) => acc + (e.execution_time_ms || 0), 0) /
   ...
   */
  static async testAutomationRule(
    ruleId: string,
    testData: any,
  ): Promise<
    ApiResponse<{
   ...
   */
  static async suggestAutomationOpportunities(
    companyId: string,
  ): Promise<ApiResponse<SmartRecommendation[]>> {
    try {
      ...
      return {
        success: true,
        data: [],
        message: "Automation opportunities analyzed and created as recommendations",
      };
    } catch (error) {
