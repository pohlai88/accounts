...
export interface MonitoringAnalysis {
    ...
    risk_trends: {
        kri_performance: { kri_name: string; current_value: number; target_value: number; status_color: string }[]
...
export class ContinuousMonitoringService {
    ...
    static async getControlFrameworks(
        status?: string
    ): Promise<ServiceResponse<ControlFramework[]>> {
        try {
    ...
    static async getInternalControls(
        filters?: {
            framework_id?: string
            process_area?: string
            risk_rating?: string
    ...
    static async executeControlTest(
        execution: Omit<ControlTestExecution, 'id' | 'created_at' | 'updated_at' | 'effectiveness_rate' | 'control' | 'exceptions'>
    ...
    static async getControlTestExecutions(
        filters?: {
            control_id?: string
            testing_plan_id?: string
            test_status?: string
    ...
    static async createMonitoringRule(
        rule: Omit<MonitoringRule, 'id' | 'created_at' | 'updated_at' | 'false_positive_rate' | 'detection_rate' | 'total_triggers_count' | 'control'>
    ...
    static async executeMonitoringRules(): Promise<ServiceResponse<{ rules_executed: number; alerts_generated: number }>> {
        try {
    ...
    static async getMonitoringAlerts(
        filters?: {
            severity?: string
            status?: string
            date_from?: string
    ...
    static async calculateControlEffectiveness(
        ...
        `)
            ...
            const totalExceptions = executions?.reduce((sum, e) => sum + (e.exceptions?.length || 0), 0) || 0
            const materialWeaknesses = executions?.filter(e => e.test_conclusion === 'Material Weakness').length || 0
            const significantDeficiencies = executions?.filter(e => e.test_conclusion === 'Significant Deficiency').length || 0
            ...
            const metricData: Omit<ControlEffectivenessMetric, 'id' | 'created_at' | 'updated_at' | 'control_reliability_percentage' | 'control'> = {
                control_id,
    ...
    static async recordKRIMeasurement(
        ...
        try {
            ...
            } else if (kri.red_threshold_max !== null && measurement.measured_value <= kri.red_threshold_max) {
                statusColor = 'Red'
                riskLevel = 'High'
                thresholdBreached = true
            } else if (kri.yellow_threshold_min !== null && measurement.measured_value >= kri.yellow_threshold_min) {
                statusColor = 'Yellow'
                riskLevel = 'Medium'
                thresholdBreached = true
            } else if (kri.yellow_threshold_max !== null && measurement.measured_value <= kri.yellow_threshold_max) {
                statusColor = 'Yellow'
                riskLevel = 'Medium'
                thresholdBreached = true
            }
    ...
    static async getMonitoringDashboard(): Promise<ServiceResponse<MonitoringDashboard>> {
        try {
            ...
            const controlSummary = {
                ...
                high_risk_controls: controls?.filter(c => c.inherent_risk_rating === 'High' || c.inherent_risk_rating === 'Critical').length || 0,
                sox_controls: controls?.filter(c => c.sox_relevant).length || 0,
                automated_controls: controls?.filter(c => c.automated_testing).length || 0,
                controls_requiring_testing: controls?.filter(c => !c.last_update_date || new Date(c.last_update_date) < new Date(Date.now() - 90 * 24 * 60 * 60 * 1000)).length || 0
            }
            ...
            const { data: executions } = await this.supabase
                .from('control_test_executions')
                .select('*')
                .gte('test_date', new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0])
            ...
            const testingSummary = {
                ...
                overall_effectiveness_rate: executions?.length ?
                    (executions.filter(e => e.test_conclusion === 'Effective').length / executions.length) * 100 : 0,
                material_weaknesses: executions?.filter(e => e.test_conclusion === 'Material Weakness').length || 0,
                significant_deficiencies: executions?.filter(e => e.test_conclusion === 'Significant Deficiency').length || 0,
            ...
            const monitoringSummary = {
                ...
                false_positive_rate: alerts?.length ?
                    (alerts.filter(a => a.alert_status === 'false_positive').length / alerts.length) * 100 : 0,
        ...
        `)
            ...
            const kriSummary = {
                ...
                yellow_status_count: latestMeasurements?.filter(m => m.status_color === 'Yellow').length || 0,
                green_status_count: latestMeasurements?.filter(m => m.status_color === 'Green').length || 0,
                trending_negative: latestMeasurements?.filter(m => m.trend_vs_previous === 'Declining').length || 0,
    ...
    static async getMonitoringAnalysis(
        ...
        try {
            ...
            const analysis: MonitoringAnalysis = {
                ...
                risk_trends: {
                    kri_performance: [
                        { kri_name: 'Days Sales Outstanding', current_value: 32.5, target_value: 30.0, status_color: 'Yellow' },
                        { kri_name: 'Control Deficiency Rate', current_value: 4.2, target_value: 5.0, status_color: 'Green' },
                        { kri_name: 'Exception Rate', current_value: 8.7, target_value: 10.0, status_color: 'Green' },
                        { kri_name: 'Testing Coverage', current_value: 87.3, target_value: 90.0, status_color: 'Yellow' },
                        { kri_name: 'Management Response Time', current_value: 6.2, target_value: 5.0, status_color: 'Red' }
                    ],
                    risk_escalation: [
