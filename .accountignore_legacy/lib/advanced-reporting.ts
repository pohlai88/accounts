...
export class AdvancedReportingEngine {
  ...
  private static async calculateKPI(
    ...
    try {
      ...
      const previousValue = await this.evaluateFormula(
        ratio.formula, 
        companyId, 
        { period: 'previous' }
      )
  ...
  private static async evaluateFormula(
    ...
    try {
      ...
      const dateFilter = period === 'previous' 
  ...
  private static detectSeasonality(
    ...
    const seasonalVariance = pattern.reduce((sum, val) => sum + Math.pow(val - overallMean, 2), 0) / pattern.length
  ...
  private static getSeasonalPeriod(period: TrendPeriod): number {
    switch (period) {
      case 'daily': return 7 // Weekly pattern
      case 'weekly': return 4 // Monthly pattern
      case 'monthly': return 12 // Yearly pattern
      case 'quarterly': return 4 // Yearly pattern
