...
export type ExpenseClaimStatus = 'Draft' | 'Submitted' | 'Approved by Manager' | 'Approved by Finance' | 'Rejected' | 'Paid' | 'Cancelled'
export type ApprovalStatus = 'Pending' | 'Approved' | 'Rejected'
export type OCRStatus = 'Pending' | 'Processing' | 'Completed' | 'Failed' | 'Not Required'
export type ReimbursementMethod = 'Bank Transfer' | 'Check' | 'Cash' | 'Payroll' | 'Credit to Account'
export type ReimbursementStatus = 'Pending' | 'Processing' | 'Paid' | 'Failed' | 'Cancelled'
export type AdvanceStatus = 'Draft' | 'Submitted' | 'Approved' | 'Rejected' | 'Disbursed' | 'Settled' | 'Partially Settled'
...
export class ExpenseManagementService {
     ...
     */
    static async createExpenseCategory(input: CreateExpenseCategoryInput): Promise<ApiResponse<ExpenseCategory>> {
        try {
     ...
     */
    static async createExpensePolicy(input: CreateExpensePolicyInput): Promise<ApiResponse<ExpensePolicy>> {
        try {
     ...
     */
    static async createExpenseClaim(input: CreateExpenseClaimInput): Promise<ApiResponse<ExpenseClaim>> {
        try {
            ...
            return { success: true, data: claim, message: `Expense claim ${claimNo} created successfully` }
    ...
    static async getExpenseClaims(
        ...
        try {
            ...
            if (filters?.search) {
                query = query.or(`claim_no.ilike.%${filters.search}%,claim_title.ilike.%${filters.search}%,employee_name.ilike.%${filters.search}%`)
            }
     ...
     */
    static async submitExpenseClaim(claimId: string, managerId?: string): Promise<ApiResponse<boolean>> {
        try {
     ...
     */
    static async createExpenseAdvance(input: CreateExpenseAdvanceInput): Promise<ApiResponse<ExpenseAdvance>> {
        try {
            ...
            return { success: true, data: advance, message: `Expense advance ${advanceNo} created successfully` }
    ...
    static async checkPolicyLimits(
        ...
        try {
            ...
            if (!claim || !claim.expense_policy_id) {
                return { success: true, data: { is_within_limit: true, limit_type: 'No Policy', limit_amount: 0, current_usage: 0, remaining_limit: 0 } }
            }
     ...
     */
    static async getExpenseAnalytics(companyId: string, filters?: {
        from_date?: string
        to_date?: string
        employee_id?: string
        department?: string
        ...
        try {
            ...
            // Calculate analytics
            const totalExpenses = claims?.reduce((sum, claim) => sum + (claim.total_claimed_amount || 0), 0) || 0
            const totalReimbursed = claims?.reduce((sum, claim) => sum + (claim.total_reimbursement_amount || 0), 0) || 0
            const pendingApproval = claims?.filter(c => c.status === 'Submitted').reduce((sum, claim) => sum + (claim.total_claimed_amount || 0), 0) || 0
            const rejectedAmount = claims?.filter(c => c.status === 'Rejected').reduce((sum, claim) => sum + (claim.total_claimed_amount || 0), 0) || 0
