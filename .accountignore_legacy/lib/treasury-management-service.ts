...
export type ForecastRuleType = 'Fixed Amount' | 'Percentage of Revenue' | 'Historical Average' | 'Seasonal Pattern' | 'Linear Regression' | 'Custom Formula'
export type AlertType = 'Cash Balance' | 'Working Capital' | 'DSO Threshold' | 'Liquidity Risk' | 'Covenant Breach' | 'Forecast Variance' | 'Custom KPI'
...
export class TreasuryManagementService {
    ...
    static async getTreasuryDashboard(companyId: string): Promise<{
        ...
        try {
            ...
            const { data: forecasts, error: forecastError } = await supabase
                .rpc('generate_cash_flow_forecast', {
            ...
            const forecast30Days = forecasts?.reduce((sum: number, f: any) => {
            ...
            const forecast90Days = forecasts?.reduce((sum: number, f: any) =>
                sum + (f.forecasted_amount || 0), 0) || 0
            ...
            const { data: workingCapitalTrend, error: trendError } = await supabase
                ...
                .eq('company_id', companyId)
                .gte('kpi_date', new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0])
            ...
            const { data: cashFlows, error: flowError } = await supabase
                ...
                `)
                .eq('company_id', companyId)
                .gte('flow_date', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0])
            ...
            cashFlows?.forEach((flow: any) => {
                ...
                if (isInflow) {
                    ...
                    outflowsByCategory.set(categoryName, (outflowsByCategory.get(categoryName) || 0) + amount)
            ...
            const dashboard: TreasuryDashboard = {
                ...
                working_capital_trend: workingCapitalTrend?.map((item: any) => ({
                    date: item.kpi_date,
        ...
        try {
            ...
            for (const scenario of scenarios) {
                ...
                const forecast30Days = forecasts
                    .filter(f => new Date(f.forecast_period_start) <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000))
                    .reduce((sum, f) => sum + f.forecasted_amount, 0)

                const forecast90Days = forecasts
                    .filter(f => new Date(f.forecast_period_start) <= new Date(Date.now() + 90 * 24 * 60 * 60 * 1000))
                    .reduce((sum, f) => sum + f.forecasted_amount, 0)
    ...
    static async updateCashPositionSnapshot(
        ...
        try {
            const { data, error } = await supabase
                .rpc('calculate_cash_position_snapshot', {
                    p_company_id: companyId,
    ...
    static async getWorkingCapitalAnalysis(companyId: string): Promise<{
        ...
        try {
            ...
            const efficiencyScore = Math.max(0, Math.min(100,
                100 - (Math.abs(kpis.cash_conversion_cycle - idealCCC) / idealCCC) * 100
            ))
    ...
    private static calculateConcentrationRisk(cashPosition: CashPositionSnapshot | null): SeverityLevel {
        if (!cashPosition) return 'Medium'

        // Simplified concentration risk based on single bank balance percentage
        const concentrationPercentage = cashPosition.total_cash_and_equivalents > 0
    ...
    private static calculateForecastRisk(forecasts: any[]): SeverityLevel {
        ...
        const avgConfidence = forecasts.reduce((sum, f) => sum + (f.confidence_level || 0), 0) / forecasts.length
    ...
    private static calculateDSOSavings(currentDSO: number, targetDSO: number, totalCash: number): number {
        const improvement = currentDSO - targetDSO
        const dailySales = totalCash / 365 // Simplified calculation
        return improvement * dailySales
    }

    private static calculateDIOSavings(currentDIO: number, targetDIO: number, workingCapital: number): number {
    ...
    private static calculateDPOSavings(currentDPO: number, targetDPO: number, workingCapital: number): number {
