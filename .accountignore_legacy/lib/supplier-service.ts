...
export class SupplierService {
    ...
    static async getSuppliers(filters: SupplierFilters): Promise<ApiResponse<Supplier[]>> {
        try {
            ...
            // Apply filters
            if (filters.search) {
                query = query.or(`supplier_name.ilike.%${filters.search}%,supplier_code.ilike.%${filters.search}%,email.ilike.%${filters.search}%`)
            }
    ...
    static async getSupplierStats(companyId: string): Promise<ApiResponse<SupplierStats>> {
        try {
            ...
            const stats: SupplierStats = {
                ...
                average_credit_limit: suppliers.length > 0 ?
                    suppliers.reduce((sum, s) => sum + s.credit_limit, 0) / suppliers.length : 0,
                top_industries: [],
                top_supplier_groups: []
            }

            // Calculate top industries
            const industryCounts = suppliers.reduce((acc, s) => {
                if (s.industry) {
     ...
     * Search suppliers
     */
    static async searchSuppliers(companyId: string, searchTerm: string): Promise<ApiResponse<Supplier[]>> {
        try {
            const { data: suppliers, error } = await supabase
                .from('suppliers')
                .select(`
                ...
                `)
                .eq('company_id', companyId)
                .or(`supplier_name.ilike.%${searchTerm}%,supplier_code.ilike.%${searchTerm}%,email.ilike.%${searchTerm}%`)
                .limit(10)
