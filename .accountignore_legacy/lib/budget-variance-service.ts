...
export type BudgetStatus = 'Draft' | 'Pending Review' | 'Under Review' | 'Approved' | 'Active' | 'Locked' | 'Archived' | 'Rejected'
export type BudgetType = 'Operating' | 'Capital' | 'Cash Flow' | 'Project' | 'Department' | 'Master'
export type VarianceCategory = 'Minimal' | 'Acceptable' | 'Significant' | 'Critical'
export type AnalysisLevel = 'Company' | 'Department' | 'Cost Center' | 'Project' | 'Account' | 'Account Group'
export type AlertType = 'Variance Threshold' | 'Budget Exceeded' | 'Forecast Risk' | 'Missing Data' | 'Review Required'
...
export class BudgetVarianceService {
    ...
    static async getBudgetVarianceDashboard(
        ...
        try {
            ...
            const totalBudget = lineItems?.reduce((sum, item) => sum + item.budget_amount_company_currency, 0) || 0
            const totalActual = lineItems?.reduce((sum, item) => sum + item.actual_amount_company_currency, 0) || 0
            const totalVariance = totalActual - totalBudget
            const variancePercentage = totalBudget !== 0 ? (totalVariance / Math.abs(totalBudget)) * 100 : 0

            const favorableVariance = lineItems?.reduce((sum, item) =>
                item.is_favorable_variance ? sum + item.variance_amount : sum, 0) || 0
            const unfavorableVariance = lineItems?.reduce((sum, item) =>
                !item.is_favorable_variance ? sum + item.variance_amount : sum, 0) || 0
            ...
            const varianceByAccountType: Record<string, { budget: number; actual: number; variance: number }> = {}
            lineItems?.forEach(item => {
            ...
            const sortedByVariance = lineItems?.sort((a, b) => Math.abs(b.variance_amount) - Math.abs(a.variance_amount)) || []
            ...
            const recommendations = this.generateVarianceRecommendations(lineItems || [], variancePercentage)
            ...
            const dashboard: BudgetVarianceDashboard = {
                ...
                variance_breakdown: {
                    by_category: varianceByCategory as Record<VarianceCategory, { count: number; amount: number }>,
    ...
    static async calculateBudgetVarianceAnalysis(
        ...
        try {
            const { data, error } = await supabase
                .rpc('calculate_budget_variance_analysis', {
    ...
    static async getVarianceDrillDown(
        ...
        try {
            ...
            const totalBudget = lineItems.reduce((sum, item) => sum + item.budget_amount_company_currency, 0)
            const totalActual = lineItems.reduce((sum, item) => sum + item.actual_amount_company_currency, 0)
            const totalVariance = totalActual - totalBudget
            const variancePercentage = totalBudget !== 0 ? (totalVariance / Math.abs(totalBudget)) * 100 : 0
    ...
    private static generateVarianceRecommendations(
        ...
        const highVarianceItems = lineItems.filter(item =>
            Math.abs(item.variance_percentage) > 20
        )
        ...
        if (highVarianceItems.length > 0) {
            recommendations.push({
                ...
                potential_impact: highVarianceItems.reduce((sum, item) =>
                    sum + Math.abs(item.variance_amount), 0),
                actions: [
        ...
        // Revenue variances
        const revenueVariances = lineItems.filter(item =>
            item.account?.account_type === 'Income' && Math.abs(item.variance_percentage) > 10
        )

        if (revenueVariances.length > 0) {
            const totalRevenueVariance = revenueVariances.reduce((sum, item) => sum + item.variance_amount, 0)
            recommendations.push({
        ...
        // Expense control
        const expenseOverruns = lineItems.filter(item =>
    ...
    private static analyzeContributingFactors(lineItems: any[]): Array<{
        ...
        lineItems.forEach(item => {
            ...
            accountTypeVariances[accountType] = (accountTypeVariances[accountType] || 0) + item.variance_amount
        })
        ...
        Object.entries(accountTypeVariances)
            ...
            .forEach(([accountType, variance]) => {
                const totalBudget = lineItems.reduce((sum, item) => sum + Math.abs(item.budget_amount_company_currency), 0)
                factors.push({
