...
export class OutstandingManagementService {
    ...
    static async getCustomerOutstandingInvoices(
        ...
        const { data, error } = await supabase
            .rpc('get_customer_outstanding_invoices', {
    ...
    static calculateInvoiceStatus(
        ...
        if (outstanding <= 0.01) { // Allow for small rounding differences
            return 'Paid'
        }
     ...
     */
    static async getInvoicePaymentHistory(
        invoiceNo: string,
        companyId: string
    ): Promise<any[]> {
        const { data, error } = await supabase
    ...
    static async getCustomerCreditUtilization(
        ...
        const { data, error } = await supabase
            .rpc('get_customer_credit_utilization', {
                p_customer_id: customerId,
                p_company_id: companyId
            })
