...
export type QuotationStatus = 'Draft' | 'Open' | 'Replied' | 'Partially Ordered' | 'Ordered' | 'Lost' | 'Cancelled' | 'Expired'
export type SalesOrderStatus = 'Draft' | 'To Deliver and Bill' | 'To Bill' | 'To Deliver' | 'Completed' | 'Cancelled' | 'Closed'
...
export class SalesManagementService {
     ...
     */
    private static async generateSalesOrderNumber(companyId: string, series: string): Promise<string> {
     ...
     * Get sales analytics
     */
    static async getSalesAnalytics(companyId: string, period?: string): Promise<ApiResponse<{
        ...
        try {
            ...
            const totalSalesValue = ordersData?.reduce((sum, order) => sum + (order.grand_total || 0), 0) || 0
