...
export type BudgetDimension = 'Profit and Loss' | 'Balance Sheet' | 'Cash Flow' | 'Capital' | 'Project' | 'Department'
...
export type BudgetStatus = 'Draft' | 'Submitted' | 'Under Review' | 'Approved' | 'Active' | 'Closed' | 'Cancelled'
...
export type RevisionType = 'Budget Amendment' | 'Reallocation' | 'Supplementary' | 'Technical Adjustment'
export type ScenarioType = 'Optimistic' | 'Pessimistic' | 'Most Likely' | 'Stress Test' | 'Custom'
export type AlertType = 'Variance Threshold' | 'Budget Exceeded' | 'Under Spending' | 'Forecast Change' | 'Approval Required'
...
export class BudgetingService {
     ...
     */
    static async createBudgetTemplate(input: CreateBudgetTemplateInput): Promise<ApiResponse<BudgetTemplate>> {
        try {
     ...
     * Approve budget
     */
    static async approveBudget(budgetId: string, approvedBy: string, comments?: string): Promise<ApiResponse<boolean>> {
        try {
            const { error } = await supabase
                .from('budgets')
                .update({
    ...
    static async updateBudgetActuals(
        ...
        try {
            ...
            return { success: true, data: updatedCount, message: `${updatedCount} budget items updated with actuals` }
    ...
    static async getBudgetVarianceReport(
        ...
        try {
            ...
            if (filters?.account_type) {
                filteredItems = filteredItems.filter((item: any) => item.account_type === filters.account_type)
            }

            if (filters?.cost_center_id) {
                filteredItems = filteredItems.filter((item: any) => item.cost_center_id === filters.cost_center_id)
            }
            ...
            const budgetSummary = {
                total_budgeted: filteredItems.reduce((sum: number, item: any) => sum + (item.budgeted_amount || 0), 0),
                total_actual: filteredItems.reduce((sum: number, item: any) => sum + (item.actual_amount || 0), 0),
                total_variance: filteredItems.reduce((sum: number, item: any) => sum + (item.variance_amount || 0), 0),
            ...
            budgetSummary.variance_percentage = budgetSummary.total_budgeted !== 0
    ...
    static async generateForecast(
        ...
        try {
            ...
            return { success: true, data: updatedCount, message: `Forecast generated for ${updatedCount} budget items` }
     ...
     */
    static async createBudgetScenario(input: CreateBudgetScenarioInput): Promise<ApiResponse<BudgetScenario>> {
        try {
    ...
    static async getBudgetAnalytics(
        ...
        try {
            ...
            // Calculate analytics
            const totalBudgetAmount = budgets?.reduce((sum, b) => sum + (b.total_budgeted_amount || 0), 0) || 0
            const totalActualAmount = budgets?.reduce((sum, b) => sum + (b.total_actual_amount || 0), 0) || 0
            const totalVarianceAmount = budgets?.reduce((sum, b) => sum + (b.total_variance_amount || 0), 0) || 0
            ...
            const analytics: BudgetAnalytics = {
                ...
                overall_variance_percentage: totalBudgetAmount !== 0 ? (totalVarianceAmount / totalBudgetAmount) * 100 : 0,
                budget_utilization_rate: totalBudgetAmount !== 0 ? (totalActualAmount / totalBudgetAmount) * 100 : 0,
