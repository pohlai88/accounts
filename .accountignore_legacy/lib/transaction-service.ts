...
export class TransactionService {
    ...
    static async createPayment(input: CreatePaymentInput): Promise<{
        ...
        try {
            ...
            const totalAllocatedAmount = input.allocations.reduce((sum, alloc) => sum + alloc.allocated_amount, 0)
    ...
    static async getJournalEntries(filters: {
        ...
        `, { count: 'exact' })
            ...
            return { success: true, journal_entries: journalEntries as JournalEntry[], total: count || 0 }
        } catch (error) {
    ...
    static async submitTransaction(
        ...
        try {
            const tableName = type === 'invoice' ? 'invoices' :
                type === 'payment' ? 'payments' : 'journal_entries'
