...
export class InventoryService {
   ...
   */
  static async createUnitOfMeasure(
  ...
  static async getStockEntries(
    ...
    try {
      let query = supabase.from("stock_entries").select("*").eq("company_id", companyId);
  ...
  static async getBins(
    ...
    try {
      let query = supabase.from("bins").select("*").eq("company_id", companyId);
   ...
   */
  static async getInventoryStats(companyId: string): Promise<
    ApiResponse<{
      total_items: number;
