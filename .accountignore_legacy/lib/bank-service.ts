...
export type BankProviderType = 'Open Banking' | 'Screen Scraping' | 'Direct API' | 'File Import' | 'Manual'
export type AuthMethod = 'OAuth2' | 'API Key' | 'Certificate' | 'Username/Password' | 'None'
export type ConnectionStatus = 'Active' | 'Inactive' | 'Error' | 'Expired' | 'Reauth Required' | 'Suspended'
export type AccountType = 'Checking' | 'Savings' | 'Credit Card' | 'Line of Credit' | 'Certificate of Deposit' | 'Money Market' | 'Investment' | 'Loan' | 'Mortgage'
export type TransactionType = 'Debit' | 'Credit' | 'Transfer' | 'Fee' | 'Interest' | 'Dividend' | 'ATM' | 'Check' | 'Deposit' | 'Withdrawal' | 'Payment' | 'Refund'
export type ReconciliationStatus = 'Unmatched' | 'Matched' | 'Partially Matched' | 'Excluded' | 'Manual Review' | 'Disputed'
...
export type SyncStatus = 'Started' | 'In Progress' | 'Completed' | 'Failed' | 'Cancelled' | 'Timeout'
...
export type DescriptionMatching = 'Exact' | 'Contains' | 'Starts With' | 'Ends With' | 'Regex' | 'Ignore'
...
export class BankService {
     ...
     */
    static async createBankConnection(input: CreateBankConnectionInput): Promise<ApiResponse<BankConnection>> {
        try {
            const { data: connection, error } = await supabase
                ...
                .insert({
                    ...
                    next_sync_at: new Date(Date.now() + (input.sync_frequency_minutes || 60) * 60 * 1000).toISOString(),
     ...
     */
    static async getBankConnections(companyId: string, filters?: {
        status?: ConnectionStatus
        provider_id?: string
    ...
    static async createBankAccount(input: CreateBankAccountInput): Promise<ApiResponse<BankAccount>> {
        try {
            ...
            const accountNumberMasked = input.account_number.length > 4
     ...
     * Get bank accounts
     */
    static async getBankAccounts(companyId: string, filters?: {
     ...
     */
    static async importBankTransaction(input: ImportBankTransactionInput): Promise<ApiResponse<BankTransaction>> {
        try {
            ...
            return { success: true, data: transaction, message: 'Bank transaction imported successfully' }
    ...
    ): Promise<ApiResponse<{ transactions: BankTransaction[]; total: number; page: number; limit: number }>> {
        try {
            let query = supabase
                .from('bank_transactions')
                .select(`
     ...
     */
    static async createReconciliationRule(input: CreateReconciliationRuleInput): Promise<ApiResponse<ReconciliationRule>> {
        try {
     ...
     */
    static async getReconciliationRules(companyId: string, filters?: {
        is_active?: boolean
     ...
     */
    static async createReconciliationSession(input: CreateReconciliationSessionInput): Promise<ApiResponse<ReconciliationSession>> {
        try {
            ...
            return { success: true, data: session, message: `Reconciliation session "${input.session_name}" created successfully` }
     ...
     */
    static async getReconciliationSessions(companyId: string, filters?: {
        status?: SessionStatus
        date_from?: string
        date_to?: string
    ...
    static async completeReconciliationSession(
        ...
        try {
            ...
            return { success: true, data: true, message: 'Reconciliation session completed successfully' }
    ...
    static async generateReconciliationReport(
        ...
        try {
            ...
            return { success: true, data: report, message: 'Reconciliation report generated successfully' }
    ...
    static async getReconciliationSummary(
        ...
        try {
            ...
            const matchedCount = transactions?.filter(t => t.reconciliation_status === 'Matched').length || 0
            ...
            const summary: ReconciliationSummary = {
                ...
                manual_review_count: transactions?.filter(t => t.reconciliation_status === 'Manual Review').length || 0,
    ...
    static async getBankAnalytics(companyId: string): Promise<ApiResponse<BankAnalytics>> {
        try {
            const [
                ...
                supabase.from('bank_connections').select('status', { count: 'exact' }).eq('company_id', companyId),
                supabase.from('bank_accounts').select('id', { count: 'exact' }).eq('company_id', companyId),
                supabase.from('bank_transactions').select('reconciliation_status', { count: 'exact' }).eq('company_id', companyId),
                supabase.from('bank_sync_logs').select('*').eq('company_id', companyId).order('started_at', { ascending: false }).limit(10)
            ])

            const totalConnections = connectionsResult.count || 0
            const activeConnections = connectionsResult.data?.filter(c => c.status === 'Active').length || 0
            const totalAccounts = accountsResult.count || 0
            const totalTransactions = transactionsResult.count || 0
            const matchedTransactions = transactionsResult.data?.filter(t => t.reconciliation_status === 'Matched').length || 0
            ...
            const analytics: BankAnalytics = {
                ...
                reconciliation_rate: totalTransactions ? (matchedTransactions / totalTransactions) * 100 : 0,
