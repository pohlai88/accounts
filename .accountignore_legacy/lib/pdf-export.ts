...
export class PDFExportService {
     ...
     */
    static generateFilename(reportType: string, companyName: string, period?: { from: string; to: string }, asOfDate?: string): string {
        const sanitizedCompanyName = companyName.replace(/[^a-zA-Z0-9]/g, '_')
        const dateStr = period ? `${period.from}_to_${period.to}` : asOfDate || new Date().toISOString().split('T')[0]
