...
export class BudgetManagementService {
     ...
     */
    static async createBudgetPeriod(input: CreateBudgetPeriodInput): Promise<{ success: boolean; period?: BudgetPeriod; error?: string }> {
        try {
     ...
     * Get budget periods
     */
    static async getBudgetPeriods(companyId: string): Promise<{ success: boolean; periods?: BudgetPeriod[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('budget_periods')
                .select('*')
     ...
     * Create a budget
     */
    static async createBudget(input: CreateBudgetInput): Promise<{ success: boolean; budget?: Budget; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('budgets')
                .insert({
     ...
     */
    static async getBudgets(companyId: string): Promise<{ success: boolean; budgets?: Budget[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('budgets')
                .select('*')
     ...
     * Add budget item
     */
    static async addBudgetItem(input: CreateBudgetItemInput): Promise<{ success: boolean; budgetItem?: BudgetItem; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('budget_items')
                .insert({
     ...
     * Get budget items
     */
    static async getBudgetItems(budgetId: string): Promise<{ success: boolean; budgetItems?: BudgetItem[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('budget_items')
                .select('*')
    ...
    static async getBudgetVsActualReport(
        ...
        try {
            const { data, error } = await supabase
                .rpc('get_budget_vs_actual_report', {
     ...
     * Get budget variance
     */
    static async getBudgetVariance(budgetId: string): Promise<{ success: boolean; variance?: BudgetVariance; error?: string }> {
        try {
            const { data, error } = await supabase
    ...
    static async createBudgetFromPrevious(
        ...
        try {
            const { data, error } = await supabase
                .rpc('create_budget_from_previous', {
     ...
     */
    static async getBudgetCategories(companyId: string): Promise<{ success: boolean; categories?: BudgetCategory[]; error?: string }> {
        try {
            const { data, error } = await supabase
