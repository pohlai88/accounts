...
export class BackupService {
    ...
    private async processBackup(backupId: string, options: BackupOptions): Promise<BackupResult> {
        try {
            ...
            const compressionRatio = options.compression ?
                (backupContent.length / JSON.stringify(backupPackage).length) : 1;
        ...
        for (const table of tablesToRestore) {
            try {
                ...
                const { error } = await supabase
                    .from(table)
                    .insert(tableData.map((record: any) => ({
                        ...record,
                        company_id: this.companyId
                    })));
                ...
                errors.push(`Error restoring table ${table}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
    ...
    async scheduleAutomaticBackup(frequency: 'daily' | 'weekly' | 'monthly', tables: string[]): Promise<void> {
...
Records: ${Object.entries(backupJob.recordsCount).map(([table, count]) => `${table}: ${count}`).join(', ')}
