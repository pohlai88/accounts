...
export type FilingFrequency = 'Weekly' | 'Monthly' | 'Quarterly' | 'Semi-Annual' | 'Annual' | 'Custom'
export type TaxType = 'Sales Tax' | 'VAT' | 'GST' | 'Excise' | 'Customs' | 'Service Tax' | 'Withholding Tax' | 'Other'
...
export type ReferenceType = 'Sales Invoice' | 'Purchase Invoice' | 'Sales Order' | 'Purchase Order' | 'Quotation'
...
export class TaxManagementService {
     ...
     */
    static async createTaxAuthority(input: CreateTaxAuthorityInput): Promise<ApiResponse<TaxAuthority>> {
        try {
     ...
     * Get tax rates
     */
    static async getTaxRates(companyId: string, filters?: {
        authority_id?: string
        category_id?: string
    ...
    static async createTaxTemplate(input: CreateTaxTemplateInput): Promise<ApiResponse<TaxTemplate>> {
        try {
            ...
            return { success: true, data: completeTemplate, message: 'Tax template created successfully' }
     ...
     * Get tax templates
     */
    static async getTaxTemplates(companyId: string, templateType?: TemplateType): Promise<ApiResponse<TaxTemplate[]>> {
        try {
            let query = supabase
                .from('tax_templates')
                .select(`
     ...
     */
    static async calculateTax(request: TaxCalculationRequest): Promise<ApiResponse<TaxCalculationResult[]>> {
        try {
     ...
     */
    static async createTaxExemption(input: CreateTaxExemptionInput): Promise<ApiResponse<TaxExemption>> {
        try {
     ...
     * Get tax exemptions
     */
    static async getTaxExemptions(companyId: string, exemptionType?: ExemptionType): Promise<ApiResponse<TaxExemption[]>> {
        try {
            let query = supabase
                .from('tax_exemptions')
                .select('*')
    ...
    static async generateFilingPeriods(
        ...
        try {
            ...
            return { success: true, data: periodsGenerated, message: `${periodsGenerated} filing periods generated` }
     ...
     */
    static async getFilingPeriods(companyId: string, filters?: {
        authority_id?: string
        status?: FilingStatus
        year?: number
     ...
     */
    static async calculateFilingSummary(filingPeriodId: string): Promise<ApiResponse<{
        total_sales_amount: number
        total_purchase_amount: number
        total_tax_collected: number
        total_tax_paid: number
     ...
     * Submit tax filing
     */
    static async submitFilingPeriod(filingPeriodId: string, filedBy: string): Promise<ApiResponse<boolean>> {
        try {
     ...
     * Get tax analytics
     */
    static async getTaxAnalytics(companyId: string, year?: number): Promise<ApiResponse<{
        total_tax_collected: number
        total_tax_paid: number
        net_tax_liability: number
        ...
        try {
            ...
            // Calculate analytics
            const totalTaxCollected = taxDetails?.filter(t => t.reference_type.includes('Sales'))
                .reduce((sum, t) => sum + (t.tax_amount || 0), 0) || 0
            
            const totalTaxPaid = taxDetails?.filter(t => t.reference_type.includes('Purchase'))
                .reduce((sum, t) => sum + (t.tax_amount || 0), 0) || 0
    ...
    private static groupTaxDetails(taxDetails: any[], groupBy: 'authority' | 'category') {
        ...
        const grouped = taxDetails.reduce((acc, detail) => {
            const key = groupBy === 'authority' 
                ? detail.authority?.authority_name 
