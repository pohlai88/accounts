...
export class TaxCalculationService {
     ...
     */
    static async createTaxCategory(input: CreateTaxCategoryInput): Promise<{ success: boolean; category?: TaxCategory; error?: string }> {
        try {
     ...
     * Get tax categories
     */
    static async getTaxCategories(companyId: string): Promise<{ success: boolean; categories?: TaxCategory[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('tax_categories')
                .select('*')
     ...
     * Create a tax rate
     */
    static async createTaxRate(input: CreateTaxRateInput): Promise<{ success: boolean; rate?: TaxRate; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('tax_rates')
                .insert({
     ...
     * Get tax rates
     */
    static async getTaxRates(companyId: string): Promise<{ success: boolean; rates?: TaxRate[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('tax_rates')
                .select('*')
     ...
     * Create a tax group
     */
    static async createTaxGroup(input: CreateTaxGroupInput): Promise<{ success: boolean; group?: TaxGroup; error?: string }> {
        try {
            // Create tax group
     ...
     * Get tax groups
     */
    static async getTaxGroups(companyId: string): Promise<{ success: boolean; groups?: TaxGroup[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('tax_groups')
                .select('*')
     ...
     */
    static async calculateTax(input: CalculateTaxInput): Promise<{ success: boolean; calculation?: TaxCalculation; error?: string }> {
        try {
            const { data, error } = await supabase
                .rpc('calculate_tax', {
     ...
     * Get tax returns
     */
    static async getTaxReturns(companyId: string): Promise<{ success: boolean; returns?: TaxReturn[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('tax_returns')
                .select('*')
