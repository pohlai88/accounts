...
export interface AnalysisFilter {
    ...
    operator: 'equals' | 'not_equals' | 'contains' | 'not_contains' | 'in' | 'not_in' | 'between' | 'greater_than' | 'less_than'
...
export class FlexibleAnalysisEngine {
    ...
    static async executePivotTable(
        ...
        try {
            ...
            // Calculate totals
            const { rowTotals, columnTotals, grandTotal } = this.calculatePivotTotals(processedData, config)
     ...
     */
    private static buildAnalysisQuery(config: AnalysisConfig, companyId: string): {
        sql: string
        params: any[]
    } {
        ...
        const orderByClause = sortBy.map(sort =>
            `${this.getDimensionField(sort.dimension)} ${sort.direction}`
        ).join(', ')
     ...
     * Build pivot table query
     */
    private static buildPivotTableQuery(config: PivotTableConfig, companyId: string): {
        sql: string
        params: any[]
    } {
    ...
    private static buildFilterCondition(filter: AnalysisFilter, paramIndex: number): {
        condition: string
        params: any[]
    } {
    ...
    private static extractNodeMetadata(row: any, dimensions: AnalysisDimension[]): Record<string, any> {
