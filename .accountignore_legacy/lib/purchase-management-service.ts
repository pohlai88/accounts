...
export type MaterialRequestType = 'Purchase' | 'Material Transfer' | 'Material Issue' | 'Manufacture' | 'Customer Provided'
export type PurchaseRequestStatus = 'Draft' | 'Submitted' | 'Stopped' | 'Cancelled' | 'Pending' | 'Partially Ordered' | 'Ordered' | 'Issued' | 'Transferred' | 'Received'
export type PurchaseOrderStatus = 'Draft' | 'To Receive and Bill' | 'To Bill' | 'To Receive' | 'Completed' | 'Cancelled' | 'Closed' | 'Delivered' | 'Partly Received'
export type SupplierQuotationStatus = 'Draft' | 'Submitted' | 'Ordered' | 'Expired' | 'Lost' | 'Cancelled'
...
export class PurchaseManagementService {
     ...
     */
    static async createPurchaseRequest(input: CreatePurchaseRequestInput): Promise<ApiResponse<PurchaseRequest>> {
        try {
     ...
     */
    static async createPurchaseOrder(input: CreatePurchaseOrderInput): Promise<ApiResponse<PurchaseOrder>> {
        try {
     ...
     */
    static async createSupplierQuotation(input: CreateSupplierQuotationInput): Promise<ApiResponse<SupplierQuotation>> {
        try {
     ...
     */
    private static async generatePurchaseRequestNumber(companyId: string, series: string): Promise<string> {
     ...
     */
    private static async generatePurchaseOrderNumber(companyId: string, series: string): Promise<string> {
     ...
     */
    private static async generateSupplierQuotationNumber(companyId: string, series: string): Promise<string> {
     ...
     */
    static async getPurchaseAnalytics(companyId: string, period?: string): Promise<ApiResponse<{
        ...
        try {
            ...
            const totalPurchaseValue = ordersData?.reduce((sum, order) => sum + (order.grand_total || 0), 0) || 0
