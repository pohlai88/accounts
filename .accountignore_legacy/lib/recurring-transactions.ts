...
export type TransactionType = 'Sales Invoice' | 'Purchase Invoice' | 'Payment Entry' | 'Journal Entry'
...
export class RecurringTransactionsService {
     ...
     */
    static async createTemplate(input: CreateRecurringTemplateInput): Promise<{ success: boolean; template?: RecurringTemplate; error?: string }> {
        try {
     ...
     */
    static async getTemplates(companyId: string): Promise<{ success: boolean; templates?: RecurringTemplate[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('recurring_templates')
                .select('*')
     ...
     */
    static async updateTemplate(templateId: string, input: UpdateRecurringTemplateInput): Promise<{ success: boolean; template?: RecurringTemplate; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('recurring_templates')
                .update({
     ...
     */
    static async getTemplatesReadyForExecution(executionDate?: string): Promise<{ success: boolean; templates?: RecurringTemplate[]; error?: string }> {
        try {
            const date = executionDate || new Date().toISOString().split('T')[0]

            const { data, error } = await supabase
                .rpc('get_templates_ready_for_execution', { execution_date: date })
     ...
     */
    static async executeTemplate(templateId: string, executionDate?: string): Promise<{ success: boolean; executionId?: string; transactionId?: string; error?: string }> {
        try {
            const date = executionDate || new Date().toISOString().split('T')[0]
            ...
            const { error: nextDateError } = await supabase
                .rpc('update_next_execution_date', {
                    template_id: templateId,
                    execution_date: date
                })
     ...
     */
    static async getExecutionHistory(templateId: string): Promise<{ success: boolean; executions?: RecurringExecution[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('recurring_executions')
                .select('*')
     ...
     */
    static async getUpcomingExecutions(companyId: string, days: number = 30): Promise<{ success: boolean; executions?: any[]; error?: string }> {
        try {
