...
export class AccountingService {
  ...
  static async updateAccount(id: string, data: Partial<CreateAccountInput>): Promise<ApiResponse<Account>> {
    try {
      const { data: account, error } = await supabase
        .from('accounts')
        .update({
  ...
  private static validateAccountData(data: CreateAccountInput): ValidationResult {
    ...
    if (data.account_code && data.account_code.length > 50) {
      errors.push({ field: 'account_code', message: 'Account code must be less than 50 characters' })
    }
  ...
  private static validateGLEntries(entries: CreateGLEntryInput[]): ValidationResult {
    ...
    entries.forEach((entry, index) => {
      ...
      if (!entry.voucher_no || entry.voucher_no.trim().length === 0) {
        errors.push({ field: `entries[${index}].voucher_no`, message: 'Voucher number is required' })
      }
      if (!entry.posting_date) {
        errors.push({ field: `entries[${index}].posting_date`, message: 'Posting date is required' })
      }
      if (entry.debit < 0 || entry.credit < 0) {
        errors.push({ field: `entries[${index}]`, message: 'Amounts cannot be negative' })
      }
      if (entry.debit === 0 && entry.credit === 0) {
        errors.push({ field: `entries[${index}]`, message: 'Either debit or credit must be greater than zero' })
      }
      if (entry.debit > 0 && entry.credit > 0) {
        errors.push({ field: `entries[${index}]`, message: 'Entry cannot have both debit and credit amounts' })
      }
    })
  ...
  private static async addBalancesToHierarchy(hierarchy: AccountHierarchy[]): Promise<AccountHierarchy[]> {
