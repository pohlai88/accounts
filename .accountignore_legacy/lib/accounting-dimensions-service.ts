...
export class AccountingDimensionsService {
    ...
    static async getDimensionDetails(dimensionId: string): Promise<{
        ...
        try {
            ...
            const details: AccountingDimensionDetail[] = data.map(item => ({
                ...
                automaticallyPostBalancingAccountingEntry: item.automatically_post_balancing_accounting_entry
            }))
    ...
    static async validateDimensions(
        ...
        try {
            ...
            if (!dimensions) {
                return { isValid: true, errors: [], warnings: [], mandatoryDimensions: [], restrictedDimensions: [] }
            }
        ...
        try {
            ...
            for (const dimension of dimensions) {
                const dimensionValues = new Set(
                    glEntries
                        .map(entry => entry[dimension.fieldname])
                        .filter(value => value)
                )
     ...
     */
    static async deleteAccountingDimension(dimensionId: string): Promise<{ success: boolean; error?: string }> {
        try {
            ...
            if (usage && usage.length > 0) {
                return { success: false, error: 'Cannot delete dimension that is being used in GL entries' }
            }
     ...
     */
    static async setupDefaultDimensions(companyId: string): Promise<{ success: boolean; error?: string }> {
        try {
