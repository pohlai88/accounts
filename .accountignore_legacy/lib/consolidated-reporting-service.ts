...
export class ConsolidatedReportingService {
  ...
  static async getConsolidationDashboard(
    ...
    try {
      ...
      const consolidationSummary = await this.calculateConsolidationSummary(
        groupId,
        consolidationPeriod,
      );
  ...
  static async createConsolidationRun(
    ...
    try {
      const { data, error } = await supabase.rpc("create_consolidation_run", {
        p_group_id: groupId,
        p_period: period,
        p_run_name: runName,
        p_run_type: runType,
  ...
  static async processConsolidation(runId: string): Promise<{
    ...
    try {
      const { data: success, error } = await supabase.rpc("process_consolidation", {
        p_run_id: runId,
      });
  ...
  private static async calculateConsolidationSummary(
    ...
    if (trialBalance && trialBalance.length > 0) {
      ...
      avgCompleteness =
        trialBalance.reduce((sum, item) => sum + item.data_completeness, 0) / trialBalance.length;
    }
    ...
    const eliminationsTotal =
      eliminations?.reduce((sum, elim) => sum + Math.abs(elim.elimination_amount), 0) || 0;
  ...
  private static async calculateKeyMetrics(
    ...
    const totalEliminations =
      eliminations?.reduce((sum, elim) => Math.abs(elim.elimination_amount), 0) || 0;
    ...
    const processingEfficiency = latestRun
      ? Math.min(100, Math.max(0, 100 - (latestRun.processing_duration_seconds || 0) / 60))
      : 0;
  ...
  private static generateBalanceSheetData(
    ...
    return {
      ...
      liabilities: {
        ...
        non_current_liabilities: liabilities.filter(
          item => !item.account_category?.includes("Current"),
        ),
        total_liabilities: liabilities.reduce(
          (sum, item) => sum + Math.abs(item.consolidated_balance),
          0,
        ),
      },
      equity: {
        share_capital: equity.filter(item => item.account_name.toLowerCase().includes("capital")),
        retained_earnings: equity.filter(item =>
  ...
  private static generateIncomeStatementData(
    trialBalance: ConsolidatedTrialBalance[],
  ): Record<string, any> {
    ...
    return {
      ...
      expenses: {
        cost_of_goods_sold: expense.filter(item =>
          item.account_name.toLowerCase().includes("cost"),
        ),
        operating_expenses: expense.filter(
          item => !item.account_name.toLowerCase().includes("cost"),
        ),
  ...
  private static generateCashFlowStatementData(
    trialBalance: ConsolidatedTrialBalance[],
  ): Record<string, any> {
  ...
  private static async calculateEliminationImpact(groupId: string, period: string): Promise<{
    ...
    eliminations?.forEach(elim => {
      byType[elim.elimination_type] =
        (byType[elim.elimination_type] || 0) + Math.abs(elim.elimination_amount);
    });

    return {
      total_eliminations: totalEliminations,
      by_type: byType,
      material_eliminations:
        eliminations?.filter(elim => Math.abs(elim.elimination_amount) > 10000) || [],
    };
  }
