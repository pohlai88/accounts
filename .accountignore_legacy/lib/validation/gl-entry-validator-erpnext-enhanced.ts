...
import { GLEntryValidator, ValidationResult, ValidationError, ValidationWarning, VoucherValidationContext, GLEntryInput } from './gl-entry-validator'
...
export class ERPNextGLEntryValidator extends GLEntryValidator {
    ...
    private async validateCostCenterRules(
        ...
        for (const entry of context.entries) {
            ...
            if (this.config.costCenterRules.validateAllocation &&
                ...
                const allocationSum = costCenterValidation.allocation.allocations
    ...
    private async validatePeriodAndAuthorization(
        ...
        if (this.config.periodRules.validateFiscalYear) {
            const fiscalYearValidation = await this.validateFiscalYear(context.postingDate, context.companyId)
        ...
        if (this.config.authorizationRules.enforceAuthorizationLimits) {
            const totalAmount = context.entries.reduce((sum, entry) =>
                sum + Math.max(entry.debit, entry.credit), 0)
    ...
    private async validateCostCenter(entry: GLEntryInput): Promise<CostCenterValidation> {
        ...
        if (this.config.costCenterRules.validateAllocation) {
            const { data: allocations } = await supabase
                .rpc('get_cost_center_allocation_data', {
     ...
     */
    private async validateDimensions(entry: GLEntryInput, voucherType: string): Promise<DimensionValidation> {
     ...
     */
    private async validateBudget(entry: GLEntryInput, postingDate: string): Promise<BudgetValidation> {
        ...
        const requiresApproval = exceedsLimit && this.config.authorizationRules.requireApprovalAboveLimit
        ...
        return {
            ...
            exceedsLimit,
            requiresApproval,
            errors: exceedsLimit && !this.config.budgetRules.allowBudgetExceedance
    ...
    private async validateVoucherLinking(entry: GLEntryInput) {
        ...
        if (error || !data || data.length === 0) {
            return { isValid: false, message: `Referenced voucher ${entry.againstVoucherType} ${entry.againstVoucher} not found` }
        }

        if (data[0].docstatus !== 1) {
            return { isValid: false, message: `Referenced voucher ${entry.againstVoucherType} ${entry.againstVoucher} is not submitted` }
        }
    ...
    private async validateInterCompanyTransactions(context: VoucherValidationContext, errors: ValidationError[], warnings: ValidationWarning[]) {
        // Implementation for inter-company transaction validation
    }

    private async validateTaxImplications(context: VoucherValidationContext, errors: ValidationError[], warnings: ValidationWarning[]) {
        // Implementation for tax validation
    }

    private async validateWorkflowState(context: VoucherValidationContext, errors: ValidationError[], warnings: ValidationWarning[]) {
        // Implementation for workflow state validation
    }

    private async validateDocumentSeries(context: VoucherValidationContext, errors: ValidationError[], warnings: ValidationWarning[]) {
