...
import { GLEntryValidationSchema, ValidationResult, ValidationError, ValidationWarning, VoucherValidationContext, GLEntryInput } from './gl-entry-validator'
...
export class EnhancedGLEntryValidator {
            ...
            async () => {
                ...
                return config?.validation_config || {
                    rules: {
                        ...
                        maxVoucherLines: 20,
                        allowedVoucherTypes: ['Sales Invoice', 'Purchase Invoice', 'Payment Entry', 'Journal Entry']
                    },
                    thresholds: {
    ...
    async getAccountBalanceFast(accountId: string): Promise<{
        ...
        if (error || !data) {
            return { debit: 0, credit: 0, balance: 0, transactionCount: 0, lastActivity: new Date().toISOString() }
        }
     ...
     */
    private async validateCurrencyRates(entry: GLEntryInput, account: any): Promise<ValidationError[]> {
        ...
        const hasTransactionCurrency = entry.transactionCurrency && entry.transactionCurrency !== account.account_currency
     ...
     */
    private async validateExchangeRateReasonableness(rate: number, fromCurrency: string, toCurrency: string): Promise<ExchangeRateValidation> {
        const config = await this.getValidationConfig()
        ...
        return this.getCachedData(
            ...
            async () => {
                const { data: rates } = await supabase
                    ...
                    .eq('is_active', true)
                    .gte('rate_date', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]) // Last 7 days
     ...
     */
    private async validateTaxEntries(entry: GLEntryInput, context: VoucherValidationContext): Promise<ValidationError[]> {
        const errors: ValidationError[] = []
        const config = await this.getValidationConfig()
        ...
        if (account?.tax_rate && account.tax_rate > 0) {
            ...
            if (taxEntries.length === 0) {
                ...
                const actualTax = taxEntries.reduce((sum, taxEntry) => sum + taxEntry.debit + taxEntry.credit, 0)
     ...
     */
    async validateForUI(entry: Partial<GLEntryInput>, context: Partial<VoucherValidationContext>): Promise<ValidationResult> {
    ...
    private async generateEnhancedSuggestions(
        ...
        if (context.entries.length > config.rules.maxVoucherLines) {
            suggestions.push(`Consider splitting this ${context.voucherType.toLowerCase()} into multiple entries. Current entry has ${context.entries.length} lines, recommended maximum is ${config.rules.maxVoucherLines}.`)
        }
        ...
        if (totalAmount > config.thresholds.maxAmountWithoutApproval) {
            suggestions.push(`Large amount detected (${totalAmount.toLocaleString()}). Consider adding additional approval or documentation.`)
        }
        ...
        if (errors.some(e => e.code === 'CURRENCY_MISMATCH')) {
            suggestions.push('Ensure all entries use the correct account currency. Consider using multi-currency accounts if needed.')
        }

        if (warnings.some(w => w.code === 'COST_CENTER_RECOMMENDED')) {
            suggestions.push('Adding cost centers will improve financial reporting and analysis capabilities.')
        }
     ...
     */
    private async analyzeHistoricalPatterns(context: VoucherValidationContext): Promise<{ suggestion: string; confidence: number }[]> {
     ...
     */
    private async logValidationError(error: ValidationErrorWithContext, context: VoucherValidationContext) {
    ...
    private async validateVoucher(context: VoucherValidationContext): Promise<ValidationResult> {
        ...
        const eventType = result.isValid ? 'validation_success' :
            result.errors.length > 0 ? 'validation_failure' : 'validation_warning'
        await this.logValidationEvent(eventType, context, result, processingTime)

        return result
    }

    private async validateFieldRealTime(field: string, value: any, context: Partial<VoucherValidationContext>): Promise<ValidationResult> {
