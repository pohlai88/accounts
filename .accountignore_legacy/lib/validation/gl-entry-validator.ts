...
export const GLEntryValidationSchema = z
  .object({
...
export class GLEntryValidator {
  ...
  private async validateVoucherRules(context: VoucherValidationContext): Promise<{
    ...
    const duplicateCheck = await this.checkDuplicateVoucher(
      context.voucherType,
      context.voucherNo,
      context.companyId,
    );
  ...
  private async validateGLEntry(
    ...
    if (
      ...
      !entry.costCenter &&
      context.voucherType !== "Period Closing Voucher"
    ) {
    ...
    if (isForeignAccount) {
      const hasAccountAmt =
        (entry.debitInAccountCurrency ?? 0) > 0 || (entry.creditInAccountCurrency ?? 0) > 0;
      if (!hasAccountAmt) {
        errors.push({
      ...
      if (entry.transactionCurrency && entry.transactionCurrency !== account.account_currency) {
        if (!entry.transactionExchangeRate || entry.transactionExchangeRate <= 0) {
          errors.push({
            ...
            message:
              "Positive exchange rate required when transaction currency differs from account currency",
            severity: "error",
            category: "business_rule",
          });
        }
    ...
    if (account.tax_rate && account.tax_rate > 0) {
      suggestions.push(
        `Account ${account.name} has a tax rate of ${account.tax_rate}%. Consider if tax calculation is needed.`,
      );
    }
  ...
  private async validatePurchaseInvoiceRules(
    ...
    for (const entry of context.entries) {
      ...
      if (
        await this.isAccountType(entry.accountId, [
          "Direct Expense",
          "Indirect Expense",
          "Cost of Goods Sold",
        ])
  ...
  private async validatePostingDate(postingDate: string, companyId: string) {
    ...
    if (!error && data && data.length) {
      ...
      if (postingDate <= maxClosed) {
        return {
          isValid: false,
          message: `Cannot post to closed period (closed through ${maxClosed}).`,
        };
      }
    }
  ...
  private async validateAgainstVoucher(entry: GLEntryInput) {
    ...
    if (error || !data) {
      return {
        isValid: false,
        message: `Against voucher ${entry.againstVoucherType} ${entry.againstVoucher} not found`,
      };
    }
    ...
    switch (field) {
      ...
      case "voucherNo":
        if (value && context.voucherType && context.companyId) {
          const duplicate = await this.checkDuplicateVoucher(
            context.voucherType,
            value,
            context.companyId,
          );
