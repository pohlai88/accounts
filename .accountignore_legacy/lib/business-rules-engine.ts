...
export class BusinessRulesEngine {
    ...
    private static async validateAccountConfiguration(
        ...
        const receivableAccounts = accounts?.filter(acc =>
            acc.account_type === 'Asset' && acc.name.toLowerCase().includes('receivable')
        ) || []
    ...
    static async validatePaymentEntry(
        ...
        try {
            ...
            const totalAllocated = paymentData.allocations?.reduce(
            ...
            if (totalAllocated < paymentData.paid_amount) {
                info.push(`Unallocated amount: ${paymentData.paid_amount - totalAllocated} will be treated as advance payment`)
            }
            ...
            errors.push('Payment validation error: ' + (error instanceof Error ? error.message : 'Unknown error'))
            return { valid: false, errors, warnings, info }
        }
    }
