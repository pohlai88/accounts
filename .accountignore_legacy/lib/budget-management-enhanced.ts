...
export class ERPNextBudgetManagementService {
        ...
        try {
            const { data, error } = await supabase
                ...
                .insert({
                    ...
                    action_if_accumulated_monthly_budget_exceeded: budgetData.actionIfAccumulatedMonthlyBudgetExceeded,
     ...
     */
    static async submitBudget(budgetId: string, userId: string): Promise<{
        success: boolean
        error?: string
    }> {
        try {
     ...
     * Cancel budget
     */
    static async cancelBudget(budgetId: string, userId: string): Promise<{
        success: boolean
        error?: string
    }> {
        try {
    ...
    static async validateExpenseAgainstBudget(
        ...
        try {
            const { data, error } = await supabase
                .rpc('validate_expense_against_budget', {
            ...
            return {
                ...
                variancePercentage: result.budget_amount > 0
                    ? ((result.actual_amount + expenseAmount - result.budget_amount) / result.budget_amount) * 100
                    : 0
            }
        } catch (error) {
            return {
    ...
    static async getBudgets(
        ...
        try {
            ...
            const budgets: ERPNextBudget[] = data.map(item => ({
                ...
                actionIfAccumulatedMonthlyBudgetExceeded: item.action_if_accumulated_monthly_budget_exceeded,
    ...
    static async getBudgetDetails(budgetId: string): Promise<{
        ...
        try {
            ...
            const budget: ERPNextBudget = {
                ...
                actionIfAccumulatedMonthlyBudgetExceeded: budgetData.action_if_accumulated_monthly_budget_exceeded,
     ...
     */
    static async deleteBudgetAccount(accountId: string): Promise<{ success: boolean; error?: string }> {
        try {
            const { error } = await supabase
                .from('budget_accounts')
                .delete()
