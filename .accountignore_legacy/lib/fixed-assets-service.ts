...
export type DepreciationMethod = 'Straight Line' | 'Double Declining Balance' | 'Written Down Value' | 'Sum of Years Digits' | 'Manual'
export type AssetStatus = 'Draft' | 'Submitted' | 'Partially Depreciated' | 'Fully Depreciated' | 'Sold' | 'Scrapped' | 'In Maintenance' | 'Capitalized'
export type AssetCondition = 'Excellent' | 'Good' | 'Fair' | 'Poor' | 'Under Repair'
export type AssetOwner = 'Company' | 'Supplier' | 'Customer'
export type MovementType = 'Purchase' | 'Transfer' | 'Issue' | 'Receipt' | 'Disposal' | 'Maintenance' | 'Repair'
...
export class FixedAssetsService {
     ...
     */
    static async createAssetCategory(input: CreateAssetCategoryInput): Promise<ApiResponse<AssetCategory>> {
        try {
     ...
     */
    static async createAssetLocation(input: CreateAssetLocationInput): Promise<ApiResponse<AssetLocation>> {
        try {
    ...
    static async createAsset(input: CreateAssetInput): Promise<ApiResponse<Asset>> {
        try {
            ...
            const assetCode = await this.generateAssetCode(input.company_id, categoryData?.category_code || 'ASSET')
    ...
    static async getAssets(
        ...
        try {
            ...
            if (filters?.search) {
                query = query.or(`asset_name.ilike.%${filters.search}%,asset_code.ilike.%${filters.search}%,serial_no.ilike.%${filters.search}%`)
            }
        ...
        try {
            ...
            if (asset.calculate_depreciation) {
                const { error: scheduleError } = await supabase
                    .rpc('generate_asset_depreciation_schedule', { p_asset_id: assetId })
     ...
     */
    static async getDepreciationSchedule(assetId: string): Promise<ApiResponse<DepreciationSchedule[]>> {
        try {
     ...
     */
    static async getPendingDepreciationEntries(companyId: string, asOfDate?: string): Promise<ApiResponse<any[]>> {
        try {
            const { data: entries, error } = await supabase
                .rpc('get_pending_depreciation_entries', {
    ...
    static async getAssetBookValue(assetId: string, asOfDate?: string): Promise<ApiResponse<number>> {
        try {
            const { data: bookValue, error } = await supabase
                .rpc('get_asset_book_value', {
                    p_asset_id: assetId,
                    p_as_of_date: asOfDate || new Date().toISOString().split('T')[0]
                })
     ...
     */
    static async createMaintenanceLog(input: CreateMaintenanceLogInput): Promise<ApiResponse<MaintenanceLog>> {
        try {
            ...
            return { success: true, data: maintenanceLog, message: 'Maintenance log created successfully' }
     ...
     */
    static async getUpcomingMaintenance(companyId: string, daysAhead: number = 30): Promise<ApiResponse<MaintenanceLog[]>> {
        try {
     ...
     */
    static async createAssetDisposal(input: CreateAssetDisposalInput): Promise<ApiResponse<AssetDisposal>> {
        try {
            ...
            return { success: true, data: disposal, message: 'Asset disposal record created successfully' }
     ...
     * Get asset analytics
     */
    static async getAssetAnalytics(companyId: string): Promise<ApiResponse<{
        total_assets: number
        ...
        try {
            ...
            const totalAssetValue = assetSummary?.reduce((sum, asset) => sum + (asset.gross_purchase_amount || 0), 0) || 0
            const totalAccumulatedDepreciation = depreciationSummary?.reduce((sum, dep) => sum + (dep.depreciation_amount || 0), 0) || 0
            const netBookValue = totalAssetValue - totalAccumulatedDepreciation

            // Group assets by category
            const categoryGroups = assetsByCategory?.reduce((acc, asset) => {
