...
export type KPICategory = 'Financial Performance' | 'Liquidity' | 'Efficiency' | 'Profitability' | 'Growth' | 'Risk Management' | 'Operational'
export type CalculationFrequency = 'Real-time' | 'Hourly' | 'Daily' | 'Weekly' | 'Monthly' | 'Quarterly'
export type MeasurementPeriod = 'Daily' | 'Weekly' | 'Monthly' | 'Quarterly' | 'Annually' | 'YTD' | 'Rolling'
export type PerformanceRating = 'Excellent' | 'Good' | 'Acceptable' | 'Needs Attention' | 'No Target' | 'Unknown'
export type TrendDirection = 'Up' | 'Down' | 'Stable'
export type ChartType = 'line' | 'bar' | 'gauge' | 'pie' | 'area' | 'scatter' | 'heatmap' | 'trend'
export type InsightType = 'Trend Analysis' | 'Anomaly Detection' | 'Forecast Alert' | 'Performance Alert' | 'Opportunity' | 'Risk Warning' | 'Benchmark Comparison'
...
export class AdvancedKPIService {
    ...
    static async getExecutiveDashboard(
        ...
        try {
            ...
            const kpiSummary = {
                ...
                excellent_performance: kpiSummaryData?.filter(kpi => kpi.performance_rating === 'Excellent')?.length || 0,
                needs_attention: kpiSummaryData?.filter(kpi => kpi.performance_rating === 'Needs Attention')?.length || 0,
    ...
    static async getKPIAnalysis(
        ...
        try {
            ...
            // Generate forecast
            const forecast = await this.generateKPIForecast(kpiId, historicalValues || [], forecastPeriods)
            ...
            const recommendations = this.generateKPIRecommendations(kpi, currentPerformance, trendAnalysis)
    ...
    static async generateFinancialInsights(companyId: string): Promise<{
        ...
        try {
            const { data, error } = await supabase
                .rpc('generate_financial_insights', {
                    p_company_id: companyId,
    ...
    static async getExecutiveSummary(
        ...
        data?: {
            ...
            strategic_recommendations: Array<{ area: string; recommendation: string; priority: SeverityLevel }>
        }
        error?: string
    }> {
        try {
            ...
            const { data: kpiData } = await supabase
                ...
                .eq('company_id', companyId)
                .eq('recency_rank', 1)
                .in('kpi_code', ['TOTAL_REVENUE', 'NET_PROFIT_MARGIN', 'CURRENT_RATIO', 'CASH_CONVERSION_CYCLE'])

            const financialHighlights = kpiData?.map(kpi => ({
                metric: kpi.kpi_name,
                value: kpi.kpi_value,
            ...
            const performanceAlerts = kpiData?.filter(kpi =>
                kpi.performance_rating === 'Needs Attention'
            ).map(kpi => ({
                kpi: kpi.kpi_name,
    ...
    private static async calculatePerformanceOverview(companyId: string): Promise<{
        ...
        const financialKPIs = kpisByCategory.filter(kpi => kpi.kpi_category === 'Financial Performance')
    ...
    private static async getBenchmarkComparison(companyId: string): Promise<{
        ...
        const peerComparison = kpiData?.map(kpi => ({
            kpi: kpi.kpi_name,
        ...
        const avgPercentile = peerComparison.reduce((sum, item) => sum + item.percentile_ranking, 0) / peerComparison.length
        const industryRanking = avgPercentile > 80 ? 'Top 20%' :
            avgPercentile > 60 ? 'Top 40%' :
    ...
    private static calculateTrendAnalysis(values: KPIValue[]): {
        ...
        const direction: TrendDirection = overallChange > 5 ? 'Up' :
            overallChange < -5 ? 'Down' : 'Stable'
        ...
        const monthlyGrowthRate = monthlyValues.length === 2 ?
            ((monthlyValues[1].kpi_value - monthlyValues[0].kpi_value) / Math.abs(monthlyValues[0].kpi_value)) * 100 : 0

        const quarterlyGrowthRate = quarterlyValues.length >= 2 ?
            ((quarterlyValues[quarterlyValues.length - 1].kpi_value - quarterlyValues[0].kpi_value) / Math.abs(quarterlyValues[0].kpi_value)) * 100 : 0
    ...
    private static calculateConsistency(values: KPIValue[]): number {
        ...
        for (let i = 1; i < values.length; i++) {
            const change = ((values[i].kpi_value - values[i - 1].kpi_value) / Math.abs(values[i - 1].kpi_value)) * 100
            changes.push(change)
        }
    ...
    private static calculateAverageGrowthRate(values: KPIValue[]): number {
        ...
        const growthRates = []
        for (let i = 1; i < values.length; i++) {
            const rate = ((values[i].kpi_value - values[i - 1].kpi_value) / Math.abs(values[i - 1].kpi_value)) * 100
