...
export interface WorkingCapitalComponent {
    ...
    component_category: 'Current Assets' | 'Current Liabilities' | 'Inventory' | 'Receivables' | 'Payables'
...
export interface CashOptimizationStrategy {
    ...
    strategy_type: 'Cash Concentration' | 'Investment' | 'Debt Management' | 'Working Capital' | 'Hedging'
...
export interface InvestmentOpportunity {
    ...
    investment_type: 'Money Market' | 'Treasury Bills' | 'CDs' | 'Commercial Paper' | 'Corporate Bonds'
...
export interface WorkingCapitalRecommendation {
    ...
    recommendation_type: 'Receivables Optimization' | 'Inventory Optimization' | 'Payables Optimization' | 'Cash Management'
    component_id?: string
...
export interface TreasuryOptimizationAnalysis {
    working_capital_analysis: {
        by_component: { component: string; balance: number; efficiency_score: number; optimization_potential: number }[]
        by_category: { category: string; total_balance: number; avg_efficiency: number; priority_count: number }[]
        cycle_analysis: { metric: string; current_days: number; target_days: number; improvement_potential: number }[]
        trend_analysis: { period: string; working_capital: number; efficiency_score: number; optimization_savings: number }[]
    }
    optimization_performance: {
        by_strategy_type: { strategy_type: string; execution_count: number; avg_return: number; success_rate: number }[]
        by_risk_tolerance: { risk_level: string; allocation: number; returns: number; risk_adjusted_return: number }[]
        by_time_horizon: { horizon: string; investment_count: number; avg_return: number; liquidity_impact: number }[]
        execution_quality: { metric: string; average_score: number; best_practice_gap: number; improvement_area: string }[]
    }
    liquidity_analysis: {
        position_trends: { date: string; total_liquidity: number; risk_score: number; optimization_opportunity: number }[]
        account_performance: { account_type: string; avg_balance: number; utilization_rate: number; efficiency_score: number }[]
        cash_flow_patterns: { period: string; inflows: number; outflows: number; net_flow: number; predictability: number }[]
        risk_assessment: { risk_factor: string; exposure_amount: number; risk_level: string; mitigation_status: string }[]
    }
    investment_analysis: {
        opportunity_performance: { investment_type: string; avg_return: number; risk_score: number; liquidity_score: number }[]
        provider_analysis: { provider: string; opportunity_count: number; avg_rating: number; performance_score: number }[]
        market_trends: { period: string; avg_rates: number; volatility: number; opportunity_count: number }[]
        ai_recommendation_accuracy: { recommendation_score_range: string; count: number; actual_performance: number }[]
    }
    recommendations: {
...
export class TreasuryOptimizationService {
    ...
    static async getWorkingCapitalComponents(
        filters?: {
        ...
        try {
            ...
            if (component) {
                ...
                if (component.minimum_balance_required && component.maximum_balance_threshold) {
                    const optimalBalance = (component.minimum_balance_required + component.maximum_balance_threshold) / 2
                ...
                if (daysOutstanding && component.target_days_outstanding) {
                    const daysEfficiency = Math.max(0, 100 - Math.abs(daysOutstanding - component.target_days_outstanding) * 2)
    ...
    static async createCashOptimizationStrategy(
        strategy: Omit<CashOptimizationStrategy, 'id' | 'created_at' | 'updated_at' | 'total_optimized_amount' | 'total_return_generated' | 'number_of_executions'>
    ...
    static async getCashOptimizationStrategies(
        filters?: {
    ...
    static async executeOptimizationStrategy(
        ...
        try {
            ...
            const executionData: Omit<CashOptimizationExecution, 'id' | 'created_at' | 'updated_at' | 'strategy' | 'investment_opportunity'> = {
                ...
                approval_level: executionAmount > (strategy.approval_required_above || Infinity) ? 'Treasury Manager' : 'System',
                fees_paid: 0,
    ...
    static async calculateLiquidityMetrics(
        positionDate?: string
        ...
        try {
            ...
            const weightedRiskScore = positions.reduce((sum, p) =>
                sum + (p.liquidity_risk_score || 0) * (p.available_balance / totalLiquidity), 0
            )
            const optimizationOpportunities = positions.reduce((sum, p) => sum + (p.optimization_opportunity || 0), 0)
    ...
    static async generateWorkingCapitalRecommendations(): Promise<ServiceResponse<WorkingCapitalRecommendation[]>> {
        try {
            // Get current working capital components
            const componentsResult = await this.getWorkingCapitalComponents({ component_status: 'active' })
            if (!componentsResult.success || !componentsResult.data) {
                throw new Error('Failed to fetch working capital components')
            }

            const components = componentsResult.data
            const recommendations: Omit<WorkingCapitalRecommendation, 'id' | 'created_at' | 'updated_at' | 'component'>[] = []
            ...
            for (const component of components) {
                ...
                if (efficiency < 70 && component.optimization_priority === 'High') {
                    ...
                    const recommendation: Omit<WorkingCapitalRecommendation, 'id' | 'created_at' | 'updated_at' | 'component'> = {
    ...
    private static getRecommendationType(category: string): WorkingCapitalRecommendation['recommendation_type'] {
        switch (category) {
            case 'Receivables': return 'Receivables Optimization'
            case 'Inventory': return 'Inventory Optimization'
            case 'Payables': return 'Payables Optimization'
            default: return 'Cash Management'
        }
    }

    static async getWorkingCapitalRecommendations(
        filters?: {
    ...
    static async calculateTreasuryPerformance(
        ...
        try {
            ...
            const avgCashBalance = positions.length ?
                positions.reduce((sum, p) => sum + p.closing_balance, 0) / positions.length : 0
            const peakCashBalance = positions.length ?
                Math.max(...positions.map(p => p.closing_balance)) : 0
            const minCashBalance = positions.length ?
                Math.min(...positions.map(p => p.closing_balance)) : 0
            ...
            const avgEfficiency = components.length ?
                components.reduce((sum, c) => sum + (c.efficiency_score || 0), 0) / components.length : 0
            ...
            const totalReturns = executions?.reduce((sum, e) => sum + (e.actual_return || e.expected_return || 0), 0) || 0

            const performanceData: Omit<TreasuryPerformanceAnalytics, 'id' | 'created_at' | 'updated_at'> = {
                ...
                optimization_success_rate: executions?.length ?
                    (executions.filter(e => (e.actual_return || e.expected_return || 0) > 0).length / executions.length) * 100 : 0,
    ...
    private static calculateAverageCashCycle(positions: LiquidityPosition[]): number {
        const validCycles = positions.filter(p => p.cash_conversion_cycle_days).map(p => p.cash_conversion_cycle_days!)
    ...
    static async getTreasuryOptimizationDashboard(): Promise<ServiceResponse<TreasuryOptimizationDashboard>> {
        try {
            // Get working capital components
            const componentsResult = await this.getWorkingCapitalComponents({ component_status: 'active' })
            const components = componentsResult.data || []
            ...
            const strategiesResult = await this.getCashOptimizationStrategies({ strategy_status: 'active' })
            const strategies = strategiesResult.data || []

            // Get current liquidity positions
            const positionsResult = await this.getLiquidityPositions({
                position_date_from: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
            })
            const positions = positionsResult.data || []
            ...
            const workingCapitalSummary = {
                ...
                working_capital_ratio: components.length ?
                    components.reduce((sum, c) => sum + (c.efficiency_score || 0), 0) / components.length / 100 : 0,
                ...
                liquidity_score: positions.length ?
                    positions.reduce((sum, p) => sum + (100 - (p.liquidity_risk_score || 0)), 0) / positions.length : 0
            }
            ...
            const optimizationSummary = {
                ...
                average_strategy_performance: strategies.length ?
                    strategies.reduce((sum, s) => sum + (s.current_performance_score || 0), 0) / strategies.length : 0,
            ...
            const liquiditySummary = {
                ...
                days_cash_on_hand: positions.length ?
                    positions.reduce((sum, p) => sum + (p.days_cash_on_hand || 0), 0) / positions.length : 0,
                liquidity_risk_score: positions.length ?
                    positions.reduce((sum, p) => sum + (p.liquidity_risk_score || 0), 0) / positions.length : 0,
                credit_utilization: positions.length ?
                    positions.reduce((sum, p) => sum + (p.credit_utilization_ratio || 0), 0) / positions.length : 0,
                cash_flow_forecast_7d: positions.reduce((sum, p) =>
                    sum + (p.forecasted_inflows_7d || 0) - (p.forecasted_outflows_7d || 0), 0)
            }

            const investmentSummary = {
                available_opportunities: opportunities.filter(o => o.opportunity_status === 'available').length,
                total_potential_return: opportunities.reduce((sum, o) => sum + (o.expected_return_amount || 0), 0),
                average_risk_score: opportunities.length ?
                    opportunities.reduce((sum, o) => {
                        const riskScore = { 'Very Low': 20, 'Low': 40, 'Medium': 60, 'High': 80 }[o.risk_category] || 50
                        return sum + riskScore
                    }, 0) / opportunities.length : 0,
                recommended_investments: opportunities.filter(o => (o.ai_recommendation_score || 0) > 70).length,
    ...
    static async getTreasuryOptimizationAnalysis(
        ...
        try {
            const startDate = period_start || new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
            ...
            const analysis: TreasuryOptimizationAnalysis = {
                working_capital_analysis: {
                    by_component: [
                        { component: 'Accounts Receivable', balance: 2500000, efficiency_score: 87.5, optimization_potential: 375000 },
                        { component: 'Inventory', balance: 1800000, efficiency_score: 72.3, optimization_potential: 450000 },
                        { component: 'Accounts Payable', balance: -1200000, efficiency_score: 91.2, optimization_potential: 120000 },
                        { component: 'Cash & Equivalents', balance: 800000, efficiency_score: 95.8, optimization_potential: 50000 }
                    ],
                    by_category: [
                        { category: 'Current Assets', total_balance: 5100000, avg_efficiency: 83.9, priority_count: 8 },
                        { category: 'Current Liabilities', total_balance: -1200000, avg_efficiency: 91.2, priority_count: 3 },
                        { category: 'Inventory', total_balance: 1800000, avg_efficiency: 72.3, priority_count: 5 }
                    ],
                    cycle_analysis: [
                        { metric: 'Days Sales Outstanding', current_days: 45.2, target_days: 35.0, improvement_potential: 10.2 },
                        { metric: 'Days Inventory Outstanding', current_days: 32.8, target_days: 28.0, improvement_potential: 4.8 },
                        { metric: 'Days Payable Outstanding', current_days: 28.5, target_days: 35.0, improvement_potential: -6.5 }
                    ],
                    trend_analysis: [
                        { period: 'Current Quarter', working_capital: 3900000, efficiency_score: 84.2, optimization_savings: 285000 },
                        { period: 'Previous Quarter', working_capital: 3750000, efficiency_score: 81.7, optimization_savings: 320000 },
                        { period: 'Year Ago', working_capital: 3600000, efficiency_score: 78.9, optimization_savings: 380000 }
                    ]
                },
                optimization_performance: {
                    by_strategy_type: [
                        { strategy_type: 'Cash Concentration', execution_count: 24, avg_return: 0.045, success_rate: 95.8 },
                        { strategy_type: 'Investment', execution_count: 18, avg_return: 0.038, success_rate: 88.9 },
                        { strategy_type: 'Working Capital', execution_count: 12, avg_return: 0.025, success_rate: 91.7 }
                    ],
                    by_risk_tolerance: [
                        { risk_level: 'Conservative', allocation: 2500000, returns: 112500, risk_adjusted_return: 0.042 },
                        { risk_level: 'Moderate', allocation: 1800000, returns: 97200, risk_adjusted_return: 0.048 },
                        { risk_level: 'Aggressive', allocation: 700000, returns: 45500, risk_adjusted_return: 0.058 }
                    ],
                    by_time_horizon: [
                        { horizon: 'Short-term (<30 days)', investment_count: 28, avg_return: 0.032, liquidity_impact: 0.15 },
                        { horizon: 'Medium-term (30-90 days)', investment_count: 15, avg_return: 0.041, liquidity_impact: 0.35 },
                        { horizon: 'Long-term (>90 days)', investment_count: 8, avg_return: 0.058, liquidity_impact: 0.75 }
                    ],
                    execution_quality: [
                        { metric: 'Speed', average_score: 92.5, best_practice_gap: 7.5, improvement_area: 'Automation' },
                        { metric: 'Cost', average_score: 88.3, best_practice_gap: 11.7, improvement_area: 'Fee Negotiation' },
                        { metric: 'Accuracy', average_score: 96.8, best_practice_gap: 3.2, improvement_area: 'Data Quality' }
                    ]
                },
                liquidity_analysis: {
                    position_trends: [
                        { date: '2024-09-09', total_liquidity: 4200000, risk_score: 23.5, optimization_opportunity: 315000 },
                        { date: '2024-09-02', total_liquidity: 3950000, risk_score: 28.1, optimization_opportunity: 380000 },
                        { date: '2024-08-26', total_liquidity: 3800000, risk_score: 31.7, optimization_opportunity: 420000 }
                    ],
                    account_performance: [
                        { account_type: 'Operating', avg_balance: 2100000, utilization_rate: 0.78, efficiency_score: 87.2 },
                        { account_type: 'Savings', avg_balance: 1500000, utilization_rate: 0.45, efficiency_score: 92.8 },
                        { account_type: 'Investment', avg_balance: 600000, utilization_rate: 0.89, efficiency_score: 85.5 }
                    ],
                    cash_flow_patterns: [
                        { period: 'Week 1', inflows: 850000, outflows: 720000, net_flow: 130000, predictability: 89.2 },
                        { period: 'Week 2', inflows: 920000, outflows: 780000, net_flow: 140000, predictability: 91.7 },
                        { period: 'Week 3', inflows: 760000, outflows: 690000, net_flow: 70000, predictability: 85.3 }
                    ],
                    risk_assessment: [
                        { risk_factor: 'Concentration Risk', exposure_amount: 1200000, risk_level: 'Medium', mitigation_status: 'In Progress' },
                        { risk_factor: 'Interest Rate Risk', exposure_amount: 800000, risk_level: 'Low', mitigation_status: 'Hedged' },
                        { risk_factor: 'Credit Risk', exposure_amount: 450000, risk_level: 'Low', mitigation_status: 'Monitored' }
                    ]
                },
                investment_analysis: {
                    opportunity_performance: [
                        { investment_type: 'Money Market', avg_return: 0.045, risk_score: 15.2, liquidity_score: 95.8 },
                        { investment_type: 'Treasury Bills', avg_return: 0.042, risk_score: 8.7, liquidity_score: 98.2 },
                        { investment_type: 'CDs', avg_return: 0.048, risk_score: 12.5, liquidity_score: 72.3 },
                        { investment_type: 'Corporate Bonds', avg_return: 0.062, risk_score: 28.9, liquidity_score: 68.7 }
                    ],
                    provider_analysis: [
                        { provider: 'Bank of America', opportunity_count: 8, avg_rating: 4.2, performance_score: 87.5 },
                        { provider: 'JPMorgan Chase', opportunity_count: 6, avg_rating: 4.5, performance_score: 91.3 },
                        { provider: 'Wells Fargo', opportunity_count: 5, avg_rating: 3.8, performance_score: 83.7 }
                    ],
                    market_trends: [
                        { period: 'Current Month', avg_rates: 0.0425, volatility: 0.015, opportunity_count: 23 },
                        { period: 'Previous Month', avg_rates: 0.0398, volatility: 0.018, opportunity_count: 19 },
                        { period: 'Two Months Ago', avg_rates: 0.0385, volatility: 0.022, opportunity_count: 17 }
                    ],
                ...
                recommendations: [
                    {
                        ...
                        recommendation: 'Implement automated daily cash concentration across all operating accounts',
                    ...
                    },
                    {
                        ...
                        recommendation: 'Optimize accounts receivable collection processes to reduce DSO by 8 days',
                    ...
                    },
                    {
                        ...
                        recommendation: 'Diversify short-term investment portfolio across additional asset classes',
