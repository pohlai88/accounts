...
export const SecurityHelpers = {
  ...
  shouldSkipCSRF(path: string): boolean {
    return SecurityConfig.exemptPaths.csrf.some(exemptPath => path.startsWith(exemptPath));
  },
  ...
  shouldSkipRateLimit(path: string): boolean {
    return SecurityConfig.exemptPaths.rateLimit.some(exemptPath => path.startsWith(exemptPath));
  },
  ...
  shouldSkipHeaders(path: string): boolean {
    return SecurityConfig.exemptPaths.headers.some(exemptPath => path.startsWith(exemptPath));
  },
  ...
  getRateLimitRule(path: string, isBot: boolean = false): RateLimitRule {
    ...
    const rule = Object.entries(limits).find(([prefix]) => path.startsWith(prefix))?.[1];
