...
class ValidationTelemetryService {
     ...
     */
    async recordValidationEvent(event: Omit<ValidationEvent, 'eventId' | 'timestamp'>): Promise<void> {
    ...
    getHealthStatus(): {
        ...
        if (criticalAlerts > 0 || overallMetrics.successRate < 0.8) {
            status = 'unhealthy'
        } else if (highAlerts > 0 || overallMetrics.successRate < 0.95 || overallMetrics.averageProcessingTime > 1000) {
            status = 'degraded'
        }
    ...
    private logEventToConsole(event: ValidationEvent): void {
        const emoji = event.eventType === 'validation_success' ? '✅' :
            event.eventType === 'validation_failure' ? '❌' : '⚠️'
...
export const ValidationTelemetryUtils = {
    formatMetrics: (metrics: ValidationMetrics) => {
        return {
            ...
            'Top Errors': Object.entries(metrics.errorsByCode)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 3)
    ...
    getPerformanceGrade: (metrics: ValidationMetrics): 'A' | 'B' | 'C' | 'D' | 'F' => {
        ...
        if (successRate >= 0.90 && avgTime <= 500) return 'C'
        if (successRate >= 0.80 && avgTime <= 1000) return 'D'
        return 'F'
    },
    ...
    getRecommendations: (metrics: ValidationMetrics): string[] => {
        ...
        if (metrics.averageProcessingTime > 500) {
            recommendations.push('Optimize validation logic and database queries to reduce processing time')
        }
        ...
        if (topErrors.length > 0) {
            recommendations.push(`Focus on fixing top error: ${topErrors[0][0]} (${topErrors[0][1]} occurrences)`)
        }
