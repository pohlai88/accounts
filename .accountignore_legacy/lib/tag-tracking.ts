...
export class TagTrackingService {
    ...
    static async assignTagsToGLEntry(
        ...
        try {
            ...
            const totalPercentage = tagAssignments.reduce((sum, assignment) => sum + assignment.percentage, 0)
    ...
    static async getTagReport(
        ...
        `)
            ...
            report.forEach(item => {
                item.percentage_of_total = totalAmount > 0 ? (Math.abs(item.net_amount) / totalAmount) * 100 : 0
            })
    ...
    static async getSuggestedTags(
        ...
        `)
            ...
            return {
                success: true,
                suggestions: Array.from(suggestions.values()).sort((a, b) => b.recent_usage - a.recent_usage)
            }
        } catch (error) {
