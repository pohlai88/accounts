...
export class IntercompanyManagementService {
    ...
    try {
      ...
      if (!eligibilityResult.eligible) {
        return {
          success: false,
          error:
            "Companies must have a parent-child relationship to perform intercompany transactions. Please set up company hierarchy first.",
        };
      }
   ...
   */
  private static async createIntercompanyGLEntries(transaction: IntercompanyTransaction): Promise<{
    success: boolean;
    glEntries?: IntercompanyGL[];
    error?: string;
  }> {
  ...
  static async getIntercompanyBalances(
    ...
    try {
      const { data: balances, error } = await supabase.rpc("get_intercompany_balances", {
   ...
   */
  static async getIntercompanyGLEntries(transactionId: string): Promise<{
    success: boolean;
    glEntries?: IntercompanyGL[];
    error?: string;
  }> {
  ...
  static async cancelIntercompanyTransaction(
    ...
    try {
      ...
      if (transaction.status === "Approved") {
        return {
          success: false,
          error: "Cannot cancel approved transaction. Create reversal instead.",
        };
      }
   ...
   */
  static async getIntercompanyAccountStatus(companyId: string): Promise<{
    success: boolean;
   ...
   */
  static async setupIntercompanyAccounts(companyId: string): Promise<{
    success: boolean;
    accounts?: IntercompanyAccount[];
    error?: string;
  }> {
