...
export class BankReconciliationService {
   ...
   */
  static async addStatementItems(
    statementId: string,
    items: CreateBankStatementItemInput[],
  ): Promise<{ success: boolean; error?: string }> {
    try {
      const { error } = await supabase.from("bank_statement_items").insert(
        items.map(item => ({
   ...
   */
  static async getStatementItems(
    statementId: string,
  ): Promise<{ success: boolean; items?: BankStatementItem[]; error?: string }> {
    try {
      const { data, error } = await supabase
  ...
  static async findPotentialMatches(
    statementItemId: string,
    threshold: number = 80,
  ): Promise<{ success: boolean; matches?: PotentialMatch[]; error?: string }> {
    try {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          match_confidence: 100.0, // Manual matches have 100% confidence
  ...
  static async autoReconcile(
    ...
    try {
      const { data, error } = await supabase.rpc("auto_reconcile_statement_items", {
        statement_id: statementId,
      });
  ...
  static async getReconciliationRules(
    companyId: string,
  ): Promise<{ success: boolean; rules?: ReconciliationRule[]; error?: string }> {
    try {
      const { data, error } = await supabase
        .from("reconciliation_rules")
        .select("*")
