...
export class EnhancedTrialBalanceService {
        ...
        try {
            ...
            const accounts = await Promise.all(
                list.map(async (row) => {
                    ...
                    if (filters.currency && row.account_currency && filters.currency !== row.account_currency) {
                    ...
                    const closing_debit = includeOpening
                        ? toNumber(row.closing_debit)
                        : add(0, debit)
                    const closing_credit = includeOpening
                        ? toNumber(row.closing_credit)
                        : add(0, credit)
            ...
            const filteredAccounts = accounts
                ...
                .sort((a, b) => {
                    const la = (list.find((r) => r.account_id === a.account_id)?.lft ?? 2_147_483_647) as number
                    const lb = (list.find((r) => r.account_id === b.account_id)?.lft ?? 2_147_483_647) as number
    ...
    static validateTrialBalance(trialBalance: TrialBalance): TrialBalanceValidationResult {
        ...
        groupAccounts.forEach(group => {
            const children = trialBalance.accounts.filter(
                a => a.parent_account_id === group.account_id
            )

            if (children.length > 0) {
                const childSum = children.reduce((sum, child) => add(sum, child.net_closing), 0)
                if (Math.abs(group.net_closing - childSum) > EPSILON) {
                    errors.push(`Group account ${group.account_name} balance (${group.net_closing}) doesn't match sum of children (${childSum})`)
                }
            }
        })
