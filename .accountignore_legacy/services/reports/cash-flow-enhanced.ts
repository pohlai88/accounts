...
export class EnhancedCashFlowService {
    ...
    private static async getCashFlowIndirect(filters: CashFlowFilters): Promise<CashFlowData> {
        ...
        const beg = new Date(filters.fromDate); beg.setDate(beg.getDate() - 1)
        const begStr = beg.toISOString().slice(0, 10)

        const [bsBegRes, bsEndRes] = await Promise.all([
            BalanceSheetService.getBalanceSheet({ companyId: filters.companyId, asOfDate: begStr, currency: filters.currency, fiscalYear: filters.fiscalYear }),
            BalanceSheetService.getBalanceSheet({ companyId: filters.companyId, asOfDate: filters.toDate, currency: filters.currency, fiscalYear: filters.fiscalYear }),
        ])
    ...
    private static buildDirectCashFlow(receipts: any[], payments: any[], filters: CashFlowFilters): CashFlowData {
    ...
    private static operatingSection(netProfit: number, bsBeg: BS, bsEnd: BS) {
        ...
        if (Math.abs(dep) > EPS) lines.push({ description: 'Depreciation and Amortization (non-cash)', amount: dep, is_subtotal: false, indent: 0 })

        lines.push({ description: 'Changes in Working Capital', amount: 0, is_subtotal: false, indent: 0 })

        const dAR = this.deltaForType(bsBeg, bsEnd, 'Receivable')
        if (Math.abs(dAR) > EPS) lines.push({ description: 'Accounts Receivable', amount: -dAR, is_subtotal: false, indent: 1 })

        const dInv = this.deltaForType(bsBeg, bsEnd, 'Stock')
        if (Math.abs(dInv) > EPS) lines.push({ description: 'Inventory', amount: -dInv, is_subtotal: false, indent: 1 })

        const dAP = this.deltaForType(bsBeg, bsEnd, 'Payable')
        if (Math.abs(dAP) > EPS) lines.push({ description: 'Accounts Payable', amount: dAP, is_subtotal: false, indent: 1 })

        const dOCA = this.deltaOtherCurrentAssets(bsBeg, bsEnd)
        if (Math.abs(dOCA) > EPS) lines.push({ description: 'Other Current Assets', amount: -dOCA, is_subtotal: false, indent: 1 })

        const dOCL = this.deltaOtherCurrentLiabilities(bsBeg, bsEnd)
        if (Math.abs(dOCL) > EPS) lines.push({ description: 'Other Current Liabilities', amount: dOCL, is_subtotal: false, indent: 1 })
    ...
    private static investingSection(bsBeg: BS, bsEnd: BS) {
        ...
        if (Math.abs(dFA) > EPS) {
            lines.push({
                description: dFA > 0 ? 'Purchase of Fixed Assets (CAPEX)' : 'Proceeds from Sale of Fixed Assets',
                amount: -dFA,
                is_subtotal: false,
                indent: 0,
            })
    ...
    private static financingSection(bsBeg: BS, bsEnd: BS, netProfit: number) {
        ...
        if (dividendsApprox > EPS) {
            lines.push({ description: 'Dividends Paid (estimated)', amount: -dividendsApprox, is_subtotal: false, indent: 0 })
        }
    ...
    private static deltaForType(bsBeg: BS, bsEnd: BS, accountType: string): number {
        const tot = (bs: BS) =>
            sum([
                ...
                ...bs.assets.non_current_assets.filter(a => a.account_type === accountType).map(a => a.amount),
                ...bs.assets.fixed_assets.filter(a => a.account_type === accountType).map(a => a.amount),
                ...bs.liabilities.current_liabilities.filter(a => a.account_type === accountType).map(a => a.amount),
                ...bs.liabilities.non_current_liabilities.filter(a => a.account_type === accountType).map(a => a.amount),
            ])
    ...
    private static deltaOtherCurrentAssets(bsBeg: BS, bsEnd: BS): number {
        ...
        const tot = (bs: BS) => bs.assets.current_assets.filter(a => !exclude.has(a.account_type)).reduce((s, a) => add(s, a.amount), 0)
        return add(tot(bsEnd), -tot(bsBeg))
    }

    private static deltaOtherCurrentLiabilities(bsBeg: BS, bsEnd: BS): number {
        const exclude = new Set(['Payable'])
        const tot = (bs: BS) => bs.liabilities.current_liabilities.filter(a => !exclude.has(a.account_type)).reduce((s, a) => add(s, a.amount), 0)
        return add(tot(bsEnd), -tot(bsBeg))
    }
     ...
     */
    private static async analyzeCashFlow(cf: CashFlowData, filters: CashFlowFilters): Promise<CashFlowAnalysis> {
        ...
        const operatingCashFlowMargin = revenue > 0
            ? (cf.totals.net_cash_from_operating / revenue) * 100
            : 0

        // Free Cash Flow (Operating Cash Flow - CAPEX)
        const capex = cf.investing_activities.lines
            .filter(line => line.description.includes('CAPEX') || line.description.includes('Fixed Assets'))
            .reduce((sum, line) => add(sum, Math.abs(line.amount)), 0)
        const freeCashFlow = add(cf.totals.net_cash_from_operating, -capex)

        // Cash Flow Adequacy Ratio
        const cashFlowAdequacyRatio = capex > EPS
            ? cf.totals.net_cash_from_operating / capex
            : 0

        // Quality of Earnings Ratio
        const netProfit = cf.operating_activities.lines.find(l => l.description === 'Net Profit')?.amount || 0
        const qualityOfEarningsRatio = netProfit !== 0
            ? cf.totals.net_cash_from_operating / netProfit
            : 0

        // Operating Leverage
        const operatingLeverage = cf.totals.net_cash_from_operating !== 0
        ...
        if (freeCashFlow < 0) {
            warnings.push('Negative free cash flow - may indicate difficulty funding operations and growth')
        } else if (freeCashFlow > 0) {
            insights.push('Positive free cash flow provides flexibility for growth and shareholder returns')
        }

        if (cashFlowAdequacyRatio < 1 && capex > 0) {
            warnings.push('Cash flow adequacy ratio below 1 - operating cash flow may not be sufficient to cover capital expenditures')
        }
    ...
    static validateCashFlow(cf: CashFlowData): CashFlowValidationResult {
        ...
        const netFromSections = add(add(cf.totals.net_cash_from_operating, cf.totals.net_cash_from_investing), cf.totals.net_cash_from_financing)
        if (Math.abs(cf.totals.net_change_in_cash - netFromSections) > EPS) {
            errors.push(`Net change mismatch: expected ${netFromSections.toFixed(2)}, got ${cf.totals.net_change_in_cash.toFixed(2)}`)
        }

        const computedEnding = add(cf.cash_reconciliation.beginning_cash, cf.cash_reconciliation.net_change_in_cash)
        if (Math.abs(cf.cash_reconciliation.ending_cash - computedEnding) > EPS) {
            errors.push(`Ending cash mismatch: expected ${computedEnding.toFixed(2)}, got ${cf.cash_reconciliation.ending_cash.toFixed(2)}`)
        }

        const actualNetChange = add(cf.cash_reconciliation.ending_cash_per_balance_sheet, -cf.cash_reconciliation.beginning_cash)
        ...
        if (Math.abs(cf.cash_reconciliation.difference) > 1.0) {
            warnings.push(`Significant difference vs Balance Sheet: ${cf.cash_reconciliation.difference.toFixed(2)} - may require investigation`)
        }

        // Business rule validations
        if (cf.totals.net_cash_from_operating < 0 && cf.totals.net_cash_from_financing > 0) {
            warnings.push('Company is financing operations with debt/equity rather than operating cash flow')
        }

        if (cf.totals.net_cash_from_investing > 0 && Math.abs(cf.totals.net_cash_from_investing) > cf.totals.net_cash_from_operating) {
            warnings.push('Company is selling more assets than generating from operations - may not be sustainable')
        }
    ...
    private static async convertCFAmounts(
        ...
        try {
            ...
            const exchangeRate = await this.getExchangeRate(cfData.period.currency as CurrencyCode, targetCurrency, asOfDate)

            const convertLines = (lines: any[]) => lines.map(line => ({
                ...line,
            ...
            const convertedData: CashFlowData = {
                ...
                cash_reconciliation: {
                    ...
                    ending_cash_per_balance_sheet: cfData.cash_reconciliation.ending_cash_per_balance_sheet * exchangeRate,
    ...
    private static isOperatingCashFlow(entry: any): boolean {
        return entry.voucher_type?.includes('Sales') || entry.voucher_type?.includes('Purchase') || entry.remarks?.includes('operating')
    }

    private static isInvestingCashFlow(entry: any): boolean {
        return entry.voucher_type?.includes('Asset') || entry.remarks?.includes('investment') || entry.remarks?.includes('CAPEX')
    }

    private static isFinancingCashFlow(entry: any): boolean {
        return entry.voucher_type?.includes('Loan') || entry.remarks?.includes('dividend') || entry.remarks?.includes('equity')
    }

    private static buildDirectOperatingSection(receipts: any[], payments: any[]) {
        const lines = [
            { description: 'Cash Receipts from Customers', amount: sum(receipts.map(r => r.credit)), is_subtotal: false, indent: 0 },
            { description: 'Cash Payments to Suppliers', amount: -sum(payments.map(p => p.debit)), is_subtotal: false, indent: 0 },
        ]
        const subtotal = sum(lines.map(l => l.amount))
        return { title: 'Cash Flows from Operating Activities', lines, subtotal }
    }
    ...
    private static buildDirectInvestingSection(receipts: any[], payments: any[]) {
        const lines = [
            { description: 'Proceeds from Asset Sales', amount: sum(receipts.map(r => r.credit)), is_subtotal: false, indent: 0 },
            { description: 'Purchase of Assets', amount: -sum(payments.map(p => p.debit)), is_subtotal: false, indent: 0 },
        ]
        const subtotal = sum(lines.map(l => l.amount))
        return { title: 'Cash Flows from Investing Activities', lines, subtotal }
    }
    ...
    private static buildDirectFinancingSection(receipts: any[], payments: any[]) {
        const lines = [
            { description: 'Proceeds from Financing', amount: sum(receipts.map(r => r.credit)), is_subtotal: false, indent: 0 },
            { description: 'Financing Payments', amount: -sum(payments.map(p => p.debit)), is_subtotal: false, indent: 0 },
        ]
        const subtotal = sum(lines.map(l => l.amount))
        return { title: 'Cash Flows from Financing Activities', lines, subtotal }
    }
    ...
    private static generateCSV(cf: CashFlowData & { analysis?: CashFlowAnalysis }): string {
        ...
        const writeSection = (title: string, sec: any) => {
            ...
            for (const line of sec.lines) {
                ...
                lines.push([title, `${indent}${line.description}`, line.amount.toFixed(2), ''].map(q).join(','))
            }
        ...
        lines.push(['', 'Beginning Cash', cf.cash_reconciliation.beginning_cash.toFixed(2), ''].map(q).join(','))
        lines.push(['', 'Net Change in Cash', cf.cash_reconciliation.net_change_in_cash.toFixed(2), ''].map(q).join(','))
        lines.push(['', 'Ending Cash (Calculated)', cf.cash_reconciliation.ending_cash.toFixed(2), ''].map(q).join(','))
        lines.push(['', 'Ending Cash (Balance Sheet)', cf.cash_reconciliation.ending_cash_per_balance_sheet.toFixed(2), ''].map(q).join(','))
        lines.push(['', 'Difference', cf.cash_reconciliation.difference.toFixed(2), cf.cash_reconciliation.difference > EPS ? 'Investigate' : 'OK'].map(q).join(','))

        // Add analysis section
        if (cf.analysis) {
            lines.push('')
            lines.push(q('FINANCIAL ANALYSIS'))
            lines.push(['', 'Free Cash Flow', cf.analysis.free_cash_flow.toFixed(2), ''].map(q).join(','))
            lines.push(['', 'Operating Cash Flow Margin', cf.analysis.operating_cash_flow_margin.toFixed(2) + '%', ''].map(q).join(','))
            lines.push(['', 'Cash Flow Adequacy Ratio', cf.analysis.cash_flow_adequacy_ratio.toFixed(2), ''].map(q).join(','))
            lines.push(['', 'Quality of Earnings Ratio', cf.analysis.quality_of_earnings_ratio.toFixed(2), ''].map(q).join(','))
    ...
    private static async generateExcel(cf: CashFlowData & { analysis?: CashFlowAnalysis }): Promise<ArrayBuffer> {
    ...
    private static async generatePDF(cf: CashFlowData & { analysis?: CashFlowAnalysis }): Promise<ArrayBuffer> {
