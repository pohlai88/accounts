...
export class EnhancedProfitLossService {
  ...
  static async getProfitLoss(filters: PLFilters): Promise<{
    ...
    try {
      ...
      const useRows = (filters.includeZeroLines ? rows : rows.filter(r => Math.abs(toNum(r.amount)) > EPS))
        .sort((a, b) => {
      ...
      const indirectIncome = useRows.filter(r => r.pl_subcategory === 'Indirect Income').map(mapRow)
      ...
      const indirectExp = useRows.filter(r => r.pl_subcategory === 'Indirect Expenses' || r.pl_subcategory === 'Depreciation').map(mapRow)
        ...
        if (prev.success && prev.data) {
          ...
          directIncome.forEach(currentAccount => {
            ...
            if (previousAccount) {
              ...
              currentAccount.variance_percent = pct(currentAccount.variance, previousAccount.amount)
            }
          })
  ...
  static async getMonthlyTrend(
    ...
    success: boolean
    data?: {
      months: { month: string; total_income: number; total_expenses: number; net_profit: number; gross_profit_margin: number; net_profit_margin: number }[]
      totals: { total_income: number; total_expenses: number; net_profit: number; average_monthly_profit: number }
    }
    error?: string
  }> {
    try {
      ...
      const months: {
        month: string; total_income: number; total_expenses: number; net_profit: number; gross_profit_margin: number; net_profit_margin: number
      }[] = []
  ...
  static validateProfitLoss(pl: PLData): PLValidationResult {
    ...
    if (Math.abs(pl.income.total - incomeCalc) > EPS) {
      errors.push(`Income total mismatch: expected ${incomeCalc.toFixed(2)}, got ${pl.income.total.toFixed(2)}`)
    }

    const expensesCalc = add(
      add(pl.expenses.cost_of_goods_sold.reduce((s, r) => add(s, r.amount), 0),
    ...
    if (Math.abs(pl.expenses.total - expensesCalc) > EPS) {
      errors.push(`Expense total mismatch: expected ${expensesCalc.toFixed(2)}, got ${pl.expenses.total.toFixed(2)}`)
    }

    const netCalc = add(pl.totals.total_income, -pl.totals.total_expenses)
    if (Math.abs(pl.totals.net_profit - netCalc) > EPS) {
      errors.push(`Net profit mismatch: expected ${netCalc.toFixed(2)}, got ${pl.totals.net_profit.toFixed(2)}`)
    }
  ...
  private static async convertPLAmounts(
    ...
    try {
      ...
      const exchangeRate = await this.getExchangeRate(plData.period.currency as CurrencyCode, targetCurrency, asOfDate)
  ...
  private static async getBudgetData(
    ...
    const emptyBudget: PLData = {
      ...
      totals: { total_income: 0, total_expenses: 0, gross_profit: 0, net_profit: 0, gross_profit_margin: 0, net_profit_margin: 0 },
      comparative: {},
      metadata: { generated_at: new Date().toISOString(), period_closed: false, base_currency: currency, presentation_currency: currency }
    }
  ...
  private static generateCSV(pl: PLData): string {
    ...
    lines.push('')
    lines.push(['Account Code', 'Account Name', 'Type', 'Amount', '% of Revenue', '% of Section'].map(q).join(','))
    ...
    lines.push([
      '', 'Total Income', '', pl.income.total.toFixed(2), '100.00%', ''
    ].map(q).join(','))
    lines.push('')

    section('Cost of Goods Sold', pl.expenses.cost_of_goods_sold)
    lines.push([
      '', 'Gross Profit', '', pl.totals.gross_profit.toFixed(2), pl.totals.gross_profit_margin.toFixed(2) + '%', ''
    ].map(q).join(','))
    lines.push('')
    ...
    lines.push([
      '', 'Net Profit', '', pl.totals.net_profit.toFixed(2), pl.totals.net_profit_margin.toFixed(2) + '%', ''
    ].map(q).join(','))
