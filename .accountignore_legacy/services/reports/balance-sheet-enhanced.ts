...
export class EnhancedBalanceSheetService {
    ...
    try {
      ...
      if (filters.includeComparative && filters.comparativeDate) {
        ...
        if (prev.success && prev.data) {
          ...
          const collect = (arr: any[]) =>
            arr.forEach(x => comparativeMap!.set(x.account_id, x.amount));
      ...
      const mapRow = (r: FBSRow) => {
        ...
        const variance_percent =
          prev && prev !== 0 ? (variance! / Math.abs(prev)) * 100 : undefined;
      ...
      const assetsCurrent = rows
        .filter(r => r.bs_category === "Assets" && r.bs_subcategory === "Current Assets")
        .map(mapRow);
      const assetsFixed = rows
        .filter(r => r.bs_category === "Assets" && r.bs_subcategory === "Fixed Assets")
        .map(mapRow);
      const assetsNonCurrent = rows
        .filter(r => r.bs_category === "Assets" && r.bs_subcategory === "Non-Current Assets")
        .map(mapRow);
      const liabilitiesCurrent = rows
        .filter(r => r.bs_category === "Liabilities" && r.bs_subcategory === "Current Liabilities")
        .map(mapRow);
      const liabilitiesNonCurr = rows
        .filter(
          r => r.bs_category === "Liabilities" && r.bs_subcategory === "Non-Current Liabilities",
        )
        .map(mapRow);
      ...
      const otherEquity = equityAll.filter(
        e => !/share|capital|retained\s*earnings/i.test(e.account_name),
      );

      // 8) Totals & section percentages
      const sectionTotal = (arr: any[]) => sum(arr.map(x => x.amount));
      const totalAssets = sum([
        sectionTotal(assetsCurrent),
        sectionTotal(assetsFixed),
        sectionTotal(assetsNonCurrent),
      ]);
      ...
      const fillPct = (arr: any[], base: number) =>
        arr.forEach(x => (x.percentage_of_total = safePct(x.amount, base)));
      ...
      const ratios = {
        current_ratio:
          currentLiabilitiesAmt !== 0 ? currentAssetsAmt / currentLiabilitiesAmt : undefined,
        quick_ratio:
          currentLiabilitiesAmt !== 0
            ? add(currentAssetsAmt, -inventoryAmt) / currentLiabilitiesAmt
            : undefined,
      ...
      const financialHealth: FinancialHealthIndicators = {
        ...
        working_capital_ratio:
          currentAssetsAmt !== 0 ? workingCapital / currentAssetsAmt : undefined,
        asset_coverage_ratio:
          currentLiabilitiesAmt !== 0
            ? add(totalAssets, -inventoryAmt) / currentLiabilitiesAmt
            : undefined,
  ...
  static validateBalanceSheet(bs: BSData): BSValidationResult {
    ...
    if (Math.abs(bs.assets.total - assetsCalc) > EPS) {
      errors.push(
        `Asset total mismatch: expected ${assetsCalc.toFixed(2)}, got ${bs.assets.total.toFixed(2)}`,
      );
    }
    ...
    if (Math.abs(bs.liabilities.total - liabCalc) > EPS) {
      errors.push(
        `Liability total mismatch: expected ${liabCalc.toFixed(2)}, got ${bs.liabilities.total.toFixed(2)}`,
      );
    }

    const equityCalc = [
      ...bs.equity.share_capital,
      ...bs.equity.retained_earnings,
      ...bs.equity.other_equity,
    ].reduce((s, r) => add(s, r.amount), 0);
    if (Math.abs(bs.equity.total - equityCalc) > EPS) {
      errors.push(
        `Equity total mismatch: expected ${equityCalc.toFixed(2)}, got ${bs.equity.total.toFixed(2)}`,
      );
    }

    const balanced =
      Math.abs(bs.totals.total_assets - add(bs.totals.total_liabilities, bs.totals.total_equity)) <
      EPS;
    if (!balanced) {
      errors.push(
        `Not balanced: Assets ${bs.totals.total_assets.toFixed(2)} ≠ Liabilities+Equity ${add(bs.totals.total_liabilities, bs.totals.total_equity).toFixed(2)}`,
      );
    }
    ...
    if (bs.ratios.current_ratio && bs.ratios.current_ratio < 1) {
      warnings.push(
        `Low current ratio: ${bs.ratios.current_ratio.toFixed(2)} (may indicate liquidity issues)`,
      );
    }

    if (bs.ratios.debt_to_equity && bs.ratios.debt_to_equity > 2) {
      warnings.push(
        `High debt-to-equity ratio: ${bs.ratios.debt_to_equity.toFixed(2)} (may indicate high financial risk)`,
      );
    }

    if (bs.totals.total_equity < 0) {
      warnings.push(
        `Negative equity: ${bs.totals.total_equity.toFixed(2)} (company may be insolvent)`,
      );
    }
  ...
  private static async convertBSAmounts(
    ...
    try {
      ...
      const exchangeRate = await this.getExchangeRate(
        bsData.period.currency as CurrencyCode,
        targetCurrency,
        asOfDate,
      );
  ...
  private static calculateLeverageScore(
    debtToEquity?: number,
    debtToAssets?: number,
  ...
  ): "Strong" | "Healthy" | "Concerning" | "Critical" {
    const scores = {
      Excellent: 4,
      Good: 3,
      Fair: 2,
  ...
  private static generateCSV(
    bs: BSData & { financial_health?: FinancialHealthIndicators },
  ): string {
    const lines: string[] = [];
    ...
    const section = (title: string, rows: any[]) => {
      ...
      lines.push(
        [
          ...
          "",
          base.toFixed(2),
          safePct(base, bs.assets.total).toFixed(2) + "%",
          "",
          "",
        ]
          .map(q)
    ...
    lines.push(
      ["", "", "Total Assets", "", bs.totals.total_assets.toFixed(2), "100.00%", "", ""]
        .map(q)
        .join(","),
    );
    lines.push("");
    ...
    lines.push(
      [
        ...
        "Total Liabilities",
        "",
        bs.liabilities.total.toFixed(2),
        safePct(bs.liabilities.total, bs.assets.total).toFixed(2) + "%",
        "",
        "",
      ]
        .map(q)
    ...
    lines.push(
      [
        ...
        "Total Equity",
        "",
        bs.equity.total.toFixed(2),
        safePct(bs.equity.total, bs.assets.total).toFixed(2) + "%",
        "",
        "",
      ]
        .map(q)
    ...
    lines.push(
      [
        ...
        "Liabilities + Equity",
        "",
        bs.totals.total_liabilities_and_equity.toFixed(2),
        safePct(bs.totals.total_liabilities_and_equity, bs.assets.total).toFixed(2) + "%",
        "",
        "",
      ]
        .map(q)
    ...
    if (bs.financial_health) {
      ...
      if (fh.current_ratio)
        lines.push(
          ["", "Current Ratio", "", "", fh.current_ratio.toFixed(2), "", "", ""].map(q).join(","),
        );
      if (fh.quick_ratio)
        lines.push(
          ["", "Quick Ratio", "", "", fh.quick_ratio.toFixed(2), "", "", ""].map(q).join(","),
        );
      if (fh.debt_to_equity)
        lines.push(
          ["", "Debt to Equity", "", "", fh.debt_to_equity.toFixed(2), "", "", ""].map(q).join(","),
        );
      if (fh.debt_to_assets)
        lines.push(
          ["", "Debt to Assets", "", "", fh.debt_to_assets.toFixed(2), "", "", ""].map(q).join(","),
        );
      lines.push(
        ["", "Working Capital", "", "", fh.working_capital.toFixed(2), "", "", ""].map(q).join(","),
      );
