...
export function PaymentEntryForm({
  ...
  const getAvailableInvoices = () => {
    return invoices.filter(
      invoice => invoice.party_name === watchedPartyType || invoice.outstanding_amount > 0,
    );
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <Input id="party_name" {...register("party_name")} placeholder="Enter party name" />
                {errors.party_name && (
                          ...
                          <TableRow key={field.id}>
                            <TableCell>
                              <Select
                                ...
                                onValueChange={value =>
                                  setValue(`allocations.${index}.invoice_id`, value)
                                }
                              >
                                <SelectTrigger>
                                ...
                                <SelectContent>
                                  {getAvailableInvoices().map(invoice => (
                                    <SelectItem key={invoice.id} value={invoice.id}>
                                      {invoice.invoice_no} - {invoice.party_name} (
                            ...
                            <TableCell>
                              <Input
                                type="number"
                                step="0.01"
                                {...register(`allocations.${index}.discount_amount`, {
                                  valueAsNumber: true,
                                })}
                                placeholder="0.00"
                              />
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-lg font-bold ${unallocatedAmount < 0 ? "text-red-500" : "text-primary"}`}
                    >
