...
import { OutstandingManagementService, OutstandingInvoice, PaymentAllocationResult } from '@/lib/outstanding-management'
...
export function EnhancedPaymentEntryForm({
    ...
    const loadOutstandingInvoices = async () => {
        ...
        try {
            ...
            const outstanding = watchedPartyType === 'Customer'
                ? await OutstandingManagementService.getCustomerOutstandingInvoices(party.id, currentCompany.id)
                : await OutstandingManagementService.getSupplierOutstandingInvoices(party.id, currentCompany.id)
    ...
    const handleAllocationChange = (invoiceId: string, allocatedAmount: number) => {
        setPaymentAllocations(prev => prev.map(allocation =>
    ...
    const handleSubmitPayment = async () => {
        ...
        try {
            ...
            const result = await DocumentWorkflowEngine.submitPaymentEntry(
                payment.id,
                {
                    userId: user.id,
                ...
                {outstandingInvoices.length > 0 && (
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Allocate payment against outstanding invoices
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    {paymentAllocations.map((allocation) => (
                                        <TableRow key={allocation.invoice_id}>
                                            ...
                                            <TableCell>{format(new Date(allocation.invoice_date), 'MMM dd, yyyy')}</TableCell>
                                            ...
                                            <TableCell>
                                                <Input
                                                    ...
                                                    onChange={(e) =>
                                                        handleAllocationChange(allocation.invoice_id, parseFloat(e.target.value) || 0)
                                                    }
                            ...
                            <div className="mt-4 p-4 bg-muted rounded-lg">
                                <div className="grid grid-cols-3 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <div className={`text-lg font-bold ${getUnallocatedAmount() < 0 ? 'text-red-600' : 'text-green-600'}`}>
            ...
            <Dialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to submit this payment? This will create GL entries and update outstanding amounts.
                        </DialogDescription>
                    </DialogHeader>
