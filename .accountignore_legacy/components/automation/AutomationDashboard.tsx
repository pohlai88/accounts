...
export default function AutomationDashboard() {
    ...
    const handleUpdateRecommendationStatus = async (
        ...
        const result = await AutomationService.updateRecommendationStatus(recommendationId, status, 'current-user-id', notes)
        if (result.success) {
            loadDashboardData() // Refresh data
        }
    }
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground">
                                {analytics.total_rules} total rules
                            </p>
                        </CardContent>
                    </Card>
                                    ...
                                    <tbody>
                                        {automationRules.map((rule) => (
                                            <tr key={rule.id} className="border-b hover:bg-muted/50">
                                                ...
                                                <td className="p-3">
                                                    <div className="text-sm">
                                                        <div className={`font-medium ${(rule.success_count / Math.max(rule.execution_count, 1)) > 0.9
                                                                ? 'text-green-600'
                                                                : 'text-orange-600'
                                                            }`}>
                                                ...
                                                <td className="p-3">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {rule.is_active && <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />}
                                                    </div>
                                                </td>
                                                <td className="p-3">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {aiModels.map((model) => (
                                    <div key={model.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                <span>Avg Time:</span>
                                                <span className="font-medium">{formatTime(model.average_prediction_time_ms)}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-start justify-between mb-3">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <Badge variant={getStatusBadgeVariant(recommendation.priority)} className={getPriorityColor(recommendation.priority)}>
                                                ...
                                                {recommendation.estimated_impact && Object.keys(recommendation.estimated_impact).length > 0 && (
                                                    <div className="text-xs text-muted-foreground mb-3">
                                                        <strong>Estimated Impact:</strong> {JSON.stringify(recommendation.estimated_impact)}
                                                    </div>
                                                )}
                                                    ...
                                                    <div className="space-y-1">
                                                        ...
                                                        {recommendation.suggested_actions.slice(0, 2).map((action, index) => (
                                                            <div key={index} className="text-xs text-muted-foreground flex items-center gap-1">
                                                                ...
                                                                {typeof action === 'string' ? action : action.action || action.description || 'Action'}
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}
                                                ...
                                                <div className="flex gap-1">
                                                    {recommendation.status === 'New' && (
                                                        <>
                                                            <Button
                                                                size="sm"
                                                                variant="outline"
                                                                onClick={() => handleUpdateRecommendationStatus(recommendation.id, 'Implemented')}
                                                            >
                                                                Implement
                                                            </Button>
                                                            <Button
                                                                size="sm"
                                                                variant="ghost"
                                                                onClick={() => handleUpdateRecommendationStatus(recommendation.id, 'Dismissed', 'Not relevant')}
                                                            >
                                                                Dismiss
                                                            </Button>
                                                        </>
                                                    ...
                                                    {recommendation.status === 'Viewed' && (
                                                        <Button
                                                            size="sm"
                                                            variant="outline"
                                                            onClick={() => handleUpdateRecommendationStatus(recommendation.id, 'In Progress')}
                                                        >
                                                            Start Work
                                                        </Button>
                                                    )}
                                    ...
                                    <tbody>
                                        {anomalies.map((anomaly) => (
                                            <tr key={anomaly.id} className="border-b hover:bg-muted/50">
                                                ...
                                                <td className="p-3">
                                                    <div className="text-sm">
                                                        <div className={`font-medium ${anomaly.anomaly_score > 0.8 ? 'text-red-600' :
                                                                anomaly.anomaly_score > 0.6 ? 'text-orange-600' :
                                                                    anomaly.anomaly_score > 0.4 ? 'text-yellow-600' : 'text-green-600'
                                                            }`}>
