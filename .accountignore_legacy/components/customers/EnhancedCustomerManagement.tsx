                ...
                const customersWithOutstanding = await Promise.all(
                    result.data.map(async (customer) => {
                        try {
                            ...
                            const totalOutstanding = outstanding.reduce((sum, invoice) => sum + invoice.outstanding_amount, 0)
                    ...
                    <form onSubmit={handleSubmit(handleCreateCustomer)} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Select onValueChange={(value: 'Company' | 'Individual') => setValue('customer_type', value)}>
                                    <SelectTrigger>
                                ...
                                <div className="mt-2 grid grid-cols-2 gap-4">
                                    <div className="p-3 bg-muted rounded-lg">
                                        ...
                                        <div className="text-lg font-bold">
                                            {((selectedCustomer as CustomerWithOutstanding).outstanding_amount || 0).toFixed(2)}
                                        </div>
                                    </div>
                                    <div className="p-3 bg-muted rounded-lg">
                                        <div className="text-sm text-muted-foreground">Overdue Amount</div>
                                        <div className="text-lg font-bold text-red-600">
                                            {((selectedCustomer as CustomerWithOutstanding).overdue_amount || 0).toFixed(2)}
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <DialogContent>
                    ...
                    {customerToDelete && (
                        <Alert variant="destructive">
                            ...
                            <AlertDescription>
                                You are about to delete <strong>{customerToDelete.customer_name}</strong>.
