  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [
        ...
        BankService.getBankTransactions(
          companyId,
          {
              ...
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <select
                    ...
                    onChange={e =>
                      setSelectedStatus(e.target.value as ReconciliationStatus | "All")
                    }
                    ...
                    {transactions?.transactions.map(transaction => (
                      <tr key={transaction.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            <Badge
                              variant={getStatusBadgeVariant(transaction.reconciliation_status)}
                            >
                  ...
                  <tbody>
                    {sessions.slice(0, 5).map(session => (
                      <tr key={session.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-3">
                          <div className="text-sm">
                            {new Date(session.date_from).toLocaleDateString()} -{" "}
                            {new Date(session.date_to).toLocaleDateString()}
                          </div>
                        </td>
                        <td className="p-3">
                        ...
                        <td className="p-3">
                          <div className="text-sm">
                            <span className="font-medium">{session.matched_transactions}</span> /{" "}
                            {session.total_bank_transactions}
                          </div>
                          <div className="text-xs text-muted-foreground">
