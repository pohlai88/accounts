...
export default function ConsolidatedReportingDashboard({ consolidationGroupId }: ConsolidatedReportingDashboardProps) {
    const [dashboardData, setDashboardData] = useState<ConsolidationDashboard | null>(null)
    const [consolidationAnalysis, setConsolidationAnalysis] = useState<ConsolidationAnalysis | null>(null)
    ...
    const getEliminationTypeIcon = (type: EliminationType) => {
        switch (type) {
            case 'Intercompany Sales': return <ArrowRightLeft className="w-4 h-4 text-blue-600" />
            case 'Intercompany Receivables': return <DollarSign className="w-4 h-4 text-green-600" />
            case 'Intercompany Payables': return <Minus className="w-4 h-4 text-red-600" />
            case 'Investment Elimination': return <Building2 className="w-4 h-4 text-purple-600" />
                ...
                <div className="flex gap-2">
                    <Select value={selectedPeriod} onValueChange={setSelectedPeriod}>
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value={new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}>Last Quarter</SelectItem>
                        </SelectContent>
                    </Select>
                    <Button
                        variant="outline"
                    ...
                    <Dialog open={showNewRunDialog} onOpenChange={setShowNewRunDialog}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    This will process the consolidation for {selectedPeriod} including intercompany eliminations and currency translation.
                                </DialogDescription>
                            </DialogHeader>
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <div className="text-sm text-muted-foreground">
                                    Started: {dashboardData.latest_run.started_at ? new Date(dashboardData.latest_run.started_at).toLocaleString() : 'Not started'}
                                </div>
                            </div>
                            <Badge className={getRunStatusColor(dashboardData.latest_run.run_status)} variant="outline">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-4 gap-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <div className="font-medium">{formatCurrency(dashboardData.latest_run.total_amount_consolidated)}</div>
                                </div>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <strong className="text-amber-800 dark:text-amber-200">
                                    {dashboardData.validation_issues.length} Validation Issue{dashboardData.validation_issues.length > 1 ? 's' : ''}
                                </strong>
                                <div className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                                    {dashboardData.validation_issues[0].description}
                                    {dashboardData.validation_issues.length > 1 && ` and ${dashboardData.validation_issues.length - 1} more`}
                                </div>
                            </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Progress value={dashboardData.key_metrics.processing_efficiency} className="w-20 h-2" />
                                    ...
                                    <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                                        <div>
                                            ...
                                            <div className="text-xs text-muted-foreground">
                                                {new Date(statement.reporting_period).toLocaleDateString()} - {statement.statement_version}
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                ...
                                <CardContent className="p-4">
                                    ...
                                    <div className="space-y-2 text-sm">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-medium">{formatPercentage(entity.ownership_percentage)}</span>
                                        </div>
                                        <div className="flex justify-between">
                        ...
                        <CardContent className="p-0">
                            <div className="border rounded-lg overflow-hidden">
                                ...
                                {trialBalance.slice(0, 20).map((item, index) => (
                                    <div key={index} className="grid grid-cols-7 gap-4 p-3 border-t text-sm hover:bg-muted/50">
                                        <div>
                                            <div className="font-medium">{item.account_name}</div>
                                            <div className="text-xs text-muted-foreground">{item.account_code}</div>
                                        </div>
                                        <div className="text-right">
                                            {item.pre_elimination_debit > 0 ? formatCurrency(item.pre_elimination_debit) : '-'}
                                        </div>
                                        <div className="text-right">
                                            {item.pre_elimination_credit > 0 ? formatCurrency(item.pre_elimination_credit) : '-'}
                                        </div>
                                        <div className="text-right">
                                            {(item.elimination_debit + item.elimination_credit) > 0 ?
                                                formatCurrency(item.elimination_debit + item.elimination_credit) : '-'}
                                        </div>
                                        <div className={`text-right font-medium ${item.consolidated_balance >= 0 ? 'text-green-600' : 'text-red-600'
                                            }`}>
                                ...
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1">
                                            ...
                                            <p className="text-sm text-muted-foreground mb-2">{elimination.description}</p>

                                            <div className="grid grid-cols-3 gap-4 text-sm">
                                                <div>
                                                    <span className="text-muted-foreground">Amount: </span>
                                                    <span className="font-medium">{formatCurrency(elimination.elimination_amount)}</span>
                                                </div>
                                                <div>
                                        ...
                                        <div className="flex items-center gap-2 ml-4">
                                            <Badge className={`${elimination.approval_status === 'Approved' ? 'bg-green-50 text-green-600' :
                                                    elimination.approval_status === 'Pending' ? 'bg-yellow-50 text-yellow-600' :
                ...
                <TabsContent value="statements" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                        {['Balance Sheet', 'Income Statement', 'Cash Flow Statement'].map((statementType) => {
                            const statement = dashboardData.financial_statements.find(s => s.statement_type === statementType)

                            return (
                                <Card key={statementType} className={statement ? 'border-green-200' : 'border-dashed'}>
                                    <CardContent className="p-4">
                                        <div className="flex items-start justify-between mb-3">
                                            <div>
                                        ...
                                        {statement && (
                                            <div className="space-y-2 text-sm">
                                                ...
                                                <div className="flex justify-between">
                                                    ...
                                                    <Badge variant={statement.published ? 'default' : 'secondary'} className="text-xs">
                                                        {statement.review_status}
                                                    </Badge>
                                                </div>
                                            </div>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-center">
                                    ...
                                    <Progress value={dashboardData.consolidation_summary.data_completeness * 100} className="mb-4" />
                                    <Badge variant={
                                        dashboardData.consolidation_summary.data_completeness >= 0.95 ? 'default' :
                                            dashboardData.consolidation_summary.data_completeness >= 0.85 ? 'secondary' :
                                                'outline'
                                    }>
                                        {dashboardData.consolidation_summary.data_completeness >= 0.95 ? 'Excellent' :
                                            dashboardData.consolidation_summary.data_completeness >= 0.85 ? 'Good' :
                                                'Needs Improvement'}
                                    </Badge>
                                </div>
                            </CardContent>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    ...
                                    <Progress value={dashboardData.key_metrics.processing_efficiency} className="h-2" />
                                        ...
                                        <div>
                                            ...
                                            <div className="font-medium">
                                                {dashboardData.latest_run?.processing_duration_seconds ?
                                                    `${dashboardData.latest_run.processing_duration_seconds}s` : 'N/A'}
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div className="font-medium">
                                                {dashboardData.latest_run?.error_count ?
                                                    `${dashboardData.latest_run.error_count} errors` : '0 errors'}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="space-y-2">
                                            {consolidationAnalysis.entity_contribution.slice(0, 5).map((entity, index) => (
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="space-y-2">
                                            {Object.entries(consolidationAnalysis.elimination_impact.by_type).map(([type, amount]) => (
