...
export function ReconciliationInterface({
    ...
    const [selectedBankTransactions, setSelectedBankTransactions] = React.useState<Set<string>>(new Set())
    ...
    const filteredBankTransactions = React.useMemo(() => {
        return bankTransactions.filter(transaction => {
            ...
            if (searchQuery && !transaction.description.toLowerCase().includes(searchQuery.toLowerCase()) &&
                !transaction.reference?.toLowerCase().includes(searchQuery.toLowerCase())) return false
    ...
    const filteredGLEntries = React.useMemo(() => {
        return glEntries.filter(entry => {
            ...
            if (searchQuery && !entry.description.toLowerCase().includes(searchQuery.toLowerCase()) &&
                ...
                !entry.account_name.toLowerCase().includes(searchQuery.toLowerCase())) return false
    ...
    const stats = React.useMemo(() => {
        ...
        const matchedBankAmount = matchedBankTransactions.reduce((sum, t) => sum + Math.abs(t.amount), 0)
        ...
        try {
            ...
            for (const bankTransaction of unmatchedBankTransactions) {
                ...
                const potentialMatches = unmatchedGLEntries.filter(entry => {
                    const amountMatch = Math.abs(Math.abs(bankTransaction.amount) - Math.abs(entry.debit || entry.credit)) < 0.01
                    const dateMatch = Math.abs(bankTransaction.date.getTime() - entry.posting_date.getTime()) < 7 * 24 * 60 * 60 * 1000 // 7 days
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                    <Badge variant="outline">
                        {stats.matchPercentage.toFixed(1)}% Matched
                    </Badge>
                    {session && (
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {stats.matchedBankTransactions} matched, {stats.unmatchedBankTransactions} unmatched
                                </p>
                            </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            ...
                            <Badge variant="outline">
                                {selectedBankTransactions.size} selected
                            </Badge>
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            ...
                            <Badge variant="outline">
                                {selectedGLEntries.size} selected
                            </Badge>
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
