...
export function AccountSelector({
    ...
    const filteredAccounts = React.useMemo(() => {
        ...
        return filtered
    }, [accounts, accountTypes, filterType, excludeInactive, onlyLeafAccounts, allowGroups, showOnlyFavorites, searchQuery])
    ...
    const getAccountIcon = (account: Account) => {
        switch (account.type) {
            case 'Asset': return <Building2 className="h-4 w-4 text-blue-500" />
        ...
        return accountList.map(account => (
            <div key={account.id}>
                <div
                    ...
                    onClick={() => account.is_group ? toggleGroup(account.id) : handleAccountSelect(account.id)}
                >
                ...
                {account.is_group && expandedGroups.has(account.id) && account.children && (
                    <div className="ml-4">
                        {renderAccountTree(account.children)}
                    </div>
                )}
            </div>
        ))
