...
export function CostCenterSelector({
                ...
                <div key={costCenter.id} className={`ml-${level * 4}`}>
                    <div className="flex items-center py-2 px-3 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-md cursor-pointer">
                        ...
                        <div
                            ...
                            onClick={() => !costCenter.isGroup && onCostCenterChange(costCenter.id, costCenter.costCenterName)}
                        >
                            <Building2 className="h-4 w-4 mr-2 text-blue-500" />
                            <span className={`${costCenter.isGroup ? 'font-semibold' : ''} ${selectedCostCenter === costCenter.id ? 'text-blue-600 font-medium' : ''}`}>
                    ...
                    {costCenter.isGroup && expandedNodes.has(costCenter.id) && costCenter.children && (
                        <div className="ml-4">
                            {renderCostCenterTree(costCenter.children, level + 1)}
                        </div>
                    )}
                </div>
            ))
    ...
    const selectedCostCenterName = costCenters
        .flatMap(cc => [cc, ...(cc.children || [])])
        .find(cc => cc.id === selectedCostCenter)?.costCenterName || ''
                            ...
                            <div className="space-y-4">
                                                ...
                                                <TableCell>
                                                    <Select
                                                        ...
                                                        onValueChange={(value) => {
                                                            ...
                                                            updateAllocation(index, 'costCenterName', costCenter?.costCenterName || '')
                                                        }}
                                                    >
                                                        <SelectTrigger>
                                                        ...
                                                        <SelectContent>
                                                            {costCenters.flatMap(cc => [cc, ...(cc.children || [])])
                                ...
                                {tempAllocations.length > 0 && (
                                    <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                                        ...
                                        <span className={`font-bold ${Math.abs(tempAllocations.reduce((sum, alloc) => sum + alloc.percentage, 0) - 100) < 0.01
                                                ? 'text-green-600'
                                                : 'text-red-600'
                                            }`}>
                                ...
                                <div className="flex justify-end space-x-2">
                                    ...
                                    <Button onClick={handleAllocationSave}>
                                        Save Allocations
                                    </Button>
                                </div>
                            </div>
                ...
                <Dialog>
                    ...
                    <DialogContent className="max-w-md">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Choose a cost center for this transaction
                            </DialogDescription>
                        </DialogHeader>
