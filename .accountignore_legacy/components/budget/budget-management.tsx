        ...
        <div className="space-y-6">
                    ...
                    <Dialog open={showCreateBudgetDialog} onOpenChange={setShowCreateBudgetDialog}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Set up a new budget for tracking and analysis
                                </DialogDescription>
                            </DialogHeader>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => setBudgetForm(prev => ({ ...prev, budgetPeriodId: value }))}
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => setBudgetForm(prev => ({ ...prev, budgetType: value as any }))}
                                        >
                                            <SelectTrigger>
                                                <SelectValue />
                                            </SelectTrigger>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Select a budget to view detailed analysis
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {filteredBudgets.length > 0 ? (
                        <div className="space-y-4">
                            {filteredBudgets.map((budget) => (
                                <Card
                                    key={budget.id}
                                    className={`cursor-pointer transition-colors ${selectedBudget?.id === budget.id ? 'ring-2 ring-blue-500' : 'hover:bg-muted/50'
                                        }`}
                                        ...
                                        <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex items-center space-x-3 mb-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-sm text-muted-foreground">
                                                            {budget.budgetType} • {budgetPeriods.find(p => p.id === budget.budgetPeriodId)?.periodName}
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <div className="flex items-center space-x-1">
                                                    ...
                                                    <span className={`text-sm font-medium ${getVarianceColor(budget.totalVarianceAmount)}`}>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    ...
                                    <div className="text-center">
                                        <div className={`text-2xl font-bold ${getVarianceColor(selectedBudget.totalVarianceAmount)}`}>
                                            {formatCurrency(selectedBudget.totalVarianceAmount)}
                                        </div>
                                        <div className="text-sm text-muted-foreground">Variance</div>
                                    </div>
                                    <div className="text-center">
                                        <div className={`text-2xl font-bold ${getVarianceColor(selectedBudget.totalVariancePercentage)}`}>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="font-medium">
                                                {((budgetVariance.totalActual / budgetVariance.totalBudget) * 100).toFixed(1)}%
                                            </span>
                                        </div>
                                        <Progress
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            ...
                                            <div>
                                                ...
                                                <div className={`font-medium ${getVarianceColor(budgetVariance.totalVariance)}`}>
                                                    {budgetVariance.totalVariance > 0 ? 'Over Budget' :
                                                        budgetVariance.totalVariance < 0 ? 'Under Budget' : 'On Budget'}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <CardContent>
                                {budgetReport.length > 0 ? (
                                            ...
                                            <TableBody>
                                                {budgetReport.map((item, index) => (
                                                    <TableRow key={index}>
                                                        ...
                                                        <TableCell className={`text-right font-medium ${getVarianceColor(item.varianceAmount)}`}>
                                                            {formatCurrency(item.varianceAmount)}
                                                        </TableCell>
                                                        <TableCell className={`text-right font-medium ${getVarianceColor(item.variancePercentage)}`}>
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-muted-foreground">
                                            Add budget items to generate the report
                                        </p>
                                    </div>
                                )}
                            </CardContent>
                                            ...
                                            <TableBody>
                                                {budgetItems.map((item) => (
                                                    <TableRow key={item.id}>
                                                        <TableCell>
                                                            {accounts.find(a => a.id === item.accountId)?.name || 'Unknown Account'}
                                                        </TableCell>
                                                        <TableCell className="text-right font-medium">
                                                            {formatCurrency(item.budgetAmount)}
                                                        </TableCell>
                    ...
                    <TabsContent value="variance" className="space-y-4">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Analyze budget variances and identify trends
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div className="text-center py-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(e) => setItemForm(prev => ({ ...prev, budgetAmount: parseFloat(e.target.value) }))}
                            />
                        </div>
                        <div>
