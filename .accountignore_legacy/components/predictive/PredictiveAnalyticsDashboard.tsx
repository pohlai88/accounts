...
export default function PredictiveAnalyticsDashboard({ companyId }: PredictiveAnalyticsDashboardProps) {
    ...
    const getModelStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'staging': return 'text-blue-600 bg-blue-50 border-blue-200'
            case 'testing': return 'text-yellow-600 bg-yellow-50 border-yellow-200'
            case 'development': return 'text-purple-600 bg-purple-50 border-purple-200'
            case 'deprecated': return 'text-gray-600 bg-gray-50 border-gray-200'
    ...
    const getAlgorithmIcon = (algorithm: string) => {
        switch (algorithm) {
            case 'Neural Network': return <BrainCircuit className="w-4 h-4 text-purple-600" />
            case 'Random Forest': return <Network className="w-4 h-4 text-green-600" />
            case 'XGBoost': return <Rocket className="w-4 h-4 text-blue-600" />
            case 'ARIMA': return <LineChart className="w-4 h-4 text-orange-600" />
            case 'Prophet': return <Sparkles className="w-4 h-4 text-pink-600" />
        ...
        <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <strong className="text-orange-800 dark:text-orange-200">
                                    {dashboardData.insight_summary.high_priority_insights} High Priority Insight{dashboardData.insight_summary.high_priority_insights > 1 ? 's' : ''}
                                </strong>
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <div className="text-sm font-medium text-muted-foreground mb-2">Today's Predictions</div>
                        ...
                        <div>
                            <div className="text-sm font-medium text-muted-foreground mb-2">Performance Trends</div>
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                ...
                <TabsContent value="overview" className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {Object.entries(dashboardData.model_summary.models_by_type).map(([type, count]) => (
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {Object.entries(dashboardData.model_summary.models_by_domain).map(([domain, count]) => (
                                        ...
                                        <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                                            <div className="flex-1">
                                                ...
                                                <div className="text-xs text-muted-foreground">
                                                    ...
                                                    Confidence: {formatPercentage(prediction.confidence_score || 0)}
                                                </div>
                                            </div>
                                    ...
                                    {dashboardData.recent_insights.slice(0, 5).map((insight, index) => (
                                        <div key={index} className="flex items-start gap-3 p-3 border rounded-lg">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Badge
                                                    variant="outline"
                                                    className={`text-xs ${insight.action_priority === 'Urgent' ? 'border-red-200 text-red-600' :
                                                            insight.action_priority === 'High' ? 'border-orange-200 text-orange-600' :
                                                                insight.action_priority === 'Medium' ? 'border-yellow-200 text-yellow-600' :
                                ...
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex-1">
                                            ...
                                            <Badge className={getModelStatusColor(model.deployment_status)} variant="outline">
                                ...
                                {dashboardData.recent_predictions.slice(0, 10).map((prediction, index) => (
                                    <div key={index} className="grid grid-cols-7 gap-4 p-3 border-t text-sm hover:bg-muted/50">
                                        <div>
                                            ...
                                            <div className="text-xs text-muted-foreground">{prediction.model?.algorithm_type}</div>
                                        </div>
                                        ...
                                        <div>
                                            {prediction.risk_level && (
                                                <Badge variant="outline" className={`text-xs ${prediction.risk_level === 'Critical' ? 'border-red-200 text-red-600' :
                                                        prediction.risk_level === 'High' ? 'border-orange-200 text-orange-600' :
                                                            prediction.risk_level === 'Medium' ? 'border-yellow-200 text-yellow-600' :
                                                                'border-green-200 text-green-600'
                                                    }`}>
                                                    {prediction.risk_level}
                                                </Badge>
                                ...
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex-1">
                                            ...
                                            <p className="text-sm text-muted-foreground mb-3">{insight.insight_description}</p>

                                            <div className="grid grid-cols-3 gap-4 text-sm mb-4">
                                                <div>
                                                    <span className="text-muted-foreground">Confidence: </span>
                                                    <span className="font-medium">{insight.confidence_level.toFixed(1)}%</span>
                                                </div>
                                                <div>
                                        ...
                                        <div className="flex items-center gap-2 ml-4">
                                            <Badge
                                                className={`${insight.action_priority === 'Urgent' ? 'bg-red-50 text-red-600 border-red-200' :
                                                        insight.action_priority === 'High' ? 'bg-orange-50 text-orange-600 border-orange-200' :
                                                            insight.action_priority === 'Medium' ? 'bg-yellow-50 text-yellow-600 border-yellow-200' :
            ...
            <Dialog open={showAnalysisDialog} onOpenChange={setShowAnalysisDialog}>
                <DialogContent className="max-w-6xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>
                            Predictive Analytics Analysis Report
                        </DialogTitle>
                        <DialogDescription>
                                            ...
                                            {analysisData.prediction_patterns.by_confidence.map((pattern, index) => (
                                                <div key={index} className="flex justify-between items-center text-sm">
                                                    ...
                                                    <div className="flex gap-4">
                                                        ...
                                                        <span className="text-green-600">{formatPercentage(pattern.avg_accuracy)} accurate</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
