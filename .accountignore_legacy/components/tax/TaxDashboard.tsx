            ...
            {analytics && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className={`text-2xl font-bold ${analytics.net_tax_liability >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            ...
                            <Shield className={`h-4 w-4 ${getComplianceColor(analytics.filing_compliance_rate)}`} />
                        </CardHeader>
                        <CardContent>
                            <div className={`text-2xl font-bold ${getComplianceColor(analytics.filing_compliance_rate)}`}>
                                            ...
                                            <tr key={authority.id} className="border-b hover:bg-muted/50">
                                                <td className="p-3">
                                                    <div>
                                                        ...
                                                        <div className="text-sm text-muted-foreground">{authority.authority_code}</div>
                                                    </div>
                                                </td>
                                                <td className="p-3">
                                                    <Badge variant="outline">
                                                        {authority.jurisdiction_type}
                                                    </Badge>
                                                </td>
                                                <td className="p-3">
                                                ...
                                                <td className="p-3">
                                                    <span className="text-sm">{authority.tax_registration_number || 'Not set'}</span>
                                                </td>
                                                <td className="p-3">
                                    ...
                                    <tbody>
                                        {filingPeriods.slice(0, 10).map((period) => (
                                            <tr key={period.id} className="border-b hover:bg-muted/50">
                                                ...
                                                <td className="p-3">
                                                    <span className="text-sm">{(period as any).authority?.authority_name}</span>
                                                </td>
                                                <td className="p-3">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-sm">{new Date(period.due_date).toLocaleDateString()}</span>
                                                        {new Date(period.due_date) < new Date() && period.filing_status === 'Open' && (
                                                            <AlertTriangle className="w-4 h-4 text-red-500" />
                                                        )}
                                                    </div>
                                                </td>
                                                <td className="p-3">
                                                    <span className="font-medium">{formatCurrency(period.net_tax_liability)}</span>
                                                </td>
                                                <td className="p-3">
                                                    <Badge variant={getStatusBadgeVariant(period.filing_status)}>
                                                        {period.filing_status}
                                        ...
                                        {analytics.tax_by_authority.map((authority) => (
                                            <div key={authority.name} className="flex items-center justify-between">
                                                ...
                                                <div className="text-right">
                                                    <div className="font-medium">{formatCurrency(authority.total_amount)}</div>
                                                    <div className="text-xs text-muted-foreground">{authority.transaction_count} transactions</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                        ...
                                        {analytics.tax_by_category.map((category) => (
                                            <div key={category.name} className="flex items-center justify-between">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <div className="text-xs text-muted-foreground">{category.transaction_count} transactions</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
