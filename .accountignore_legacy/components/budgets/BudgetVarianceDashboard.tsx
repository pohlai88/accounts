...
export default function BudgetVarianceDashboard({ budgetPlanId, companyId }: BudgetVarianceDashboardProps) {
    ...
    const handleDrillDown = async (level: AnalysisLevel, dimensionId?: string) => {
        try {
            const result = await BudgetVarianceService.getVarianceDrillDown(budgetPlanId, level, dimensionId)
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${dashboardData.summary.variance_percentage > 0 ?
                                dashboardData.summary.total_budget > 0 ? 'text-green-600' : 'text-red-600' :
                                dashboardData.summary.total_budget > 0 ? 'text-red-600' : 'text-green-600'
                            }`}>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <strong className="text-amber-800 dark:text-amber-200">
                                    {dashboardData.alerts.length} Active Budget Alert{dashboardData.alerts.length > 1 ? 's' : ''}
                                </strong>
                                <div className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                                    {dashboardData.alerts[0].alert_title}
                                    {dashboardData.alerts.length > 1 && ` and ${dashboardData.alerts.length - 1} more`}
                                </div>
                            </div>
            ...
            <Tabs defaultValue="overview" className="w-full">
                ...
                <TabsContent value="overview" className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {Object.entries(dashboardData.variance_breakdown.by_category).map(([category, data]) => (
                                        <div key={category} className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <Badge className={getSeverityColor(category as VarianceCategory)} variant="outline">
                                                    {category}
                                                </Badge>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {Object.entries(dashboardData.variance_breakdown.by_account_type).map(([accountType, data]) => (
                                        <div key={accountType} className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm font-medium">{accountType}</span>
                                                <span className={`text-sm font-medium ${data.variance >= 0 ?
                                                        data.budget > 0 ? 'text-green-600' : 'text-red-600' :
                                                        data.budget > 0 ? 'text-red-600' : 'text-green-600'
                                                    }`}>
                                                    {formatCurrency(data.variance)}
                                                </span>
                                            </div>
                                            <div className="text-xs text-muted-foreground">
                                                Budget: {formatCurrency(data.budget)} | Actual: {formatCurrency(data.actual)}
                                            </div>
                                            <Progress
                                                value={data.budget !== 0 ? Math.min(Math.abs((data.actual / data.budget) * 100), 200) : 0}
                                                className="h-2"
                                            />
                                        </div>
                                    ))}
                ...
                <TabsContent value="analysis" className="space-y-6">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <Select value={selectedLevel} onValueChange={(value) => setSelectedLevel(value as AnalysisLevel)}>
                        ...
                        <CardContent>
                            <div className="h-64 bg-gray-50 dark:bg-gray-800 rounded-lg flex items-center justify-center">
                                <div className="text-center text-muted-foreground">
                                    ...
                                    <p className="text-xs">Select analysis level and click Analyze to view detailed breakdown</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <Badge variant={
                                                    alert.alert_status === 'Open' ? 'destructive' :
                                                        alert.alert_status === 'Resolved' ? 'default' : 'secondary'
                                                }>
                                                    {alert.alert_status}
                                                </Badge>
                                            </div>
                ...
                <TabsContent value="trends" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-center">
                                    <div className={`text-2xl font-bold ${dashboardData.trends.variance_trend === 'Improving' ? 'text-green-600' :
                                            dashboardData.trends.variance_trend === 'Worsening' ? 'text-red-600' :
                                                'text-yellow-600'
                                        }`}>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-center">
                                    ...
                                    <Progress
                                        value={dashboardData.trends.forecast_accuracy * 100}
                                        className="mt-2"
                                    />
                                    <p className="text-sm text-muted-foreground mt-2">
            ...
            <Dialog open={showDrillDown} onOpenChange={setShowDrillDown}>
                <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>
                            Variance Drill-down: {drillDownData?.dimension_name}
                        </DialogTitle>
                        <DialogDescription>
                            Detailed analysis with root cause identification
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-4 text-center">
                                ...
                                <div>
                                    ...
                                    <div className={`text-2xl font-bold ${drillDownData.is_favorable ? 'text-green-600' : 'text-red-600'
                                        }`}>
                                ...
                                <div className="border rounded-lg overflow-hidden">
                                    ...
                                    {drillDownData.child_items.slice(0, 10).map((item, index) => (
                                        <div key={index} className="grid grid-cols-5 gap-4 p-3 border-t text-sm">
                                            ...
                                            <div className={`text-right ${item.variance_percentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>
