...
export const ValidationAnnouncer: React.FC<ValidationAnnouncerProps> = ({
  ...
  const [debouncedValidation, setDebouncedValidation] = useState<ValidationResult | null>(
    validation,
  );
  ...
  const createAnnouncement = useCallback(
    (validation: ValidationResult | null): string => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !announcementHistory.includes(newAnnouncement)
    ) {
...
}> = ({ validation, priority = "polite", includeFieldNames = true, maxFieldsToAnnounce = 3 }) => {
  const i18n = useValidationI18n();
  const [announcement, setAnnouncement] = useState("");
...
export const ProgressValidationAnnouncer: React.FC<{
  ...
  currentField?: string;
  priority?: "polite" | "assertive";
}> = ({ total, validated, errors, currentField, priority = "polite" }) => {
  const i18n = useValidationI18n();
  const [announcement, setAnnouncement] = useState("");
...
export const ValidationStatusAnnouncer: React.FC<{
  ...
  hasWarnings: boolean;
  priority?: "polite" | "assertive";
}> = ({ isValidating, isValid, hasErrors, hasWarnings, priority = "polite" }) => {
  const i18n = useValidationI18n();
  const [announcement, setAnnouncement] = useState("");
