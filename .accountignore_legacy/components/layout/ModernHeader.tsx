...
export default function ModernHeader({ onToggleSidebar }: ModernHeaderProps) {
    ...
    const hasUrgentNotifications = sampleNotifications.some(n => n.type === 'error' || n.type === 'warning')
                                    ...
                                    {filteredSearchResults.length > 0 ? (
                                        <div className="space-y-2 max-h-96 overflow-y-auto">
                                            {filteredSearchResults.map((result) => (
                                                <Card key={result.id} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
