        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${getHealthColor(dashboardData?.system_health?.overall_score || 0)}`}>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${getHealthColor(dashboardData?.system_health?.database_health || 0)}`}>
                            {dashboardData?.system_health?.database_health || 0}%
                        </div>
                        <div className="mt-2">
                            <Progress value={dashboardData?.system_health?.database_health || 0} className="h-2" />
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${getHealthColor(dashboardData?.system_health?.cache_health || 0)}`}>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${getHealthColor(dashboardData?.system_health?.memory_health || 0)}`}>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-3">
                                {[
                                    { query: 'SELECT * FROM gl_entries WHERE company_id = ? AND posting_date BETWEEN ? AND ?', duration: '2.3s', count: 45 },
                                    { query: 'SELECT SUM(debit - credit) FROM gl_entries WHERE account_id IN (?)', duration: '1.8s', count: 23 },
                                    { query: 'UPDATE budget_line_items SET actual_amount = ? WHERE budget_plan_id = ?', duration: '1.2s', count: 12 }
                                ].map((slowQuery, index) => (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-3">
                                    {[
                                        { type: 'Query Optimization', improvement: '+45%', timestamp: '2 hours ago', status: 'success' },
                                        { type: 'Cache Strategy', improvement: '+23%', timestamp: '4 hours ago', status: 'success' },
                                        { type: 'Index Addition', improvement: '+67%', timestamp: '1 day ago', status: 'success' }
                                    ].map((opt, index) => (
