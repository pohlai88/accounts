...
import { CalendarIcon, Plus, Minus, FileText, Send, X, DollarSign, AlertTriangle, CheckCircle, Truck, Receipt } from 'lucide-react'
import { cn } from '@/lib/utils'
import { SalesManagementService, CreateSalesOrderInput, SalesOrder, SalesOrderStatus } from '@/lib/sales-management-service'
...
export function SalesOrderForm({ companyId, order, quotationId, onSuccess, onCancel }: SalesOrderFormProps) {
    ...
    const getStatusBadgeVariant = (status: SalesOrderStatus) => {
        switch (status) {
            ...
            case 'To Deliver and Bill': return 'default'
            case 'To Bill': return 'default'
            case 'To Deliver': return 'default'
    ...
    const getStatusIcon = (status: SalesOrderStatus) => {
        switch (status) {
            ...
            case 'To Deliver': return <Truck className="h-4 w-4" />
            case 'To Bill': return <Receipt className="h-4 w-4" />
            ...
            <div className="flex items-center justify-between">
                <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground">
                        {order ? 'Update sales order details' : quotationId ? 'Convert quotation to sales order' : 'Create a new sales order'}
                    </p>
                </div>
                {order && (
            ...
            {creditLimitCheck?.credit_limit_exceeded && (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>
                        ...
                        Outstanding: ${creditLimitCheck.outstanding_amount.toFixed(2)},
                        Available: ${creditLimitCheck.credit_available.toFixed(2)}
                    </AlertDescription>
                </Alert>
            )}
                                    ...
                                    <div key={field.id} className="border rounded-lg p-4 space-y-4">
                                        ...
                                        {order && order.items?.[index] && (
                                            <div className="grid grid-cols-3 gap-4 text-sm">
                                                <div>
                                                    ...
                                                    <span className="font-medium">{order.items[index].delivered_qty.toFixed(3)}</span>
                                                </div>
                                                <div>
                                                    <span className="text-muted-foreground">Billed: </span>
                                                    <span className="font-medium">{order.items[index].billed_qty.toFixed(3)}</span>
                                                </div>
                                                <div>
                                                    <span className="text-muted-foreground">Pending: </span>
                                                    <span className="font-medium">{order.items[index].pending_qty.toFixed(3)}</span>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex justify-end">
                                            <div className="text-sm text-muted-foreground">
                                                Amount: ${((watchedItems[index]?.qty || 0) * (watchedItems[index]?.rate || 0)).toFixed(2)}
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                ...
                                <div className="mt-4 p-3 bg-muted rounded-lg">
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium">${creditLimitCheck.outstanding_amount.toFixed(2)}</p>
                                        </div>
                                        <div>
                                            <p className="text-muted-foreground">Utilization</p>
                                            <p className="font-medium">{creditLimitCheck.credit_utilization.toFixed(1)}%</p>
                                        </div>
                                        <div>
                                            <p className="text-muted-foreground">Available</p>
                                            <p className={cn(
                                                "font-medium",
                                                creditLimitCheck.credit_available < 0 ? "text-destructive" : "text-green-600"
                                            )}>
                    ...
                    <div className="flex items-center justify-end space-x-2">
                        ...
                        {order && order.docstatus === 0 && (
                            <Button
                                ...
                                disabled={isLoading || (creditLimitCheck?.credit_limit_exceeded && !form.getValues('customer_id'))}
                            >
