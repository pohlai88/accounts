...
export function SalesDashboard({ companyId }: SalesDashboardProps) {
    ...
    const loadDashboardData = async () => {
        ...
        const quotationsCall = () => apiClient.getQuotations({
            limit: 50,
        ...
        const ordersCall = () => apiClient.getSalesOrders({
            limit: 50,
            sort_by: 'transaction_date',
            sort_order: 'desc'
        })
    ...
    const generateAnalytics = () => {
        ...
        const monthlyQuotations = quotations.filter(q =>
            new Date(q.transaction_date) >= thisMonth
        )

        const monthlyOrders = orders.filter(o =>
            new Date(o.transaction_date) >= thisMonth
        )
        ...
        setAnalytics({
            ...
            conversionRate: monthlyQuotations.length > 0
                ? (monthlyOrders.length / monthlyQuotations.length) * 100
                : 0,
    ...
    const getStatusBadge = (status: QuotationStatus | SalesOrderStatus, type: 'quotation' | 'order') => {
        const getVariant = () => {
            if (type === 'quotation') {
        ...
        const getIcon = () => {
            if (type === 'quotation') {
                switch (status as QuotationStatus) {
                    case 'Ordered': return <CheckCircle className="h-3 w-3" />
                    case 'Open': return <Clock className="h-3 w-3" />
                    case 'Lost': return <AlertCircle className="h-3 w-3" />
                ...
                switch (status as SalesOrderStatus) {
                    ...
                    case 'To Deliver': return <Truck className="h-3 w-3" />
                    case 'To Bill': return <Receipt className="h-3 w-3" />
                    case 'To Deliver and Bill': return <Clock className="h-3 w-3" />
                    default: return <FileText className="h-3 w-3" />
                }
            }
        }
    ...
    const filteredQuotations = quotations.filter(quotation => {
        const matchesSearch = quotation.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
