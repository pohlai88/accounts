        ...
        <div className={cn('space-y-2', className)}>
                                    ...
                                    return (
                                        <SelectItem key={curr} value={curr}>
                                            <div className="flex items-center space-x-2">
                                                ...
                                                {info && <span className="text-xs text-muted-foreground">{info.symbol}</span>}
                                            </div>
                                        </SelectItem>
                                    )
                                })}
            ...
            {showQuickAmounts && quickAmounts.length > 0 && (
                <div className="flex flex-wrap gap-1 p-2 bg-muted rounded-md">
                    {quickAmounts.map((amount, index) => (
                        ...
                        >
                            {currencyInfo.symbol}{formatAmount(amount)}
                        </Button>
                    ))}
                </div>
            )}
            ...
            {trend && comparisonValue && (
                <div className="text-xs text-muted-foreground flex items-center space-x-1">
                    ...
                    <span>
                        {trend.percentage.toFixed(1)}% vs {currencyInfo.symbol}{formatAmount(comparisonValue)}
                    </span>
                </div>
            )}
            ...
            {(min !== undefined || max !== undefined) && (
                <div className="text-xs text-muted-foreground">
                    {min !== undefined && max !== undefined && (
                        <>Range: {currencyInfo.symbol}{formatAmount(min)} - {currencyInfo.symbol}{formatAmount(max)}</>
                    )}
                    {min !== undefined && max === undefined && (
                        <>Minimum: {currencyInfo.symbol}{formatAmount(min)}</>
                    )}
                    {min === undefined && max !== undefined && (
                        <>Maximum: {currencyInfo.symbol}{formatAmount(max)}</>
                    )}
                </div>
            )}
        </div>
