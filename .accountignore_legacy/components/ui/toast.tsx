  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        business:
          "border-purple-200 bg-purple-50 text-purple-900 dark:border-purple-800 dark:bg-purple-900/20 dark:text-purple-100",
      },
    },
    defaultVariants: {
      variant: "default",
...
toast.transaction = {
  created: (type: string, amount?: number, currency = "MYR") =>
    toast.success(
      `${type} created successfully${amount ? ` for ${currency} ${amount.toLocaleString()}` : ""}`,
      {
...
toast.payment = {
  received: (amount: number, currency = "MYR", customer?: string) =>
    toast.success(
      `Payment received: ${currency} ${amount.toLocaleString()}${customer ? ` from ${customer}` : ""}`,
      {
  ...
  sent: (amount: number, currency = "MYR", supplier?: string) =>
    toast.success(
      `Payment sent: ${currency} ${amount.toLocaleString()}${supplier ? ` to ${supplier}` : ""}`,
      {
        businessContext: { type: "payment", amount, currency },
      },
    ),
