...
export function useGLValidation(options: UseGLValidationOptions) {
     ...
     */
    const createCacheKey = useCallback((field: string, value: any, context: Partial<VoucherValidationContext>): string => {
     ...
     */
    const validateVoucher = useCallback(async (context: VoucherValidationContext): Promise<ValidationResult> => {
        ...
        try {
            ...
            const validationError: ValidationError = new Error(`Voucher validation error: ${error.message}`)
    ...
    const validateField = useCallback(async (
        ...
        return new Promise((resolve) => {
            const timer = setTimeout(async () => {
                try {
                    const result = await validatorRef.current!.validateFieldRealTime(
                        field,
                        value,
                        { companyId, ...context }
                    )
                    ...
                    // Store the error for debugging
                    const validationError: ValidationError = new Error(`Field validation error for ${field}: ${error.message}`)
    ...
    }, [companyId, enableRealTimeValidation, debounceMs, createCacheKey, getCachedResult, setCachedResult])
    ...
    const validateFields = useCallback(async (
        ...
        const tokens = fieldsToValidate.reduce((acc, { field }) => {
            const nextToken = (fieldTokensRef.current.get(field) ?? 0) + 1
            fieldTokensRef.current.set(field, nextToken)
            acc[field] = nextToken
            return acc
        ...
        return new Promise((resolve) => {
            const timer = setTimeout(async () => {
                try {
                    ...
                    const batchResultsMap = batchResults.reduce((acc, { field, result }) => {
                        acc[field] = result
                        return acc
    ...
    }, [companyId, enableRealTimeValidation, enableBatchValidation, debounceMs, createCacheKey, getCachedResult, setCachedResult])
    ...
    const getValidationErrorsByCategory = useCallback(() => {
        const v = validationState.voucherValidation
        if (!v) return {}
        return v.errors.reduce((groups, error) => {
     ...
     */
    const validateVoucherRealTime = useCallback(async (
     ...
     */
    const getAccessibilityProps = useCallback((field: string) => {
        ...
        return {
            ...
            'aria-describedby': hasErrors || hasWarnings || isValidating
     ...
     */
    const retryValidation = useCallback(async (field?: string) => {
        if (field) {
            ...
            const failedFields = Object.keys(validationState.fieldValidations).filter(f => hasFieldErrors(f))
    ...
    return {
        ...
        setCachedResult: (field: string, value: any, context: Partial<VoucherValidationContext>, result: ValidationResult) =>
...
export function useValidationFormState<T extends Record<string, any>>(
    ...
    const handleSubmit = useCallback(async (onSubmit: (values: T) => Promise<void> | void) => {
        ...
        const hasErrors = Object.keys(values).some(field =>
            validation.hasFieldErrors(field)
        )
    ...
    const getFieldProps = useCallback(<K extends keyof T>(field: K) => {
        ...
        return {
            ...
            helperText: (isFieldTouched(field) || submitted)
    ...
    return {
        ...
        isFieldTouched,
        getFieldProps,
        isValid: Object.keys(values).every(field =>
