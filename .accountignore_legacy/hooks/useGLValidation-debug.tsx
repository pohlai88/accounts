...
export const useValidationDebug = (validation: ReturnType<typeof useGLValidation>) => {
    useEffect(() => {
        if (process.env.NODE_ENV === 'development') {
            ...
            console.log('Last Validated:', validation.lastValidated ? new Date(validation.lastValidated).toLocaleTimeString() : 'Never')
            console.groupEnd()
        }
    }, [validation.voucherValidation, validation.fieldValidations, validation.isValidating, validation.lastValidated])
    ...
    const getValidationSummary = useCallback(() => {
        ...
        return {
            ...
            fieldWarnings: Object.values(fieldValidations).filter(v =>
                v.warnings.length > 0
            ).length,
    ...
    const logValidationTrace = useCallback((action: string, field?: string, details?: any) => {
        if (process.env.NODE_ENV === 'development') {
 ...
 */
export const createMockValidation = (overrides: Partial<ReturnType<typeof useGLValidation>> = {}) => {
    const defaultValidation: ReturnType<typeof useGLValidation> = {
        ...
        // Actions
        validateVoucher: jest.fn().mockResolvedValue({ isValid: true, errors: [], warnings: [], suggestions: [] }),
        validateField: jest.fn().mockResolvedValue({ isValid: true, errors: [], warnings: [], suggestions: [] }),
        validateFields: jest.fn().mockResolvedValue({}),
        validateVoucherRealTime: jest.fn().mockResolvedValue({ isValid: true, errors: [], warnings: [], suggestions: [] }),
...
export class ValidationPerformanceMonitor {
    ...
    recordValidation(field: string, duration: number, fromCache: boolean, hasError: boolean) {
        ...
        this.metrics.averageTime = this.validationTimes.reduce((a, b) => a + b, 0) / this.validationTimes.length
    ...
    private getRecommendations(): string[] {
        ...
        if (metrics.errorRate > 0.1) {
            recommendations.push('High error rate detected. Check validation logic and network connectivity.')
        }

        if (metrics.averageTime > 500) {
            recommendations.push('Average validation time is high. Consider optimizing validation logic or enabling caching.')
        }

        if (metrics.cacheHitRate < 0.5) {
            recommendations.push('Low cache hit rate. Consider increasing cache TTL or reviewing cache key strategy.')
        }

        if (metrics.p95Time > 1000) {
            recommendations.push('95th percentile validation time is high. Investigate performance bottlenecks.')
        }
