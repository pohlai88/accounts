...
test.describe('Journal Posting Flow with RLS', () => {
  ...
  test('should create and post a balanced journal entry', async ({ page }) => {
    ...
    await test.step('Save journal as draft', async () => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Journal saved as draft')
    ...
    await test.step('Post journal entry', async () => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Journal posted successfully')
    ...
    await test.step('Verify audit trail created', async () => {
      ...
      await expect(page.locator('[data-testid="audit-entry"]').first()).toContainText('Journal posted')
      await expect(page.locator('[data-testid="audit-user"]').first()).toContainText('Test Accountant')
    })
  })
  ...
  test('should handle concurrent editing gracefully', async ({ page, context }) => {
    ...
    await test.step('Attempt concurrent modifications', async () => {
      ...
      await expect(page2.locator('[data-testid="conflict-warning"]')).toContainText('Journal has been modified')
    })
  ...
  test('should maintain performance under load', async ({ page }) => {
    ...
    await test.step('Create journal with many lines', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        await page.click('[data-testid="add-line-button"]')
        await page.selectOption(`[data-testid="line-${i}-account"]`, i % 2 === 0 ? 'e2e-account-1001' : 'e2e-account-4001')
