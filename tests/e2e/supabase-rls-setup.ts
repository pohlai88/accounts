...
async function cleanupTestData(serviceClient: any) {
  ...
  const e2eUsers = authUsers?.users?.filter(user => 
    user.email?.includes('@e2etest.aibos.com')
  ) || []
...
async function setupChartOfAccounts(serviceClient: any, tenantId: string, companyId: string) {
  ...
  const accounts = [
    ...
    { code: '2001', name: 'Accounts Payable', accountType: 'LIABILITY', level: 1, parentId: '2000' },
    { code: '2002', name: 'Accrued Expenses', accountType: 'LIABILITY', level: 1, parentId: '2000' },
    ...
    { code: '5002', name: 'Operating Expenses', accountType: 'EXPENSE', level: 1, parentId: '5000' }
  ]
...
async function verifyRLSPolicies(
  ...
  for (const user of users.slice(0, 2)) { // Test first 2 users
    ...
    const { data: tenantData, error: tenantError } = await userClient
      .from('tenants')
      .select('*')
