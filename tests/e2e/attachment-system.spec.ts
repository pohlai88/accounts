...
test.describe('Attachment System E2E Tests', () => {
  ...
  test.describe('Document Preview and Download', () => {
    ...
    test('should preview PDF document', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="preview-filename"]')).toContainText(TEST_FILES.pdf.name);
    ...
    test('should show document metadata', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="metadata-filename"]')).toContainText(TEST_FILES.pdf.name);
  ...
  test.describe('OCR Processing Workflow', () => {
    ...
    test('should display OCR results', async ({ page }) => {
      ...
      await page.evaluate((fileId) => {
        ...
        window.dispatchEvent(new CustomEvent('ocr-completed', {
          detail: {
            attachmentId: fileId,
            confidence: 0.85,
    ...
    test('should handle OCR errors', async ({ page }) => {
      // Mock OCR failure
      await page.evaluate((fileId) => {
        window.dispatchEvent(new CustomEvent('ocr-failed', {
          detail: {
            attachmentId: fileId,
      ...
      await expect(page.locator('[data-testid="ocr-error"]')).toContainText('File format not supported');
    });
  });
  ...
  test.describe('Document Approval Workflow', () => {
    ...
    test('should start approval workflow', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="approval-notification"]')).toContainText('Approval request sent');
    });

    test('should handle approval decision', async ({ page }) => {
      // Mock approval decision
      await page.evaluate((fileId) => {
        window.dispatchEvent(new CustomEvent('approval-decision', {
          detail: {
            attachmentId: fileId,
            decision: 'approved',
      ...
      await expect(page.locator('[data-testid="approval-comments"]')).toContainText('approved for payment');
    });
    ...
    test('should handle approval rejection', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="rejection-reason"]')).toContainText('exceeds budget');
    });
  });
  ...
  test.describe('Batch Operations', () => {
    ...
    test('should select multiple documents', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="selection-count"]')).toContainText('3 documents selected');
    ...
    test('should perform bulk delete operation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="bulk-delete-success"]')).toContainText('3 documents deleted');
    });
  ...
  test.describe('Document Retention and Compliance', () => {
    ...
    test('should handle legal hold', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="legal-hold-status"]')).toContainText('On Legal Hold');
    ...
    test('should show retention warnings', async ({ page }) => {
      ...
      await page.evaluate((fileId) => {
        window.dispatchEvent(new CustomEvent('retention-warning', {
          detail: {
            attachmentId: fileId,
            daysUntilExpiry: 7,
            action: 'archive'
  ...
  test.describe('Accessibility and Usability', () => {
    ...
    test('should work on mobile viewports', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="document-list"]')).toHaveCSS('flex-direction', 'column');
    });
  });
});
