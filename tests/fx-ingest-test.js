...
import { ingestFxRates, getCurrentFxRate, STALENESS_THRESHOLDS } from '../packages/accounting/src/fx/ingest.js';
...
async function testFxIngest() {
    try {
        ...
        if (result.success) {
            ...
            console.log(`   Staleness: ${result.staleness.isStale ? 'STALE' : 'FRESH'} (${result.staleness.ageMinutes.toFixed(1)} min)`);
