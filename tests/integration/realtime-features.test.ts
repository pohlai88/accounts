...
import { WebSocketManager, RealtimeEventSystem, PresenceSystem, NotificationSystem } from '@aibos/realtime';
...
describe.skipIf(skipTests)('Real-time Features Integration', () => {
    ...
    describe('Event System', () => {
        ...
        it('should filter events by criteria', async () => {
            ...
            eventSystem.publishTenantEvent(tenantId, 'test.event', { priority: 'low' }, {
                channel: `${tenantId}:filtered`
            });
            eventSystem.publishTenantEvent(tenantId, 'test.event', { priority: 'high' }, {
                channel: `${tenantId}:filtered`
            });
