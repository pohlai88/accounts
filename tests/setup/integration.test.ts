...
describe("Integration Tests", () => {
  ...
  it("should handle complete member management workflow", async () => {
    ...
    const currentUserMember = data.data.find(
      (member: any) => member.user_id === testSetup.testUser.id,
    );
  ...
  it("should handle realtime updates during tenant operations", async () => {
    ...
    const channel = testSetup.supabase.channel(`test-tenant:${testSetup.testTenant.id}`).on(
      "postgres_changes",
      {
        event: "*",
        schema: "public",
    ...
    await Promise.race([subscribePromise, new Promise(resolve => setTimeout(resolve, 2000))]);
  ...
  it("should enforce tenant isolation across all operations", async () => {
    ...
    // Create user profile
    await testSetup.supabase.from("users").insert({
      id: otherUser.id,
    ...
    if (otherTenantData) {
      ...
      await testSetup.supabase.from("memberships").insert({
      ...
      await testSetup.supabase.from("tenants").delete().eq("id", otherTenantData.id);
    }
