...
import { getMonitoringHealth, getMonitoringStats } from '../../../../middleware/monitoring-middleware';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const overallStatus = health.status === 'unhealthy' ? 'unhealthy' : 
                         health.components?.metrics?.status === 'degraded' ? 'degraded' : 'healthy';
    ...
    const response = {
      ...
      issues: health.components ? [
        ...
        ...(health.components.tracing?.status === 'unhealthy' ? ['Tracing system unhealthy'] : []),
        ...(health.components.logging?.status === 'unhealthy' ? ['Logging system unhealthy'] : [])
      ] : [],
