...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error("Get analytics dashboard error", error instanceof Error ? error : new Error(String(error)));
...
async function getUsageMetrics(tenantId: string, startDate: Date, endDate: Date) {
  ...
  if (error) {
    logger.error("Error fetching usage metrics", error instanceof Error ? error : new Error(String(error)));
    return [];
  }
...
async function getSubscriptionData(tenantId: string) {
  ...
  if (error) {
    logger.error("Error fetching subscription data", error instanceof Error ? error : new Error(String(error)));
    return null;
  }
