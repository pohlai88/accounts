...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const validatedParams = ListAttachmentsReq.parse({
      ...
      sortBy: searchParams.sortBy as 'created_at' | 'updated_at' | 'filename' | 'file_size' | 'category' || 'created_at',
    ...
    // Apply sorting
    const sortColumn = validatedParams.sortBy === 'created_at' ? 'created_at' :
      validatedParams.sortBy === 'updated_at' ? 'updated_at' :
        validatedParams.sortBy === 'filename' ? 'filename' :
          validatedParams.sortBy === 'file_size' ? 'file_size' :
            validatedParams.sortBy === 'category' ? 'category' : 'created_at';
    ...
    const availableTags = [...new Set(
      tagData?.flatMap(d => d.tags || []).filter(Boolean)
    )].sort();
