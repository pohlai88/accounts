...
export class PerformanceMiddleware {
    ...
    startTracking(req: NextRequest): PerformanceTracker {
        ...
        return new PerformanceTracker(
            req,
            this.config,
            (metrics) => this.recordMetrics(metrics)
        );
    }
    ...
    getStats(timeWindow?: number): {
        ...
        const slowRequests = recentMetrics.filter(m => m.duration > this.config.slowRequestThreshold).length;
    ...
    getHealthStatus(): {
        ...
        if (stats.slowRequests > stats.totalRequests * 0.1) {
            issues.push(`High slow request rate: ${((stats.slowRequests / stats.totalRequests) * 100).toFixed(2)}%`);
        }
...
export class PerformanceTracker {
    ...
    constructor(
        ...
        this.requestId = req.headers.get('x-request-id') || `req_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
