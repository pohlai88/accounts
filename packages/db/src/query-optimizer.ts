...
function hasTenantId(table: unknown): table is { tenantId: AnyColumn } {
    return typeof table === "object" && table !== null && "tenantId" in (table as Record<string, unknown>);
}
...
export class QueryOptimizer {
    ...
    async optimizeAggregationQuery(
        ...
        const start = Date.now();
        const queryId = `AGGREGATION_${Object.keys(aggregations)
            .sort()
            .join("_")}`;
        ...
        try {
            ...
            let q = db
                .select(aggregations)
                .from(table)
                .where(whereExpr) as Executable<unknown>;
    ...
    async optimizeSearchQuery<T>(
        ...
        try {
            const likeConds = searchColumns.map((col) =>
                sql`${getColumn(table, col)} ILIKE ${`%${searchTerm}%`}`,
            );
            ...
            if (typeof q.orderBy === "function" && searchColumns.length > 0) {
                const cases = searchColumns.map((col) =>
    ...
    private logQuery(
        id: string,
        status: "SUCCESS" | "ERROR" | "CACHE_HIT" | "BATCH_SUCCESS" | "BATCH_ERROR" | "AGGREGATION_SUCCESS" | "AGGREGATION_ERROR" | "SEARCH_SUCCESS" | "SEARCH_ERROR",
        ms: number,
        error?: unknown,
    ): void {
