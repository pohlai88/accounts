...
export interface ExportManagerService {
    ...
    downloadExport(id: string, userId: string): Promise<{ success: boolean; url?: string; error?: string }>;
...
export function createExportManagerService(): ExportManagerService {
    ...
    return {
        ...
        async downloadExport(id: string, userId: string): Promise<{ success: boolean; url?: string; error?: string }> {
            const exportRecord = exports.get(id);
            ...
            console.log(`Export ${id} downloaded by user ${userId} (${exportRecord.downloadCount}/${exportRecord.maxDownloads})`);
        ...
        async getExportStats(tenantId: string, companyId?: string): Promise<ExportStats> {
            ...
            const exportsByFormat = filtered.reduce((acc, exp) => {
                acc[exp.format] = (acc[exp.format] || 0) + 1;
                return acc;
            }, {} as Record<ExportFormat, number>);

            const exportsByReportType = filtered.reduce((acc, exp) => {
            ...
            const userCounts = filtered.reduce((acc, exp) => {
