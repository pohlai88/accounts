...
export function useCreateInvoice(
    ...
    const mutate = useCallback(async (invoiceData: unknown) => {
        if (options?.enabled === false) return;
...
export function usePostInvoice(
    ...
    const mutate = useCallback(async (data: { invoiceId: string; arAccountId: string }) => {
        if (options?.enabled === false) return;
...
export function useCreateCustomer(
    ...
    const mutate = useCallback(async (customerData: unknown) => {
        if (options?.enabled === false) return;
