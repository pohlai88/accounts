...
export interface EmailPayload { to: string; subject: string; html: string; idemKey?: string; from?: "system"|"billing"|"support" }
export interface EmailPort { send(p: EmailPayload): Promise<{ id: string }> }
...
export const emailAdapter: EmailPort = {
  async send(p) {
    ...
    const to = process.env.NODE_ENV === "production" ? p.to : (process.env.DEV_FALLBACK_EMAIL || "test@example.com");
