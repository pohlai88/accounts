...
export class RedisCacheAdapter implements CacheAdapter {
    ...
    async connect(): Promise<void> {
        try {
            ...
            (this.client as { on: (event: string, callback: (err: Error) => void) => void }).on('error', (err: Error) => {
    ...
    async get<T>(key: string): Promise<T | null> {
        try {
            ...
            const result = await (this.client as { get: (key: string) => Promise<string | null> }).get(fullKey);
    ...
    async set<T>(key: string, value: T, options: CacheOptions = {}): Promise<boolean> {
        try {
            ...
            if (ttl > 0) {
                await (this.client as { setEx: (key: string, ttl: number, value: string) => Promise<void> }).setEx(fullKey, ttl, serialized);
            } else {
                await (this.client as { set: (key: string, value: string) => Promise<void> }).set(fullKey, serialized);
            }
    ...
    async delPattern(pattern: string): Promise<number> {
        try {
            ...
            const keys = await (this.client as { keys: (pattern: string) => Promise<string[]> }).keys(fullPattern);
    ...
    async delByTags(tags: string[]): Promise<number> {
        try {
            ...
            for (const tag of tags) {
                ...
                const keys = await (this.client as { sMembers: (key: string) => Promise<string[]> }).sMembers(tagKey);
    ...
    async exists(key: string): Promise<boolean> {
        try {
            ...
            const fullKey = this.buildKey(key);
            const result = await (this.client as { exists: (key: string) => Promise<number> }).exists(fullKey);
            return result === 1;
        } catch (error) {
    ...
    async expire(key: string, ttl: number): Promise<boolean> {
        try {
            ...
            const fullKey = this.buildKey(key);
            const result = await (this.client as { expire: (key: string, ttl: number) => Promise<number> }).expire(fullKey, ttl);
            return result === 1;
        } catch (error) {
    ...
    async flush(): Promise<boolean> {
        try {
            ...
            const keys = await (this.client as { keys: (pattern: string) => Promise<string[]> }).keys(pattern);
    ...
    private async storeTags(key: string, tags: string[]): Promise<void> {
        for (const tag of tags) {
            const tagKey = this.buildTagKey(tag);
            await (this.client as { sAdd: (key: string, member: string) => Promise<number> }).sAdd(tagKey, key);
            // Set expiration for tag set
            await (this.client as { expire: (key: string, ttl: number) => Promise<number> }).expire(tagKey, this.config.ttl);
        }
    }
    ...
    private async cleanupTags(key: string): Promise<void> {
        ...
        const tagKeys = await (this.client as { keys: (pattern: string) => Promise<string[]> }).keys(pattern);

        for (const tagKey of tagKeys) {
            await (this.client as { sRem: (key: string, member: string) => Promise<number> }).sRem(tagKey, key);
        }
    }
}
