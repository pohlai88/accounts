...
export async function getEnhancedUserContext(request: NextRequest): Promise<EnhancedUserContext> {
    ...
    try {
        ...
        const memberPermissions: MemberPermissions | undefined = membershipResult.data?.permissions || undefined;
        ...
        const allRoles = [primaryRole, ...additionalRoles].filter((role, index, arr) => arr.indexOf(role) === index);
