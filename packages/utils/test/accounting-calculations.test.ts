...
class MockAccountingService {
    ...
    calculateDepreciation(assetCost: number, salvageValue: number, usefulLife: number, method: string): number {
        switch (method) {
            case 'straight-line':
    ...
    createJournalEntry(entries: Array<{ account: string; debit: number; credit: number }>): { success: boolean; error?: string } {
...
describe('Accounting Calculations', () => {
    ...
    describe('Tax Calculations', () => {
        it('should calculate sales tax correctly', () => {
            ...
            const taxRate = 8.25;
            const expectedTax = 82.50;

            FinancialTestUtils.expectCorrectTaxCalculation(
                taxableAmount,
                taxRate,
                expectedTax
            );
            FinancialTestUtils.expectFinancialPrecision(accountingService.calculateTax(taxableAmount, taxRate), expectedTax);
        });
    ...
    describe('Edge Cases and Error Handling', () => {
        it('should handle very small amounts', () => {
            ...
            const taxRate = 8.25;
            const expectedTax = 0.00; // Rounded down
