...
export class ReportTestUtils {
    ...
    static expectValidCashFlowStatement(data: {
        ...
        const expectedNetCashFlow = data.operatingCashFlow + data.investingCashFlow + data.financingCashFlow;
     ...
     */
    static expectValidTrialBalance(accounts: Array<{
        accountName: string;
        debit: number;
        credit: number;
    }>): void {
     ...
     */
    static expectValidDepreciationSchedule(schedule: Array<{
        year: number;
        ...
        for (let i = 0; i < schedule.length; i++) {
            ...
            if (i < schedule.length - 1) {
                FinancialTestUtils.expectFinancialPrecision(entry.endingValue, schedule[i + 1].beginningValue);
            }
        }
    }
    ...
    static expectValidBudgetVsActual(data: {
        ...
        const expectedVariancePercentage = data.budget !== 0 ? (expectedVariance / data.budget) * 100 : 0;
        FinancialTestUtils.expectFinancialPrecision(data.variancePercentage, expectedVariancePercentage);
    }
    ...
    static expectValidPayrollCalculation(data: {
        ...
        const totalDeductions = data.federalTax + data.stateTax + data.socialSecurity +
            data.medicare + data.otherDeductions;
