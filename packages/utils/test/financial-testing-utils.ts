...
export class FinancialTestUtils {
    ...
    static expectCorrectLoanPayment(
        ...
        const calculatedPayment = principal * (monthlyRate * Math.pow(1 + monthlyRate, numberOfPayments)) /
...
export const accountingMatchers = {
    toBeBalanced: (received: { debits: number[]; credits: number[] }) => {
        ...
        return {
            message: () => `Expected debits (${totalDebits}) to equal credits (${totalCredits}). Difference: ${difference}`,
            pass: difference < 0.01
        };
    },

    toHaveFinancialPrecision: (received: number, expected: number, precision: number = 2) => {
        const roundedReceived = Math.round(received * Math.pow(10, precision)) / Math.pow(10, precision);
        const roundedExpected = Math.round(expected * Math.pow(10, precision)) / Math.pow(10, precision);

        return {
            message: () => `Expected ${roundedReceived} to equal ${roundedExpected} with ${precision} decimal precision`,
