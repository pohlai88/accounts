...
describe("AttachmentService - Advanced Features", () => {
  ...
  describe("batchDelete", () => {
    it("should delete multiple attachments successfully", async () => {
      ...
      // Act
      const result = await attachmentService.batchDelete(attachmentIds, tenantId, userId);
  ...
  describe("Error Handling", () => {
    it("should handle unexpected errors gracefully", async () => {
      ...
      // Act & Assert
      await expect(attachmentService.searchAttachments("tenant-123")).rejects.toThrow(
        "Unexpected error",
      );
    });
  });
