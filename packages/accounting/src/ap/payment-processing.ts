  ...
  try {
    ...
    if (billAllocations.length > 0) {
      ...
      const totalBillPayments =
        billAllocations.reduce((sum, a) => sum + a.allocatedAmount, 0) * input.exchangeRate;
      journalLines.push({
    ...
    if (invoiceAllocations.length > 0) {
      ...
      const totalInvoiceReceipts =
        invoiceAllocations.reduce((sum, a) => sum + a.allocatedAmount, 0) * input.exchangeRate;
      journalLines.push({
        accountId: input.bankAccountId,
...
export function validatePaymentBusinessRules(input: PaymentProcessingInput): {
  ...
  if (input.allocations) {
    ...
    if (Math.abs(totalAllocated - input.amount) > 0.01) {
      errors.push(
        `Total allocated amount (${totalAllocated}) does not match payment amount (${input.amount})`,
      );
    }
  ...
  for (const allocation of allocations) {
    ...
    if (allocation.allocatedAmount > outstandingBalance) {
      if (outstandingBalance === 0) {
        ...
        warnings.push(
          `Document ${allocation.documentNumber}: Allocated amount (${allocation.allocatedAmount}) exceeds outstanding balance (${outstandingBalance})`,
        );
      }
    }
  }
