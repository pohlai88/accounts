...
export function validateBillLines(lines: BillLineInput[]): {
    ...
    for (const line of lines) {
        ...
        if (Math.abs(line.lineAmount - expectedAmount) > 0.01) {
            errors.push(`Line ${line.lineNumber}: Line amount ${line.lineAmount} does not match quantity × unit price (${expectedAmount})`);
        }
        ...
        if (line.taxCode && line.taxRate) {
            ...
            if (Math.abs((line.taxAmount || 0) - expectedTaxAmount) > 0.01) {
                errors.push(`Line ${line.lineNumber}: Tax amount ${line.taxAmount} does not match line amount × tax rate (${expectedTaxAmount})`);
            }
        }
