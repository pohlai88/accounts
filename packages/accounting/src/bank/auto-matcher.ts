...
export async function autoMatchTransactions(
    ...
    const automaticMatches = matches.filter(m => m.confidence >= fullConfig.autoMatchThreshold).length;
...
function calculateMatchScore(
    ...
    } else if (amountDifference <= candidate.amount * 0.01) { // Within 1%
        totalScore += config.exactAmountWeight * 0.8;
        reasons.push('Close amount match (within 1%)');
    } else if (amountDifference <= candidate.amount * 0.05) { // Within 5%
    ...
    if (transaction.reference && candidate.reference) {
        ...
        } else if (transaction.reference.toLowerCase().includes(candidate.reference.toLowerCase()) ||
    ...
    if (config.enableFuzzyMatching) {
        ...
        if (commonWords.length > 0) {
            const keywordScore = config.descriptionMatchWeight * (commonWords.length / Math.max(transactionWords.length, candidateWords.length));
...
export function filterCandidatesByDateRange(
    ...
    return candidates.filter(candidate =>
        candidate.date >= bufferStart && candidate.date <= bufferEnd
    );
}
