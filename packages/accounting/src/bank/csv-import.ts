  ...
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      try {
        ...
        errors.push(
          `Row ${rowNumber}: ${error instanceof Error ? error.message : "Unknown parsing error"}`,
        );
      }
    }
    ...
    return {
      ...
      warnings: [
        ...warnings,
        ...duplicates.map(i => `Row ${i + 1}: Duplicate transaction detected`),
      ],
      summary: {
...
function parseTransactionRow(
  ...
  const reference = format.referenceColumn
    ? row[format.referenceColumn]?.toString().trim() || undefined
    : undefined;
  ...
  const transactionType = format.typeColumn
    ? row[format.typeColumn]?.toString().trim() || undefined
    : undefined;
...
function validateTransaction(
  ...
  if (transaction.description.length > 255) {
    warnings.push(
      `Row ${rowNumber}: Description is very long (${transaction.description.length} characters)`,
    );
  }
