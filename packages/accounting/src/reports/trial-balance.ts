...
export async function generateTrialBalance(
    ...
    try {
        ...
        const metadata = {
            ...
            accountsWithActivity: trialBalanceAccounts.filter(a =>
                a.periodDebits > 0 || a.periodCredits > 0 || a.closingBalance !== 0
            ).length,
 ...
 */
function calculateTrialBalanceTotals(accounts: TrialBalanceAccount[]): TrialBalanceResult['totals'] {
    let totalDebits = 0;
    let totalCredits = 0;
    let totalAssets = 0;
    ...
    for (const account of accounts) {
        ...
        switch (account.accountType.toUpperCase()) {
            case 'ASSET':
                totalAssets += account.normalBalance === 'DEBIT' ? account.closingBalance : -account.closingBalance;
                break;
            case 'LIABILITY':
                totalLiabilities += account.normalBalance === 'CREDIT' ? account.closingBalance : -account.closingBalance;
                break;
            case 'EQUITY':
                totalEquity += account.normalBalance === 'CREDIT' ? account.closingBalance : -account.closingBalance;
                break;
            case 'REVENUE':
                totalRevenue += account.normalBalance === 'CREDIT' ? account.closingBalance : -account.closingBalance;
                break;
            case 'EXPENSE':
                totalExpenses += account.normalBalance === 'DEBIT' ? account.closingBalance : -account.closingBalance;
                break;
        }
    }
