...
function classifyProfitLossAccounts(
    ...
    const revenueAccounts = plAccounts.filter(a =>
        a.accountType.toUpperCase() === 'REVENUE'
    );

    const expenseAccounts = plAccounts.filter(a =>
...
function createRevenueSection(revenueAccounts: ProfitLossAccount[]): ProfitLossSection[] {
    ...
    return [{
        ...
        comparativeSubtotal: operatingRevenue.reduce((sum, acc) => sum + (acc.comparativePeriodAmount || 0), 0),
...
function createCostOfSalesSection(expenseAccounts: ProfitLossAccount[]): ProfitLossSection[] {
    ...
    return [{
        ...
        comparativeSubtotal: costOfSalesAccounts.reduce((sum, acc) => sum + (acc.comparativePeriodAmount || 0), 0),
...
function createOperatingExpensesSection(expenseAccounts: ProfitLossAccount[]): ProfitLossSection[] {
    ...
    return [{
        ...
        accounts: operatingExpenseAccounts.sort((a, b) => a.accountNumber.localeCompare(b.accountNumber)),
        subtotal: operatingExpenseAccounts.reduce((sum, acc) => sum + acc.currentPeriodAmount, 0),
        comparativeSubtotal: operatingExpenseAccounts.reduce((sum, acc) => sum + (acc.comparativePeriodAmount || 0), 0),
...
function createOtherIncomeSection(revenueAccounts: ProfitLossAccount[]): ProfitLossSection[] {
    ...
    return [{
        ...
        comparativeSubtotal: otherIncomeAccounts.reduce((sum, acc) => sum + (acc.comparativePeriodAmount || 0), 0),
...
function createOtherExpensesSection(expenseAccounts: ProfitLossAccount[]): ProfitLossSection[] {
    ...
    return [{
        ...
        comparativeSubtotal: otherExpenseAccounts.reduce((sum, acc) => sum + (acc.comparativePeriodAmount || 0), 0),
...
function calculateProfitLossMetrics(
    ...
    const totalOperatingExpenses = operatingExpenses.reduce((sum, section) => sum + section.subtotal, 0);
    ...
    // Comparative metrics
    const comparativeTotalRevenue = revenue.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
    const comparativeCostOfSales = costOfSales.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
    const comparativeOperatingExpenses = operatingExpenses.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
    const comparativeOtherIncome = otherIncome.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
    const comparativeOtherExpenses = otherExpenses.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
    ...
    const comparativeNetIncome = comparativeOperatingIncome + comparativeOtherIncome - comparativeOtherExpenses;
...
function calculateSectionVariancePercent(accounts: ProfitLossAccount[]): number {
    ...
    const comparativeTotal = accounts.reduce((sum, acc) => sum + (acc.comparativePeriodAmount || 0), 0);
...
function validateProfitLossInput(input: ProfitLossInput): {
    ...
    if (input.reportFormat && !['STANDARD', 'COMPARATIVE', 'MULTI_PERIOD', 'DEPARTMENTAL'].includes(input.reportFormat)) {
