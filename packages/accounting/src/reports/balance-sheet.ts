    ...
    for (const account of trialBalanceAccounts) {
        switch (account.accountType.toUpperCase()) {
            case 'REVENUE':
                ...
                totalRevenue += account.normalBalance === 'CREDIT'
                    ? account.closingBalance
                    : -account.closingBalance;
                break;
            case 'EXPENSE':
                // Expense accounts have debit normal balance
                totalExpenses += account.normalBalance === 'DEBIT'
                    ? account.closingBalance
                    : -account.closingBalance;
                break;
        }
...
function classifyBalanceSheetAccounts(
    ...
    const liabilityAccounts = balanceSheetAccounts.filter(a => a.accountType.toUpperCase() === 'LIABILITY');
    ...
    const buildBalanceSheetAccounts = (accounts: TrialBalanceAccount[]): BalanceSheetAccount[] => {
        return accounts.map(account => {
            ...
            const variancePercent = comparativeBalance !== 0
...
function createAssetSections(assetAccounts: BalanceSheetAccount[]): BalanceSheetSection[] {
    ...
    const currentAssets = assetAccounts.filter(account =>
        isCurrentAsset(account.accountCategory)
    );
    ...
    if (currentAssets.length > 0) {
        sections.push({
            ...
            comparativeSubtotal: currentAssets.reduce((sum, acc) => sum + (acc.comparativeBalance || 0), 0)
        });
    }
    ...
    if (nonCurrentAssets.length > 0) {
        sections.push({
            ...
            comparativeSubtotal: nonCurrentAssets.reduce((sum, acc) => sum + (acc.comparativeBalance || 0), 0)
        });
    }
...
function createLiabilitySection(liabilityAccounts: BalanceSheetAccount[]): BalanceSheetSection[] {
    ...
    if (currentLiabilities.length > 0) {
        sections.push({
            ...
            comparativeSubtotal: currentLiabilities.reduce((sum, acc) => sum + (acc.comparativeBalance || 0), 0)
        });
    }
    ...
    if (nonCurrentLiabilities.length > 0) {
        sections.push({
            ...
            sectionType: 'LIABILITY',
            accounts: nonCurrentLiabilities.sort((a, b) => a.accountNumber.localeCompare(b.accountNumber)),
            subtotal: nonCurrentLiabilities.reduce((sum, acc) => sum + acc.currentBalance, 0),
            comparativeSubtotal: nonCurrentLiabilities.reduce((sum, acc) => sum + (acc.comparativeBalance || 0), 0)
        });
    }
...
function createEquitySection(
    ...
    const retainedEarningsAccount: BalanceSheetAccount = {
        ...
        variance: comparativeRetainedEarnings ? retainedEarnings - comparativeRetainedEarnings : undefined,
        variancePercent: comparativeRetainedEarnings && comparativeRetainedEarnings !== 0
            ? ((retainedEarnings - comparativeRetainedEarnings) / Math.abs(comparativeRetainedEarnings)) * 100
            : undefined
    };

    const allEquityAccounts = [...equityAccounts, retainedEarningsAccount]
    ...
    return [{
        ...
        comparativeSubtotal: allEquityAccounts.reduce((sum, acc) => sum + (acc.comparativeBalance || 0), 0)
    }];
}
...
function calculateBalanceSheetTotals(
    ...
    const totalNonCurrentAssets = assets.find(s => s.sectionName === 'Non-Current Assets')?.subtotal || 0;

    const totalLiabilities = liabilities.reduce((sum, section) => sum + section.subtotal, 0);
    const totalCurrentLiabilities = liabilities.find(s => s.sectionName === 'Current Liabilities')?.subtotal || 0;
    const totalNonCurrentLiabilities = liabilities.find(s => s.sectionName === 'Non-Current Liabilities')?.subtotal || 0;

    const totalEquity = equity.reduce((sum, section) => sum + section.subtotal, 0);
    const retainedEarnings = equity[0]?.accounts.find(a => a.accountId === 'retained-earnings')?.currentBalance || 0;
    ...
    if (comparativeAccounts) {
        comparativeTotalAssets = assets.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
        comparativeTotalLiabilities = liabilities.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
        comparativeTotalEquity = equity.reduce((sum, section) => sum + (section.comparativeSubtotal || 0), 0);
    }
    ...
    return {
        ...
        comparativeTotalAssets: comparativeTotalAssets ? Math.round(comparativeTotalAssets * 100) / 100 : undefined,
        comparativeTotalLiabilities: comparativeTotalLiabilities ? Math.round(comparativeTotalLiabilities * 100) / 100 : undefined,
        comparativeTotalEquity: comparativeTotalEquity ? Math.round(comparativeTotalEquity * 100) / 100 : undefined
    };
}
...
function validateBalanceSheetInput(input: BalanceSheetInput): {
    ...
    if (input.reportFormat && !['STANDARD', 'COMPARATIVE', 'CONSOLIDATED'].includes(input.reportFormat)) {
