...
export async function generateCashFlow(
  ...
  try {
    ...
    const metadata = {
      totalActivities:
        operatingActivities.activities.length +
...
async function getCashFlowActivities(
  ...
  current: Map<
    string,
    { debits: number; credits: number; netActivity: number; accountType: string }
  >;
  comparative?: Map<
    string,
    { debits: number; credits: number; netActivity: number; accountType: string }
  ...
  if (currentError) {
    throw new Error(
      `Failed to fetch current period cash flow activities: ${(currentError as { message: string }).message}`,
    );
  }
  ...
  let comparativeActivity:
    | Map<string, { debits: number; credits: number; netActivity: number; accountType: string }>
    | undefined = undefined;
  if (comparativePeriod) {
    ...
    if (comparativeError) {
      throw new Error(
        `Failed to fetch comparative period cash flow activities: ${(comparativeError as { message: string }).message}`,
      );
    }
...
function classifyCashFlowActivities(
  currentActivities: Map<string, unknown>,
  comparativeActivities?: Map<string, unknown>,
  ...
  for (const [accountId, activity] of currentActivities) {
    ...
    const comparativeActivityData = comparativeActivity as
      | { debits: number; credits: number; netActivity: number; accountType: string }
      | undefined;
    const currentAmount = activityData.netActivity || 0;
    const comparativeAmount = comparativeActivityData?.netActivity || 0;
    ...
    const cashFlowActivity: CashFlowActivity = {
      ...
      activityType: activityData.accountType,
    ...
    if (isOperatingActivity(activityData.accountType)) {
      ...
      isInvestingActivity(
        activityData.accountType,
        (activityData as { accountCategory?: string }).accountCategory,
      )
    ) {
  ...
  return {
    operatingActivities: {
      ...
      comparativeSubtotal: operatingActivitiesList.reduce(
        (sum, act) => sum + (act.comparativePeriodAmount || 0),
        0,
      ),
    ...
    investingActivities: {
      ...
      comparativeSubtotal: investingActivitiesList.reduce(
        (sum, act) => sum + (act.comparativePeriodAmount || 0),
        0,
      ),
    ...
    financingActivities: {
      ...
      comparativeSubtotal: financingActivitiesList.reduce(
        (sum, act) => sum + (act.comparativePeriodAmount || 0),
        0,
      ),
...
async function calculateCashBalances(
  ...
  const { data: beginningData } = await (
    dbClient as {
      query: (sql: string, params?: unknown[]) => Promise<{ data: unknown; error: unknown }>;
    }
  ...
  const { data: endingData } = await (
    dbClient as {
      query: (sql: string, params?: unknown[]) => Promise<{ data: unknown; error: unknown }>;
    }
  ).query(cashBalanceQuery.replace("< $3", "<= $3"), [tenantId, companyId, endDate]);

  const beginningCashBalance = parseFloat(
    (beginningData as Array<{ balance: string }>)?.[0]?.balance || "0",
  );
  const endingCashBalance = parseFloat(
    (endingData as Array<{ balance: string }>)?.[0]?.balance || "0",
  );
  ...
  if (comparativePeriod) {
    ...
    comparativeBeginningCashBalance = parseFloat(
      (compBeginningData as Array<{ balance: string }>)?.[0]?.balance || "0",
    );
    comparativeEndingCashBalance = parseFloat(
      (compEndingData as Array<{ balance: string }>)?.[0]?.balance || "0",
    );
  }
...
function calculateCashFlowMetrics(
  ...
  const comparativeNetChangeInCash =
    (comparativeNetCashFromOperating || 0) +
  ...
  return {
    ...
    beginningCashBalance:
      Math.round(
        (cashBalances as { beginningCashBalance: number; endingCashBalance: number })
          .beginningCashBalance * 100,
      ) / 100,
    endingCashBalance:
      Math.round(
        (cashBalances as { beginningCashBalance: number; endingCashBalance: number })
          .endingCashBalance * 100,
      ) / 100,

    comparativeNetCashFromOperating: comparativeNetCashFromOperating
      ? Math.round(comparativeNetCashFromOperating * 100) / 100
      : undefined,
    comparativeNetCashFromInvesting: comparativeNetCashFromInvesting
      ? Math.round(comparativeNetCashFromInvesting * 100) / 100
      : undefined,
    comparativeNetCashFromFinancing: comparativeNetCashFromFinancing
      ? Math.round(comparativeNetCashFromFinancing * 100) / 100
      : undefined,
    comparativeNetChangeInCash: comparativeNetChangeInCash
      ? Math.round(comparativeNetChangeInCash * 100) / 100
      : undefined,
...
async function generateReconciliation(
  ...
  cashFlowActivities: Map<string, unknown>,
  ...
  const workingCapitalChanges = await calculateWorkingCapitalChanges(
...
async function calculateNonCashAdjustments(
  trialBalanceAccounts: TrialBalanceAccount[],
  _cashFlowActivities: Map<string, unknown>,
  ...
  for (const account of trialBalanceAccounts) {
    if (
      account.accountName?.toLowerCase().includes("gain") &&
      account.accountName?.toLowerCase().includes("disposal")
    ) {
      disposalGainLoss -=
        account.normalBalance === "CREDIT" ? account.closingBalance : -account.closingBalance;
    } else if (
      account.accountName?.toLowerCase().includes("loss") &&
      account.accountName?.toLowerCase().includes("disposal")
    ) {
      disposalGainLoss +=
        account.normalBalance === "DEBIT" ? account.closingBalance : -account.closingBalance;
...
async function calculateWorkingCapitalChanges(
  ...
  const workingCapitalChanges: Array<{
    description: string;
    amount: number;
    type: "INCREASE" | "DECREASE";
  }> = [];
  ...
  let prepaidChange = 0;
  for (const account of trialBalanceAccounts) {
    if (account.accountType === "ASSET" && account.accountName?.toLowerCase().includes("prepaid")) {
...
function calculateSectionVariancePercent(activities: CashFlowActivity[]): number {
  ...
  const comparativeTotal = activities.reduce(
    (sum, act) => sum + (act.comparativePeriodAmount || 0),
    0,
  );
