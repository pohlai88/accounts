...
export class DeploymentManager {
    ...
    private async executeStep(step: DeploymentStep): Promise<DeploymentResult["steps"][0]> {
        ...
        try {
            ...
            const duration = Date.now() - stepStartTime;
            console.log(`❌ ${step.name} failed after ${duration}ms`);
    ...
    private async restoreBackup(): Promise<void> {
        try {
            ...
            console.log(`✅ Backup restored: ${latestBackup.id}`);
        } catch (error) {
    ...
    private async runMigrations(): Promise<void> {
        if (!this.config.migrationEnabled) {
            console.log("⚠️ Migrations disabled, skipping...");
            return;
        }
    ...
    private async deployWithoutDocker(): Promise<void> {
        try {
            ...
            console.log("📦 Simulating non-Docker deployment...");
            ...
            console.log("✅ Non-Docker deployment simulation completed");
        } catch (error) {
    ...
    private async rollbackApplication(): Promise<void> {
        try {
            if (this.config.dockerEnabled) {
                ...
                console.log("🔄 Rolling back non-Docker deployment...");
    ...
    private async sendNotifications(): Promise<void> {
        if (!this.config.notificationEnabled) {
            console.log("⚠️ Notifications disabled, skipping...");
            return;
        }
