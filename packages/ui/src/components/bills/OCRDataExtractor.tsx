...
import { Upload, FileText, Eye, CheckCircle, AlertCircle, Loader2, Download, RefreshCw } from 'lucide-react';
...
export const OCRDataExtractor: React.FC<OCRDataExtractorProps> = ({
    ...
    const processOCR = async (file: File): Promise<OCRProcessingResult> => {
        ...
        const mockData: OCRExtractedData = {
            vendor: file.name.includes('amazon') ? 'Amazon Web Services' :
                file.name.includes('office') ? 'Office Depot' :
                    file.name.includes('utilities') ? 'City Utilities' : 'Vendor Name',
        ...
        <div className={cn('space-y-6', className)}>
            ...
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-sys-text-tertiary">Upload bills and receipts for automatic data extraction</p>
                </div>
            </div>
                                ...
                                {result.status === 'completed' && result.extractedData && (
                                    <div className="bg-sys-fill-low rounded-lg p-4 space-y-3">
                                                ...
                                                <div className="space-y-1">
                                                    {result.extractedData.lineItems.map((item, index) => (
                                                        <div key={index} className="flex justify-between text-sm">
                                                            ...
                                                            <span className="text-sys-text-primary">${item.total.toFixed(2)}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex space-x-2">
                                            <button
                                                onClick={() => result.extractedData && onDataExtracted?.(result.extractedData)}
            ...
            <div className="bg-sys-fill-low border border-sys-border-hairline rounded-lg p-4">
                <h4 className="text-sm font-medium text-sys-text-primary mb-2">Tips for better OCR results:</h4>
