...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    ...
    >
      {icon && (
        <div
          className="flex h-16 w-16 items-center justify-center rounded-full bg-sys-fill-low text-sys-text-tertiary"
          aria-hidden="true"
        >
          {icon}
        </div>
      ...
      <div className="space-y-2">
        <h3 id="empty-state-title" className="text-lg font-semibold text-sys-text-primary">
          {title}
        </h3>
        <p id="empty-state-description" className="text-sm text-sys-text-secondary max-w-md">
          {description}
        </p>
      </div>
