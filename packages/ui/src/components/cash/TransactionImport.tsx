...
const mockCategories = [
  'Revenue', 'Rent', 'Technology', 'Payroll', 'Meals & Entertainment',
  'Travel', 'Office Supplies', 'Marketing', 'Legal', 'Insurance'
];
...
export function TransactionImport({
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'matched' | 'unmatched' | 'uncategorized'>('all');
  ...
  const handleCategorize = (transactionId: string, category: string) => {
    ...
    if (updatedTransaction && onTransactionUpdated) {
      onTransactionUpdated({ ...updatedTransaction, category, isCategorized: true, confidence: 1.0 });
    }
  };
  ...
  const getStatusBadge = (transaction: Transaction) => {
    if (transaction.isMatched && transaction.isCategorized) {
      return <Badge className="bg-sys-green-100 text-sys-green-800 border-sys-green-200">Complete</Badge>;
    }
    if (transaction.isMatched) {
      return <Badge className="bg-sys-blue-100 text-sys-blue-800 border-sys-blue-200">Matched</Badge>;
    }
    if (transaction.isCategorized) {
      return <Badge className="bg-sys-yellow-100 text-sys-yellow-800 border-sys-yellow-200">Categorized</Badge>;
    }
        ...
        <CardContent>
          ...
          {selectedTransactions.size > 0 && (
            <div className="flex items-center gap-4 mb-4 p-3 bg-sys-brand-50 border border-sys-brand-200 rounded-md">
              ...
              <div className="flex gap-2">
                <select
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleBulkCategorize(e.target.value)}
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {transaction.confidence > 0 && (
                          <span className={cn("text-xs", getConfidenceColor(transaction.confidence))}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!transaction.isCategorized && (
                        <select
                          ...
                          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleCategorize(transaction.id, e.target.value)}
