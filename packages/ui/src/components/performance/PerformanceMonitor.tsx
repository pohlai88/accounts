...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'good': return 'text-[var(--sys-status-success)] bg-[var(--sys-status-success)]/10';
    ...
    const formatMetric = (metric: keyof PerformanceMetrics, value: number) => {
        switch (metric) {
            ...
            case 'bundleSize': return `${(value / 1024).toFixed(1)}KB`;
            case 'memoryUsage': return `${value}%`;
            case 'cpuUsage': return `${value}%`;
            case 'databaseQueries': return value.toLocaleString();
            case 'cacheHitRate': return `${value}%`;
            case 'errorRate': return `${value}%`;
            case 'uptime': return `${value}%`;
            case 'activeUsers': return value.toLocaleString();
                        ...
                        <Card key={key} className="p-4">
                            <CardContent className="p-0">
                                <div className="flex items-center justify-between mb-2">
                                    <div className={cn("flex items-center space-x-2 px-2 py-1 rounded-full text-xs font-medium", statusColor)}>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            ...
                            <div className="w-full bg-[var(--sys-fill-low)] rounded-full h-2">
                                <div
                                    className={cn(
                                        ...
                                        metrics.memoryUsage > 85 ? "bg-[var(--sys-status-error)]" :
                                            metrics.memoryUsage > 70 ? "bg-[var(--sys-status-warning)]" :
                            ...
                            <div className="text-xs text-[var(--sys-text-tertiary)]">
                                {metrics.memoryUsage > 85 ? 'High memory usage detected' :
                                    metrics.memoryUsage > 70 ? 'Memory usage is elevated' :
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="w-full bg-[var(--sys-fill-low)] rounded-full h-2">
                                <div
                                    className={cn(
                                        "h-2 rounded-full transition-all duration-300",
                                        metrics.cpuUsage > 80 ? "bg-[var(--sys-status-error)]" :
                                            metrics.cpuUsage > 60 ? "bg-[var(--sys-status-warning)]" :
                                                "bg-[var(--sys-status-success)]"
                                    )}
                            ...
                            <div className="text-xs text-[var(--sys-text-tertiary)]">
                                {metrics.cpuUsage > 80 ? 'High CPU usage detected' :
                                    metrics.cpuUsage > 60 ? 'CPU usage is elevated' :
                                        'CPU usage is normal'}
                            </div>
                        </div>
                    </CardContent>
