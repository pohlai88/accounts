...
export const LoadTesting: React.FC<LoadTestingProps> = ({
    ...
    const getTestTypeColor = (type: string) => {
        switch (type) {
            case 'stress': return 'text-[var(--sys-status-error)] bg-[var(--sys-status-error)]/10';
            case 'spike': return 'text-[var(--sys-status-warning)] bg-[var(--sys-status-warning)]/10';
            case 'volume': return 'text-[var(--sys-status-info)] bg-[var(--sys-status-info)]/10';
            case 'endurance': return 'text-[var(--sys-status-success)] bg-[var(--sys-status-success)]/10';
            default: return 'text-[var(--sys-text-tertiary)] bg-[var(--sys-fill-low)]';
        }
    };
    ...
    const getPlanStatusColor = (status: string) => {
        switch (status) {
            case 'implemented': return 'text-[var(--sys-status-success)] bg-[var(--sys-status-success)]/10';
            case 'approved': return 'text-[var(--sys-status-warning)] bg-[var(--sys-status-warning)]/10';
            case 'draft': return 'text-[var(--sys-text-tertiary)] bg-[var(--sys-fill-low)]';
            default: return 'text-[var(--sys-text-tertiary)] bg-[var(--sys-fill-low)]';
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-[var(--sys-text-primary)]">Load Testing & Capacity Planning</h2>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {filteredTests.map((test) => (
                                <div key={test.id} className="p-4 border border-[var(--sys-border-hairline)] rounded-lg hover:bg-[var(--sys-fill-low)] transition-colors">
                                    <div className="flex items-center justify-between">
                                        <div className="flex-1">
                                            <div className="flex items-center space-x-2 mb-2">
                                                <div className={cn("px-2 py-1 rounded-full text-xs font-medium", getTestTypeColor(test.type))}>
                                                    {test.type.toUpperCase()}
                                                </div>
                                                <div className={cn("px-2 py-1 rounded-full text-xs font-medium", getTestStatusColor(test.status))}>
                                            ...
                                            <p className="text-sm text-[var(--sys-text-secondary)] mb-2">
                                                ...
                                                Virtual Users: {formatNumber(test.virtualUsers)} |
                                                RPS: {formatNumber(test.requestsPerSecond)}
                                            </p>
                                            {test.results && (
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                                                    <div>
                                                        <span className="text-[var(--sys-text-tertiary)]">Avg Response:</span>
                                                        <span className="ml-1 font-medium">{test.results.averageResponseTime}ms</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-[var(--sys-text-tertiary)]">Throughput:</span>
                                                        <span className="ml-1 font-medium">{formatNumber(test.results.throughput)} req/s</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-[var(--sys-text-tertiary)]">Error Rate:</span>
                                                        <span className="ml-1 font-medium">{test.results.errorRate.toFixed(2)}%</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-[var(--sys-text-tertiary)]">Success Rate:</span>
                                                        <span className="ml-1 font-medium">{((test.results.successfulRequests / test.results.totalRequests) * 100).toFixed(1)}%</span>
                                                    </div>
                                                </div>
                                            )}
                ...
                <CardContent>
                    <div className="space-y-4">
                        {capacityPlans.map((plan) => (
                            <div key={plan.id} className="p-4 border border-[var(--sys-border-hairline)] rounded-lg hover:bg-[var(--sys-fill-low)] transition-colors">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => onUpdateCapacityPlan(plan.id)}
                                    >
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <h4 className="font-medium text-[var(--sys-text-primary)]">Load Testing Best Practices</h4>
                            <div className="space-y-3">
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Start with baseline tests</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Establish performance baselines before optimization</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Test realistic scenarios</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Use production-like data and user patterns</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Monitor system resources</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Track CPU, memory, and database performance</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Test failure scenarios</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Simulate component failures and recovery</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Monitor growth trends</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Track usage patterns and growth rates</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Plan for peak loads</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Account for seasonal and event-driven spikes</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Consider auto-scaling</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Implement dynamic resource allocation</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Regular reviews</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Update plans based on actual usage</p>
                                    </div>
                                </div>
                            </div>
                        </div>
