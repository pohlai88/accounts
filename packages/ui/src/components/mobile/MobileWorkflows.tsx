...
export const MobileWorkflows: React.FC<MobileWorkflowsProps> = ({
    ...
    const getCategoryColor = (category: string) => {
        switch (category) {
            case 'invoice': return 'text-[var(--sys-status-success)] bg-[var(--sys-status-success)]/10';
            case 'bill': return 'text-[var(--sys-status-warning)] bg-[var(--sys-status-warning)]/10';
            case 'payment': return 'text-[var(--sys-status-info)] bg-[var(--sys-status-info)]/10';
            case 'report': return 'text-[var(--sys-accent)] bg-[var(--sys-accent)]/10';
            case 'customer': return 'text-[var(--sys-text-primary)] bg-[var(--sys-fill-low)]';
            case 'vendor': return 'text-[var(--sys-text-secondary)] bg-[var(--sys-fill-low)]';
    ...
    const getDifficultyColor = (difficulty: string) => {
        switch (difficulty) {
            case 'easy': return 'text-[var(--sys-status-success)] bg-[var(--sys-status-success)]/10';
            case 'medium': return 'text-[var(--sys-status-warning)] bg-[var(--sys-status-warning)]/10';
            case 'hard': return 'text-[var(--sys-status-error)] bg-[var(--sys-status-error)]/10';
            default: return 'text-[var(--sys-text-tertiary)] bg-[var(--sys-fill-low)]';
        }
    };
    ...
    const categories = [
        ...
        { id: 'invoice', name: 'Invoices', count: workflows.filter(w => w.category === 'invoice').length },
        { id: 'bill', name: 'Bills', count: workflows.filter(w => w.category === 'bill').length },
        { id: 'payment', name: 'Payments', count: workflows.filter(w => w.category === 'payment').length },
        { id: 'report', name: 'Reports', count: workflows.filter(w => w.category === 'report').length },
        { id: 'customer', name: 'Customers', count: workflows.filter(w => w.category === 'customer').length },
        ...
        <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="text-[var(--sys-text-primary)]">
                                        {activeWorkflowData.steps.filter(s => s.isCompleted).length} / {activeWorkflowData.steps.length} steps
                                    </span>
                                </div>
                            ...
                            <div className="space-y-3">
                                {activeWorkflowData.steps.map((step, index) => {
                                    ...
                                    return (
                                        <div key={step.id} className="flex items-center space-x-3 p-3 border border-[var(--sys-border-hairline)] rounded-lg">
                                            <div className={cn(
                                                "flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium",
                                                step.isCompleted ? "bg-[var(--sys-status-success)] text-white" :
                                                    index === activeWorkflowData.steps.findIndex(s => !s.isCompleted) ? "bg-[var(--sys-accent)] text-white" :
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-sm text-[var(--sys-text-secondary)]">{step.description}</p>
                                            </div>
                            ...
                            <div className="flex items-center space-x-3 pt-4 border-t border-[var(--sys-border-hairline)]">
                                ...
                                <Button
                                    onClick={() => onViewWorkflow(activeWorkflowData.id)}
                                    variant="outline"
                                >
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <div className={cn("px-2 py-1 rounded-full text-xs font-medium", getCategoryColor(workflow.category))}>
                                    ...
                                    <div>
                                        <h4 className="font-medium text-[var(--sys-text-primary)] mb-1">{workflow.name}</h4>
                                        <p className="text-sm text-[var(--sys-text-secondary)] line-clamp-2">{workflow.description}</p>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <div className={cn("px-1 py-0.5 rounded text-xs font-medium", getDifficultyColor(workflow.difficulty))}>
                                                {workflow.difficulty}
                                            </div>
                                        </div>
                                        <Button
            ...
            {filteredWorkflows.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-[var(--sys-text-primary)] mb-2">No Workflows Found</h3>
                    <p className="text-[var(--sys-text-secondary)]">
                        No workflows match your current filters. Try adjusting your search or category selection.
                    </p>
                </div>
            )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Large Touch Targets</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Minimum 44px touch targets for easy tapping</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <MousePointer className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Swipe Gestures</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Swipe left/right for quick actions</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <Move className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Pinch to Zoom</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Zoom in on detailed content</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Local Storage</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Data cached locally for offline access</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Changes queued for sync when online</p>
                                    </div>
                                </div>
                                <div className="flex items-start space-x-3">
                                    <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)] mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-[var(--sys-text-primary)]">Conflict Resolution</p>
                                        <p className="text-xs text-[var(--sys-text-secondary)]">Smart conflict detection and resolution</p>
                                    </div>
                                </div>
                            </div>
                        </div>
