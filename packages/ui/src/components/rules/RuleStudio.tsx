...
interface RuleAction {
    id: string;
    type: 'send_email' | 'create_task' | 'update_field' | 'send_webhook' | 'assign_user' | 'create_alert';
...
interface RuleStudioProps {
    ...
    onTestRule: (rule: Rule, testData: any) => Promise<{ success: boolean; result: any; executionTime: number; cost: number }>;
...
export const RuleStudio: React.FC<RuleStudioProps> = ({
    ...
    const getActionLabel = (type: string) => {
        switch (type) {
            ...
            case 'create_alert': return 'Create Alert';
            default: return type;
        }
    };

    const handleTestRule = useCallback(async (rule: Rule) => {
        setIsTesting(true);
        try {
            const testData = {
    ...
    const handleExplainRule = useCallback((rule: Rule) => {
        const context = {
                                ...
                                {filteredRules.map((rule) => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            selectedRule?.id === rule.id ? "bg-[var(--sys-fill-low)] border-[var(--sys-accent)]" : "hover:bg-[var(--sys-fill-low)]"
                                        )}
                                    ...
                                    >
                                        <div className="flex items-start justify-between mb-2">
                                            ...
                                            <div className="flex items-center space-x-1">
                                                ...
                                                <span className="text-xs text-[var(--sys-text-tertiary)]">v{rule.version}</span>
                                            </div>
                                        </div>
                                        <p className="text-sm text-[var(--sys-text-secondary)] mb-2">{rule.description}</p>
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-3">
                                                {selectedRule.conditions.map((condition, index) => (
                                                    <div key={condition.id} className="flex items-center space-x-3 p-3 border border-[var(--sys-border-hairline)] rounded-lg">
                                                        {index > 0 && (
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-3">
                                                {selectedRule.actions.map((action) => (
                                                    <div key={action.id} className="flex items-center space-x-3 p-3 border border-[var(--sys-border-hairline)] rounded-lg">
                                            ...
                                            <div className="space-y-3">
                                                <Alert variant={testResults.success ? 'default' : 'destructive'}>
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <span>
                                                            Test {testResults.success ? 'passed' : 'failed'} in {testResults.executionTime}ms
                                                        </span>
                                                    </div>
                                                </Alert>
                                                <div className="bg-[var(--sys-fill-low)] p-4 rounded-lg">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                {[1, 2, 3].map((i) => (
                                                    <div key={i} className="flex items-center justify-between p-3 border border-[var(--sys-border-hairline)] rounded-lg">
                                                        <div className="flex items-center space-x-3">
                        ...
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                ...
                                <p className="text-[var(--sys-text-secondary)] text-center mb-4">
                                    Select a rule from the list to view and edit its details, or create a new rule to get started.
                                </p>
                                <Button
                                    onClick={() => setSelectedRule(null)}
                                    className="flex items-center space-x-2"
