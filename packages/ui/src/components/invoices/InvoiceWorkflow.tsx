...
type WorkflowStep = 'list' | 'create' | 'edit' | 'view' | 'payments' | 'send' | 'reminders' | 'timeline';
...
export const InvoiceWorkflow: React.FC<InvoiceWorkflowProps> = ({
    ...
    const getStepIcon = (step: WorkflowStep) => {
        switch (step) {
            ...
            case 'reminders': return MessageSquare;
    ...
    const getStepTitle = (step: WorkflowStep) => {
        switch (step) {
            ...
            case 'view': return 'Invoice Details';
            case 'payments': return 'Payment Capture';
            case 'send': return 'Send Invoice';
            case 'reminders': return 'Payment Reminders';
            case 'timeline': return 'Invoice Timeline';
                            ...
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex items-center space-x-2">
                                    <span className={cn(
                                        ...
                                        selectedInvoice.status === 'paid' ? 'text-sys-status-success bg-sys-status-success/10' :
                                            selectedInvoice.status === 'sent' ? 'text-sys-status-info bg-sys-status-info/10' :
                                                selectedInvoice.status === 'overdue' ? 'text-sys-status-error bg-sys-status-error/10' :
