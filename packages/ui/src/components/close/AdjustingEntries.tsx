...
import { Edit, Trash2, RotateCcw, FileText, AlertTriangle, CheckCircle, Clock, Plus, Minus } from "lucide-react";
...
export const AdjustingEntries: React.FC<AdjustingEntriesProps> = ({
    ...
    const getStatusIcon = (status: AdjustingEntry['status']) => {
        switch (status) {
            ...
            case 'reversed': return <RotateCcw className="h-4 w-4 text-[var(--sys-status-error)]" />;
            case 'draft': return <Clock className="h-4 w-4 text-[var(--sys-text-tertiary)]" />;
    ...
    const getStatusColor = (status: AdjustingEntry['status']) => {
        switch (status) {
            case 'posted': return 'bg-[var(--sys-status-success)] text-white';
            case 'reversed': return 'bg-[var(--sys-status-error)] text-white';
            case 'draft': return 'bg-[var(--sys-bg-subtle)] text-[var(--sys-text-secondary)]';
    ...
    const handleAddLine = async (entryId: string) => {
        if (onAddLine && newLineAccount.trim() && newLineDescription.trim() && (newLineDebit > 0 || newLineCredit > 0)) {
        ...
        <div className={cn("bg-[var(--sys-bg-primary)] border border-[var(--sys-border-hairline)] rounded-lg", className)}>
            ...
            <div className="p-6 border-b border-[var(--sys-border-hairline)]">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                    ...
                    <div className="bg-[var(--sys-bg-subtle)] p-4 rounded-lg">
                        <div className="text-2xl font-bold text-[var(--sys-text-tertiary)]">
                            {totals.draft}
                        </div>
