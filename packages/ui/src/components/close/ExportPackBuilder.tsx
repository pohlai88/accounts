...
export const ExportPackBuilder: React.FC<ExportPackBuilderProps> = ({
  ...
  const getDocumentTypeIcon = (type: ExportDocument["type"]) => {
    switch (type) {
      case "financial_statement":
        return <FileText className="h-4 w-4 text-[var(--sys-accent)]" />;
      case "trial_balance":
        return <FileText className="h-4 w-4 text-[var(--sys-status-success)]" />;
      case "general_ledger":
        return <FileText className="h-4 w-4 text-[var(--sys-status-warning)]" />;
      case "supporting_schedule":
      ...
      case "custom":
        return <Settings className="h-4 w-4 text-[var(--sys-text-tertiary)]" />;
      default:
  ...
  const hasRequiredDocuments = availableDocuments
    .filter(d => d.required)
  ...
  return (
    ...
    >
        ...
        {currentPack && (
          <div className="bg-[var(--sys-bg-subtle)] p-4 rounded-lg mb-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-medium text-[var(--sys-text-primary)]">{currentPack.name}</h3>
              <span
                className={cn(
                  "px-2 py-1 text-xs font-medium rounded-full",
                  currentPack.status === "ready"
                    ? "bg-[var(--sys-status-success)] text-white"
                    : currentPack.status === "generating"
                      ? "bg-[var(--sys-status-warning)] text-white"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="text-[var(--sys-text-primary)]">
                  {formatFileSize(currentPack.totalSize)}
                </div>
              </div>
              <div>
      ...
      <div className="p-6 border-b border-[var(--sys-border-hairline)]">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-[var(--sys-text-primary)]">Select Documents</h3>
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={
                selectedDocuments.size === availableDocuments.length &&
        ...
        <div className="space-y-3">
          {availableDocuments.map(document => (
            <div
              key={document.id}
              className="flex items-center gap-4 p-3 bg-[var(--sys-bg-subtle)] rounded-lg"
            >
              <input
                type="checkbox"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-[var(--sys-text-secondary)]">{document.description}</p>
