...
import { CheckCircle, Circle, AlertTriangle, Clock, User, Calendar, FileText, Lock } from "lucide-react";
...
export const CloseChecklist: React.FC<CloseChecklistProps> = ({
    ...
    const getStatusIcon = (status: ChecklistItem['status']) => {
        switch (status) {
            ...
            case 'skipped': return <AlertTriangle className="h-5 w-5 text-[var(--sys-text-tertiary)]" />;
            default: return <Circle className="h-5 w-5 text-[var(--sys-text-tertiary)]" />;
        }
    };
    ...
    const isOverdue = (dueDate: string, itemId: string) => {
        return new Date(dueDate) < new Date() && items.find(i => i.id === itemId)?.status !== 'completed';
    };
            ...
            <div className="p-6 border-b border-[var(--sys-border-hairline)]">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <label className="text-sm text-[var(--sys-text-secondary)]">
                            Show completed
                        </label>
                    </div>
                </div>
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-2">
                                    <h3 className={cn(
                                        "font-medium",
                                        item.status === 'completed' ? "text-[var(--sys-text-secondary)] line-through" : "text-[var(--sys-text-primary)]"
                                    )}>
                                        {item.title}
                                        {item.required && (
                                ...
                                <p className={cn(
                                    "text-sm mb-3",
                                    item.status === 'completed' ? "text-[var(--sys-text-tertiary)]" : "text-[var(--sys-text-secondary)]"
                                )}>
                                    {item.description}
                                </p>
                            ...
                            <div className="flex items-center gap-2">
                                <select
                                    value={item.status}
                                    onChange={(e) => handleItemStatusChange(item.id, e.target.value as ChecklistItem['status'])}
