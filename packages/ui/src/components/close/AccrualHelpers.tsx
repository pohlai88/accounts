...
import { Calculator, Calendar, TrendingUp, TrendingDown, FileText, AlertTriangle, CheckCircle, Clock } from "lucide-react";
...
export const AccrualHelpers: React.FC<AccrualHelpersProps> = ({
    ...
    const accrualTotals = React.useMemo(() => {
        ...
        const postedAmount = accruals.filter(a => a.status === 'posted').reduce((sum, a) => sum + a.amount, 0);
    ...
    const depreciationTotals = React.useMemo(() => {
        ...
        const postedDepreciation = depreciations.filter(d => d.status === 'posted').reduce((sum, d) => sum + d.calculatedDepreciation, 0);
    ...
    const getAccrualTypeIcon = (type: AccrualEntry['type']) => {
        switch (type) {
            case 'revenue': return <TrendingUp className="h-4 w-4 text-[var(--sys-status-success)]" />;
            case 'expense': return <TrendingDown className="h-4 w-4 text-[var(--sys-status-error)]" />;
            case 'prepaid': return <Calendar className="h-4 w-4 text-[var(--sys-accent)]" />;
            case 'deferred': return <Clock className="h-4 w-4 text-[var(--sys-status-warning)]" />;
    ...
    const getAccrualTypeColor = (type: AccrualEntry['type']) => {
        switch (type) {
            case 'revenue': return 'bg-[var(--sys-status-success)] text-white';
            case 'expense': return 'bg-[var(--sys-status-error)] text-white';
            case 'prepaid': return 'bg-[var(--sys-accent)] text-white';
            case 'deferred': return 'bg-[var(--sys-status-warning)] text-white';
    ...
    const getStatusIcon = (status: AccrualEntry['status'] | DepreciationEntry['status']) => {
        switch (status) {
            case 'posted': return <CheckCircle className="h-4 w-4 text-[var(--sys-status-success)]" />;
            case 'calculated': return <Calculator className="h-4 w-4 text-[var(--sys-status-warning)]" />;
            case 'pending': return <Clock className="h-4 w-4 text-[var(--sys-text-tertiary)]" />;
            case 'reversed': return <AlertTriangle className="h-4 w-4 text-[var(--sys-status-error)]" />;
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                            ...
                                            <div>
                                                <span className="text-[var(--sys-text-tertiary)]">Monthly Depreciation:</span>
                                                <div className="text-[var(--sys-text-primary)]">
                                        ...
                                        <div className="mt-2 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                                            ...
                                            <div>
                                                <span className="text-[var(--sys-text-tertiary)]">Calculated Depreciation:</span>
                                                <div className="text-[var(--sys-text-primary)] font-medium">
