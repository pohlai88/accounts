...
import { Lock, Unlock, Calendar, User, AlertTriangle, CheckCircle, Clock, FileText } from "lucide-react";
...
export interface LockStatesProps {
    ...
    locks: PeriodLock[];
    lockRules: LockRule[];
    onLockPeriod?: (periodId: string, lockType: PeriodLock['lockType'], reason?: string) => Promise<void>;
...
export const LockStates: React.FC<LockStatesProps> = ({
    ...
    const getLockTypeIcon = (lockType: PeriodLock['lockType']) => {
        switch (lockType) {
            case 'hard': return <Lock className="h-4 w-4 text-[var(--sys-status-error)]" />;
            case 'soft': return <Lock className="h-4 w-4 text-[var(--sys-status-warning)]" />;
            case 'readonly': return <FileText className="h-4 w-4 text-[var(--sys-text-secondary)]" />;
    ...
    const getLockTypeColor = (lockType: PeriodLock['lockType']) => {
        switch (lockType) {
            case 'hard': return 'bg-[var(--sys-status-error)] text-white';
            case 'soft': return 'bg-[var(--sys-status-warning)] text-white';
            case 'readonly': return 'bg-[var(--sys-bg-subtle)] text-[var(--sys-text-secondary)]';
    ...
    const getLockTypeDescription = (lockType: PeriodLock['lockType']) => {
        switch (lockType) {
            case 'hard': return 'Complete lock - no modifications allowed';
            case 'soft': return 'Soft lock - limited modifications allowed';
            case 'readonly': return 'Read-only - view only, no modifications';
            default: return 'Unknown lock type';
        }
    };
