...
export function useTenantManagement() {
    ...
    const switchTenant = useCallback(async (tenantId: string) => {
        try {
            ...
            const { data } = await (window as { supabase?: { auth: { refreshSession: () => Promise<{ data: { session: unknown } }> } } }).supabase?.auth.refreshSession() || {};
    ...
    const inviteUser = useCallback(async (tenantId: string, email: string, role: string) => {
        try {
    ...
    // Update member role
    const updateMemberRole = useCallback(async (tenantId: string, userId: string, role: string) => {
        try {
            const response = await fetch(`/api/tenants/${tenantId}/members/${userId}`, {
                method: 'PATCH',
                headers: {
