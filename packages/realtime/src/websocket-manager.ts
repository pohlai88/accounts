...
export class WebSocketManager extends EventEmitter {
     ...
     */
    broadcastToTenant(tenantId: string, message: Omit<WebSocketMessage, 'tenantId' | 'userId' | 'timestamp'>): number {
     ...
     */
    broadcastToChannel(channel: string, message: Omit<WebSocketMessage, 'tenantId' | 'userId' | 'timestamp'>): number {
        let sent = 0;
    ...
    private updateTenantStats(tenantId: string, delta: number): void {
        this.stats.connectionsByTenant[tenantId] = (this.stats.connectionsByTenant[tenantId] || 0) + delta;
