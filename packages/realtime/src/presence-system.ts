...
export class PresenceSystem extends EventEmitter {
  ...
  updatePresence(
    ...
    this.eventSystem.publishTenantEvent(
      tenantId,
      "presence.update",
      {
        userId,
  ...
  getOnlineUsers(tenantId: string): PresenceInfo[] {
    return this.getTenantPresence(tenantId).filter(presence => presence.status === "online");
  }
  ...
  subscribeToPresence(
    ...
    const filters: unknown[] = [{ field: "data.tenantId", operator: "equals", value: tenantId }];
