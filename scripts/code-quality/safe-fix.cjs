...
const TEXT_SIZE_MAP_PX = new Map([
    [12, "xs"], [14, "sm"], [16, "base"], [18, "lg"],
    [20, "xl"], [24, "2xl"], [30, "3xl"], [36, "4xl"],
]);
...
function fixSpacingBrackets(text) {
    ...
    const re = /(^|\s)((?:[a-z-]+:)*)(h|w|min-h|min-w|max-h|max-w|p|px|py|pt|pr|pb|pl|m|mx|my|mt|mr|mb|ml|gap|space-x|space-y)-\[(.*?)\]/g;
...
function collectUnfixables(text) {
    ...
    const COLOR_LIT_RE = /(?:^|\s)((?:[a-z-]+:)*)((?:text|bg|border|ring|outline))-\[#([0-9a-f]{3}|[0-9a-f]{6})\]/gi;
    ...
    const ICON_BUTTON_RE = /<button\b(?![^>]*aria-label=)[^>]*>\s*<[^>]+aria-hidden=["']?true["']?[^>]*>\s*<\/[^>]+>\s*<\/button>/g;
...
function fixIconButtonsAria(text) {
    ...
    return {
        text: text.replace(BUTTON_RE, (full, attrs, inner) => {
            ...
            const iconClassAttr = (inner.match(/class(?:Name)?=["'][^"']*(close|search|menu|edit|delete|trash|remove|settings|cog|gear|filter|download|upload|print|back|previous|next|chevron-left|arrow-left|chevron-right|arrow-right|info|help|question|calendar|share|link|copy|plus|add|minus|expand|collapse|play|pause|stop|refresh)[^"']*["']/i) || [])[1];
