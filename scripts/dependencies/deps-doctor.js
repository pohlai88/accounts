...
function checkNonWorkspaceRefs() {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (existsSync(packageJsonPath)) {
                        ...
                        try {
                            ...
                            for (const depType of dependencyTypes) {
                                if (packageJson[depType] && typeof packageJson[depType] === 'object') {
                                    for (const [depName, depVersion] of Object.entries(packageJson[depType])) {
                                        if (typeof depName === 'string' && depName.startsWith('@aibos/') && depVersion !== 'workspace:*') {
                                            log(`❌ ${relative(rootDir, packageJsonPath)}: ${depName} = "${depVersion}" (should be "workspace:*")`, 'red')
                                            found = true
                                        }
                                    }
                                }
                            ...
                            errors.push(`Parse error in ${relative(rootDir, packageJsonPath)}: ${parseError.message}`)
                        }
                    }
                }
            }
    ...
    if (!found) {
        log(`✅ All @aibos/* dependencies use workspace:* protocol (checked ${totalChecked} files)`, 'green')
    } else {
...
function checkMissingPackages() {
                    ...
                    if (existsSync(packageJsonPath)) {
                        ...
                        try {
                            ...
                            for (const depType of dependencyTypes) {
                                if (packageJson[depType] && typeof packageJson[depType] === 'object') {
                                    for (const [depName, depVersion] of Object.entries(packageJson[depType])) {
                                        if (typeof depName === 'string' && depName.startsWith('@aibos/') && !workspacePackages.has(depName)) {
                                            log(`❌ ${relative(rootDir, packageJsonPath)}: ${depName} (package not found in workspace)`, 'red')
                                            found = true
                                        }
                                    }
                                }
    ...
    if (!found) {
        log(`✅ All @aibos/* references point to existing packages (checked ${totalChecked} files)`, 'green')
    } else {
