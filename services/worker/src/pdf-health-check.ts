...
class PuppeteerHealthMonitor {
    ...
    private async performHealthCheck(): Promise<void> {
        ...
        try {
            ...
            } else if (poolStats.healthyBrowsers < poolStats.totalBrowsers * (this.config.restartThreshold / 100)) {
                status = 'degraded';
                warnings.push(`Only ${poolStats.healthyBrowsers}/${poolStats.totalBrowsers} browsers are healthy`);
            }
    ...
    private analyzePoolHealth(stats: any): { warnings: string[] } {
        ...
        // Check browser age
        if (stats.averageAge > 20) { // 20 minutes
