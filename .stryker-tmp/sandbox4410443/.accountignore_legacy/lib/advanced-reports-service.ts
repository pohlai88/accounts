...
export class AdvancedReportsService {
  ...
  static async getReportExecutions(
    ...
    try {
      let query = supabase.from("report_executions").select("*").eq("company_id", companyId);
  ...
  static async getFinancialRatios(
    ...
    try {
      let query = supabase.from("financial_ratios").select("*").eq("company_id", companyId);
  ...
  static async getAgedReceivables(
    ...
    try {
      let query = supabase.from("aged_receivables").select("*").eq("company_id", companyId);
  ...
  static async getAgedPayables(
    ...
    try {
      let query = supabase.from("aged_payables").select("*").eq("company_id", companyId);
  ...
  static async getCashFlowAnalysis(
    ...
    try {
      let query = supabase.from("cash_flow_analysis").select("*").eq("company_id", companyId);
  ...
  static async getBudgetVsActual(
    ...
    try {
      let query = supabase.from("budget_vs_actual").select("*").eq("company_id", companyId);
      ...
      const { data: budgetVsActual, error } = await query.order("period_start", {
        ascending: false,
      });
  ...
  static async getKPIMetrics(
    ...
    try {
      let query = supabase.from("kpi_metrics").select("*").eq("company_id", companyId);
   ...
   */
  static async getDashboardAnalytics(companyId: string): Promise<
    ApiResponse<{
