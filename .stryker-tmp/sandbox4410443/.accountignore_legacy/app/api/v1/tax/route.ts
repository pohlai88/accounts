...
const taxRuleSchema = z.object({
  ...
  charge_type: z
    .enum(["On Net Total", "On Previous Row Total", "On Previous Row Amount", "Actual"])
...
const taxTemplateSchema = z.object({
  ...
  tax_category: z
    .enum(["In State", "Out of State", "Export", "Import", "Exempt"])
    .default("In State"),
...
const querySchema = z.object({
  ...
  sort_by: z
    .enum(["template_name", "tax_category", "total_rate", "created_at"])
...
async function getTaxTemplates(params: any) {
  ...
  if (params.search) {
    query = query.or(
      `template_name.ilike.%${params.search}%,tax_category.ilike.%${params.search}%`,
    );
  }
