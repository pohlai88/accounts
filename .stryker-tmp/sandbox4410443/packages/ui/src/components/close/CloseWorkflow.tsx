...
export const CloseWorkflow: React.FC<CloseWorkflowProps> = ({
  ...
  const getStepIcon = (status: CloseStep["status"]) => {
    switch (status) {
      ...
      case "blocked":
        return <AlertTriangle className="h-5 w-5 text-[var(--sys-status-error)]" />;
      default:
        return <Clock className="h-5 w-5 text-[var(--sys-text-tertiary)]" />;
    }
  };
    ...
    >
      ...
      <div className="p-6 border-b border-[var(--sys-border-hairline)]">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[var(--sys-text-primary)]">Close Workflow</h1>
            <p className="text-[var(--sys-text-secondary)] mt-1">
              {workflowState.currentPeriod.name} •{" "}
              {new Date(workflowState.currentPeriod.startDate).toLocaleDateString()} -{" "}
              {new Date(workflowState.currentPeriod.endDate).toLocaleDateString()}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[var(--sys-text-secondary)]">
                {workflowState.currentPeriod.status.charAt(0).toUpperCase() +
                  workflowState.currentPeriod.status.slice(1)}
              </span>
            </div>
      ...
      <div className="p-6 border-b border-[var(--sys-border-hairline)] bg-[var(--sys-bg-subtle)]">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[var(--sys-text-primary)]">Workflow Steps</h2>

          <div className="flex items-center gap-2">
            <button
              onClick={handlePrevStep}
              disabled={
                workflowState.steps.findIndex(s => s.id === workflowState.currentStepId) === 0
              }
            ...
            <button
              ...
              disabled={
                workflowState.steps.findIndex(s => s.id === workflowState.currentStepId) ===
                workflowState.steps.length - 1
              }
              className="p-2 text-[var(--sys-text-secondary)] hover:text-[var(--sys-text-primary)] hover:bg-[var(--sys-bg-primary)] rounded disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-[var(--sys-accent)]"
          ...
          {workflowState.steps.map(step => (
            ...
            >
              ...
              <div className="mt-2 text-xs opacity-75">
                ...
                {step.actualHours && <span className="ml-2">Actual: {step.actualHours}h</span>}
              </div>
            </button>
          ))}
        </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-[var(--sys-text-secondary)]">{currentStep.description}</p>
              </div>
              <span
                className={cn(
      ...
      <div className="p-4 border-t border-[var(--sys-border-hairline)] bg-[var(--sys-bg-subtle)]">
        <div className="flex items-center justify-between">
          <div className="text-sm text-[var(--sys-text-secondary)]">
            Step {workflowState.steps.findIndex(s => s.id === workflowState.currentStepId) + 1} of{" "}
