...
export const RuleAnalytics: React.FC<RuleAnalyticsProps> = ({
  ...
  const [sortBy, setSortBy] = useState<
    "totalExecutions" | "successRate" | "totalCost" | "averageExecutionTime"
  >("totalExecutions");
  ...
  const overallStats = useMemo(() => {
    ...
    const avgExecutionTime =
      total > 0 ? filteredExecutions.reduce((sum, e) => sum + e.executionTime, 0) / total : 0;
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      case "critical":
        return "text-[var(--sys-status-error)] bg-[var(--sys-status-error)]/20";
      default:
    ...
    <div className="space-y-6">
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => onResolveAlert(alert.id)}>
                    Resolve
                  </Button>
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <div className="text-xs text-[var(--sys-text-tertiary)] mt-1">per execution</div>
        </Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--sys-border-hairline)]">
                  <th className="text-left py-3 px-4 font-medium text-[var(--sys-text-primary)]">
                    Rule Name
                  </th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--sys-text-primary)]">
                    Executions
                  </th>
                  <th className="text-right py-3 px-4 font-medium text-[var(--sys-text-primary)]">
                    Success Rate
                  ...
                  <th className="text-center py-3 px-4 font-medium text-[var(--sys-text-primary)]">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody>
                {filteredMetrics.map(metric => (
                  <tr
                    key={metric.ruleId}
                    className="border-b border-[var(--sys-border-hairline)] hover:bg-[var(--sys-fill-low)]"
                  >
        ...
        <CardContent>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-[var(--sys-text-secondary)]">
                      {execution.triggeredBy} • {execution.conditions} conditions •{" "}
          ...
          {filteredExecutions.length > 10 && (
            <div className="mt-4 text-center">
              <Button variant="outline" onClick={() => onExportExecutions(filteredExecutions)}>
