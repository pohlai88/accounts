...
export const CloseRoom: React.FC<CloseRoomProps> = ({
  ...
  const progressMetrics = React.useMemo(() => {
    ...
    const overdue = currentPeriod.tasks.filter(
    ...
    >
      ...
      <div className="p-6 border-b border-[var(--sys-border-hairline)]">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[var(--sys-text-primary)]">Close Room</h1>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="font-medium text-[var(--sys-text-primary)]">{task.title}</h3>
                  <span
                    className={cn(
                ...
                <p className="text-sm text-[var(--sys-text-secondary)] mb-3">{task.description}</p>
              ...
              <div className="flex items-center gap-2">
                {!isPeriodLocked && (
                  <select
                    value={task.status}
                    onChange={e =>
                      handleTaskStatusChange(task.id, e.target.value as CloseTask["status"])
                    }
