...
function generateSecurityRecommendations(
  ...
  if (
    (stats as { totalEvents?: number }).totalEvents &&
    (stats as { totalEvents: number }).totalEvents > 10000
  ) {
    recommendations.push("High security event volume - consider log rotation");
  }

  if (
    (stats as { recentEvents?: number }).recentEvents &&
    (stats as { recentEvents: number }).recentEvents > 100
  ) {
