...
export class CompanyHierarchyService {
   ...
   */
  static async getCompanyRelationships(companyId: string): Promise<{
    success: boolean;
    relationships?: CompanyRelationship[];
    error?: string;
  }> {
    try {
      const { data: relationships, error } = await supabase.rpc("get_company_hierarchy", {
   ...
   */
  static async getConsolidatedStructure(rootCompanyId: string): Promise<{
    success: boolean;
    structure?: ConsolidatedStructure[];
    error?: string;
  }> {
    try {
      const { data: structure, error } = await supabase.rpc("get_consolidated_structure", {
  ...
  static async validateIntercompanyEligibility(
    ...
    try {
      const { data: eligible, error } = await supabase.rpc("validate_intercompany_eligibility", {
   ...
   */
  static async getAvailableIntercompanyCompanies(companyId: string): Promise<{
    success: boolean;
    companies?: Company[];
    error?: string;
  }> {
  ...
  static async updateRelationship(
    ...
    updates: Partial<
      Pick<
        CompanyHierarchy,
        "relationship_type" | "ownership_percentage" | "is_active" | "established_date"
      >
    >,
  ): Promise<{
    success: boolean;
   ...
   */
  static async deactivateRelationship(relationshipId: string): Promise<{
    success: boolean;
    error?: string;
  }> {
    try {
   ...
   */
  static async hasRelationships(companyId: string): Promise<{
    success: boolean;
   ...
   */
  static async getRelationshipDetails(relationshipId: string): Promise<{
    success: boolean;
    relationship?: CompanyHierarchy & {
