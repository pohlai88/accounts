...
export class PortalService {
   ...
   */
  static async createPortalConfiguration(
    input: CreatePortalConfigurationInput,
  ): Promise<ApiResponse<PortalConfiguration>> {
    try {
   ...
   */
  static async getPortalConfiguration(
    companyId: string,
   ...
   */
  static async authenticatePortalUser(
    email: string,
    password: string,
    portalType: PortalType,
  ): Promise<
   ...
   */
  static async getPortalUserDocuments(
    portalUserId: string,
    filters?: {
      document_type?: DocumentType;
      search?: string;
   ...
   * Send portal message
   */
  static async sendPortalMessage(
    input: CreatePortalMessageInput,
   ...
   */
  static async processPortalPayment(
    input: ProcessPortalPaymentInput,
  ): Promise<ApiResponse<PortalPayment>> {
    try {
   ...
   */
  static async getPortalUserDashboard(
    portalUserId: string,
   ...
   */
  private static async createPortalSession(
    portalUserId: string,
  ...
  private static async processPaymentThroughGateway(
    paymentId: string,
    gatewayData: any,
  ): Promise<void> {
  ...
  private static calculateResponseDueDate(priority: TicketPriority): string {
    const hours =
      priority === "Urgent" ? 2 : priority === "High" ? 8 : priority === "Medium" ? 24 : 48;
    return new Date(Date.now() + hours * 60 * 60 * 1000).toISOString();
  }

  private static calculateResolutionDueDate(priority: TicketPriority): string {
    const days =
      priority === "Urgent" ? 1 : priority === "High" ? 3 : priority === "Medium" ? 7 : 14;
