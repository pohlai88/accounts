...
export async function GET(request: NextRequest) {
  try {
    ...
    if (params.search) {
      query = query.or(
        `name.ilike.%${params.search}%,customer_name.ilike.%${params.search}%,customer_po_no.ilike.%${params.search}%`,
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const deliveryDate =
      validatedData.delivery_date || calculateDeliveryDate(validatedData.transaction_date);
