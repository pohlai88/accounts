...
class SecurityAuditor {
  ...
  private checkAuthenticationSecurity() {
    ...
    if (!this.hasStrongPasswordPolicy()) {
      authIssues.push({
        ...
        recommendation:
          "Implement strong password requirements (min 8 chars, mixed case, numbers, symbols)",
        status: "open",
        detectedAt: Date.now(),
      });
    }
  ...
  public generateAuditReport(): SecurityAuditReport {
    ...
    const score = Math.max(
      0,
      100 - criticalVulns * 20 - highVulns * 10 - mediumVulns * 5 - lowVulns * 2,
    );
