...
export class ComplianceService {
  ...
  static async getComplianceFrameworks(filters?: {
    ...
    try {
      let query = supabase.from("compliance_frameworks").select("*");
   ...
   */
  static async createSecurityControl(
    input: CreateSecurityControlInput,
   ...
   */
  static async getSecurityControls(
    companyId: string,
    filters?: {
      framework_id?: string;
      implementation_status?: ImplementationStatus;
  ...
  static async createDataSubjectRequest(
    ...
    try {
      ...
      return {
        success: true,
        data: request,
        message: `Data subject request ${requestNumber} created successfully`,
      };
    } catch (error) {
  ...
  static async createDataBreachIncident(
    ...
    try {
      ...
      const { data: incidentNumber, error: numberError } = await supabase.rpc(
      ...
      return {
        success: true,
        data: incident,
        message: `Data breach incident ${incidentNumber} created successfully`,
      };
    } catch (error) {
   ...
   */
  static async getDataBreachIncidents(
    companyId: string,
    filters?: {
      investigation_status?: InvestigationStatus;
      risk_level?: RiskLevel;
    ...
    try {
      let query = supabase.from("data_breach_incidents").select("*").eq("company_id", companyId);
   ...
   */
  static async getAccessControlEntries(
    companyId: string,
    filters?: {
      subject_type?: SubjectType;
      resource_type?: string;
    ...
    try {
      let query = supabase.from("access_control_matrix").select("*").eq("company_id", companyId);
   ...
   */
  static async createDataClassification(
    input: CreateDataClassificationInput,
  ...
  static async getDataClassifications(
    ...
    try {
      let query = supabase.from("data_classification").select("*").eq("company_id", companyId);
  ...
  static async getComplianceDashboard(
    ...
    try {
      ...
      // Get other metrics
      const [controlsResult, dsrResult, breachesResult, auditResult, accessResult] =
        await Promise.all([
          supabase
            .from("security_controls")
            .select("effectiveness_rating", { count: "exact" })
            .eq("company_id", companyId),
          supabase
            .from("data_subject_requests")
          ...
          supabase
            .from("data_breach_incidents")
            .select("investigation_status,risk_level", { count: "exact" })
            .eq("company_id", companyId),
          supabase
            .from("system_audit_logs")
            .select("*")
          ...
          supabase
            .from("access_control_matrix")
            .select("next_review_date", { count: "exact" })
            .eq("company_id", companyId)
            .eq("status", "Active"),
        ]);
      ...
      const effectiveControls =
        controlsResult.data?.filter(c => c.effectiveness_rating === "Effective").length || 0;

      const openDSRs =
        dsrResult.data?.filter(r => !["Completed", "Rejected"].includes(r.status)).length || 0;
      const overdueDSRs =
        dsrResult.data?.filter(
          r => !["Completed", "Rejected"].includes(r.status) && new Date(r.due_date) < new Date(),
        ).length || 0;

      const openBreaches =
        breachesResult.data?.filter(b => b.investigation_status === "Open").length || 0;
      const highRiskBreaches =
        breachesResult.data?.filter(b => b.risk_level === "High" || b.risk_level === "Critical")
          .length || 0;

      const overdueAccessReviews =
        accessResult.data?.filter(
          a => a.next_review_date && new Date(a.next_review_date) < new Date(),
        ).length || 0;
