...
export class LocalizationService {
  ...
  static async getCountries(
    ...
    try {
      let query = supabase.from("countries").select("*").eq("is_active", true);
   ...
   */
  static async getLocalizationPackDetails(packId: string): Promise<
    ApiResponse<{
   ...
   */
  static async getCompanyLocalizationSettings(
    companyId: string,
  ): Promise<ApiResponse<CompanyLocalizationSettings[]>> {
    try {
      const { data: settings, error } = await supabase
   ...
   */
  static async getLocalizationStatistics(): Promise<
    ApiResponse<{
  ...
  > {
    try {
      // Get basic counts
      const [countriesResult, packsResult, publishedResult, certifiedResult, premiumResult] =
        await Promise.all([
      ...
      const packCounts =
        popularPacks?.reduce((acc: Record<string, any>, setting: any) => {
    ...
    try {
      ...
      if (applyChartOfAccounts && packDetails.data.chartTemplates.length > 0) {
        const defaultTemplate =
          packDetails.data.chartTemplates.find(t => t.is_default) ||
          packDetails.data.chartTemplates[0];
      ...
      if (applyTaxTemplates && packDetails.data.taxConfigs.length > 0) {
        ...
        for (const taxType of taxConfig.tax_types) {
          try {
            const { error } = await supabase.from("tax_templates").insert([
              {
   ...
   */
  static async getAvailableRegions(): Promise<
    ApiResponse<Array<{ region: string; country_count: number }>>
  > {
    try {
      ...
      const regionCounts =
        regions?.reduce((acc: Record<string, number>, country) => {
      ...
      const pricing =
        packs?.map((pack: any) => {
          ...
          const savingsPercentage =
            monthlyTotal > 0 ? ((monthlyTotal - annualTotal) / monthlyTotal) * 100 : 0;
  ...
  static async searchLocalizationPacks(
    ...
    try {
      ...
      if (searchTerm) {
        query = query.or(
          `pack_name.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,countries.country_name.ilike.%${searchTerm}%`,
        );
      }
