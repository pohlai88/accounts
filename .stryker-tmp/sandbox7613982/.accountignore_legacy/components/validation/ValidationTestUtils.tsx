...
export const createMockWarning = (
...
export const ValidationTestScenarios = {
  ...
  withSuggestions: (count: number = 2): ValidationResult => {
    const suggestions = Array.from({ length: count }, (_, i) => `Test suggestion ${i + 1}`);
  ...
  mixed: (): ValidationResult =>
    createMockValidation({
      ...
      suggestions: ["Consider using a shorter description", "Verify the account selection"],
    }),
...
}> = ({ children, validation, i18n, theme, locale, themeVariant = "default" }) => {
  return (
...
export const renderWithValidation = (ui: React.ReactElement, options: CustomRenderOptions = {}) => {
...
export const ValidationTestAssertions = {
  ...
  expectValidationCounts(errorCount: number, warningCount: number, suggestionCount: number = 0) {
    ...
    if (suggestionCount > 0) {
      expect(
        screen.getByText(new RegExp(`${suggestionCount}.*suggestion`, "i")),
      ).toBeInTheDocument();
    }
  },
...
export const ValidationTestData = {
  ...
  generateFieldNames(count: number): string[] {
    ...
    return Array.from(
      { length: count },
      (_, i) =>
        fieldTypes[i % fieldTypes.length] +
        (i >= fieldTypes.length ? `_${Math.floor(i / fieldTypes.length)}` : ""),
    );
  },
  ...
  generateErrorMessages(count: number): string[] {
    ...
    return Array.from(
      { length: count },
      (_, i) =>
        messageTemplates[i % messageTemplates.length] +
        (i >= messageTemplates.length ? ` (${i})` : ""),
    );
  },
  ...
  generateRealisticValidation(
    ...
    const suggestions = Array.from(
      ...
      (_, i) => `Consider reviewing ${fieldNames[i % fieldNames.length]} for better accuracy`,
    );
